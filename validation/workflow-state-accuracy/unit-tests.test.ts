...
describe("Workflow Chat Sync Store", () => {
  ...
  describe("State Synchronization", () => {
    ...
    test("should generate accurate workflow state representation", () => {
      ...
      vi.mocked(
        require("@/stores/workflows/registry/store").useWorkflowRegistry
          .getState,
    ...
    test("should handle workflow state changes correctly", () => {
      ...
      syncStore.generateWorkflowStateRepresentation = vi
        .fn()
        .mockReturnValue(mockRepresentation);
    ...
    test("should handle execution state changes correctly", () => {
      ...
      const endMessage =
        syncStore.generateStateChangeMessage(executionEndEvent);
      ...
      expect(chatStore.addMessage).toHaveBeenCalledWith(
        expect.objectContaining({
          content: expect.stringContaining(
            "✗ Failed to delete non-existent block",
          ),
          type: "workflow",
        }),
      );
  ...
  describe("Performance", () => {
    test("should handle rapid state changes efficiently", () => {
      ...
      expect(
        syncStore.generateWorkflowStateRepresentation,
