...
export class WorkflowStateAccuracyValidator {
  ...
  private async testVisualToChat(): Promise<TestResult> {
    ...
    try {
      ...
      return {
        ...
        details: 'Block addition in visual interface correctly propagated to chat',
      }
    } catch (error) {
      return {
        name: 'Visual to Chat Sync',
  ...
  private async testBidirectionalSync(): Promise<TestResult> {
    ...
    try {
      ...
      const connectionCommand = this.mockSyncStore.parseChatCommand('connect LLM 1 to LLM 2')
      ...
      const chatState = this.mockSyncStore.generateWorkflowStateRepresentation()

      // Both should reflect the same underlying workflow structure
      expect(chatState.blockSummaries.length).toBe(Object.keys(visualState.blocks).length)
      expect(chatState.connectionSummaries.length).toBe(visualState.edges.length)

      return {
        name: 'Bidirectional Sync',
        passed: true,
        duration: Date.now() - startTime,
        details: 'Visual and chat interfaces maintain consistent state representation',
      }
    } catch (error) {
      return {
        name: 'Bidirectional Sync',
  ...
  private async testStateRepresentationAccuracy(): Promise<TestResult> {
    ...
    try {
      ...
      const edges = [
        { id: 'e1', source: 'b1', target: 'b2', sourceHandle: 'response', targetHandle: 'input' },
        { id: 'e2', source: 'b2', target: 'b3', sourceHandle: 'response', targetHandle: 'input' },
      ]
      ...
      const representation = this.mockSyncStore.generateWorkflowStateRepresentation()
      ...
      const activeBlocks = representation.blockSummaries.filter((b) => b.isActive)
      expect(activeBlocks).toHaveLength(2)

      const disabledBlocks = representation.blockSummaries.filter((b) => !b.isEnabled)
      expect(disabledBlocks).toHaveLength(1)

      // Validate connection descriptions
      expect(representation.connectionSummaries[0].description).toBe('Primary LLM → API Call')
      expect(representation.connectionSummaries[1].description).toBe('API Call → Decision')
      ...
      return {
        ...
        duration: Date.now() - startTime,
        error: error instanceof Error ? error.message : 'Unknown error',
        details: 'State representation does not accurately reflect workflow state',
      }
    }
  }
  ...
  private async testExecutionStateUpdates(): Promise<TestResult> {
    ...
    try {
      ...
      return {
        ...
        passed: true,
        duration: Date.now() - startTime,
        details: 'Execution state changes are immediately reflected in chat interface',
      }
    } catch (error) {
      return {
        name: 'Execution State Updates',
  ...
  private async testActiveBlockUpdates(): Promise<TestResult> {
    ...
    try {
      ...
      for (const blockId of activeBlocks) {
        ...
        const representation = this.mockSyncStore.generateWorkflowStateRepresentation()
        const activeBlockSummaries = representation.blockSummaries.filter((b) => b.isActive)
  ...
  private async testWorkflowStructureUpdates(): Promise<TestResult> {
    ...
    try {
      ...
      return {
        ...
        passed: true,
        duration: Date.now() - startTime,
        details: 'All workflow structure changes propagated to chat in real-time',
      }
    } catch (error) {
      return {
        name: 'Workflow Structure Updates',
  ...
  private async testConcurrentUpdates(): Promise<TestResult> {
    ...
    try {
      ...
      const allUpdates = [...visualUpdates, ...chatUpdates, ...executionUpdates]

      await Promise.all(
        allUpdates.map((update) => this.mockSyncStore.handleWorkflowStateChange(update))
      )
      ...
      return {
        ...
        passed: true,
        duration: Date.now() - startTime,
        details: 'Successfully handled 120 concurrent updates without conflicts',
      }
    } catch (error) {
      return {
        name: 'Concurrent Updates',
  ...
  private compileResults(category: string, results: TestResult[]): ValidationResult {
