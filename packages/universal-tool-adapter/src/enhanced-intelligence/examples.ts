...
export async function example2_ContextAnalysis() {
  ...
  const analysisRequest: ContextAnalysisRequest = {
    userMessage: "The authentication system is failing intermittently. Users sometimes get logged out unexpectedly, and there are 401 errors in the console. The JWT tokens seem to expire randomly.",
    conversationHistory: [
      {
        role: "user",
...
export async function example3_UsageAnalytics() {
  ...
  try {
    ...
    insights.mostUsedTools.forEach((tool, index) => {
      console.log(`${index + 1}. ${tool.name} (${tool.count} uses, ${(tool.successRate * 100).toFixed(1)}% success)`);
    });
    ...
    if (insights.recommendationMetrics) {
      ...
      console.log(`Click-through Rate: ${(insights.recommendationMetrics.clickThroughRate * 100).toFixed(1)}%`);
      console.log(`User Satisfaction: ${(insights.recommendationMetrics.userSatisfaction * 100).toFixed(1)}%`);
    }
...
export async function example4_AdvancedConfiguration() {
  ...
  const seniorDevProfile: UserProfile = {
    ...
    frameworks: [
      "React", "Node.js", "FastAPI", "Kubernetes"
    ],
    workPatterns: {
  ...
  const architecturalRequest: ContextualRecommendationRequest = {
    context: {
      ...
      userMessage: "I need to architect a payment system that can handle 10k+ transactions per second with 99.99% uptime requirements. Looking for patterns, tools, and technologies.",
      conversationHistory: [
        {
          role: "user",
  ...
  try {
    ...
    recommendations.forEach((rec, index) => {
      ...
      if (rec.metadata) {
        console.log(`   Metadata: Complexity=${rec.metadata.complexity}, Domain=${rec.metadata.domain}`);
      }
    });
...
export async function example5_ExternalIntegration() {
  ...
  const integrationRequest: ContextualRecommendationRequest = {
    context: {
      ...
      userMessage: "Our database queries are slow and affecting user experience. Need tools to identify bottlenecks and optimize performance.",
      workspaceContext: {
        technology_stack: {
          backend: "Django",
          database: "PostgreSQL",
...
export async function example6_PerformanceMonitoring() {
  ...
  try {
    ...
    const avgRecommendations = performanceResults.reduce((sum, r) => sum + r.recommendationCount, 0) / performanceResults.length;
    const avgConfidence = performanceResults.reduce((sum, r) => sum + r.avgConfidence, 0) / performanceResults.length;
...
export async function runAllExamples() {
  ...
  try {
    ...
    console.log(`- Context analysis: ${results.example2?.identifiedIssues?.length || 0} issues identified`);
    console.log(`- Analytics insights: ${results.example3?.totalUsage || 0} total tool uses tracked`);
    console.log(`- Advanced config: ${results.example4?.recommendations?.length || 0} personalized recommendations`);
    console.log(`- External integration: ${results.example5?.length || 0} integrated recommendations`);
    console.log(`- Performance test: ${results.example6?.performanceMetrics?.avgDuration?.toFixed(1) || 0}ms average response time`);
