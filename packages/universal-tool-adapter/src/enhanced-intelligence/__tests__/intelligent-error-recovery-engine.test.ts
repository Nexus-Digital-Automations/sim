    ...
    return [
      {
        action: 'retry',
        confidence: 0.9,
        reasoning: 'Transient network issues often resolve with retry',
      },
...
describe('IntelligentErrorRecoveryEngine', () => {
  ...
  describe('Recovery Plan Generation', () => {
    ...
    it('should prioritize recovery actions by success probability', async () => {
      ...
      for (let i = 0; i < actions.length - 1; i++) {
        expect(actions[i].successProbability).toBeGreaterThanOrEqual(
          actions[i + 1].successProbability
        )
      }
    })
  ...
  describe('Edge Cases and Error Handling', () => {
    it('should handle null or undefined errors gracefully', async () => {
      ...
      await expect(recoveryEngine.generateRecoveryPlan(null as any, context)).rejects.toThrow(
        'Invalid error object provided'
      )

      await expect(recoveryEngine.generateRecoveryPlan(undefined as any, context)).rejects.toThrow(
        'Invalid error object provided'
      )
    })
    ...
    it('should handle invalid context gracefully', async () => {
      ...
      await expect(recoveryEngine.generateRecoveryPlan(error, null as any)).rejects.toThrow(
        'Invalid context provided'
      )

      await expect(recoveryEngine.generateRecoveryPlan(error, {} as any)).rejects.toThrow(
        'Invalid context provided'
      )
    })
