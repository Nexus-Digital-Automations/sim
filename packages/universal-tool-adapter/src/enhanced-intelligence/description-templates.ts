...
export interface ComplexityIndicator {
  metric: string
  threshold: any
  indication: 'simple' | 'moderate' | 'complex'
}
...
export interface GuidanceIntensity {
  level: 'minimal' | 'moderate' | 'intensive'
  patterns: string[]
}
export interface ConfidenceBuilder {
  technique: string
  application: string
}
...
export interface TemplateQualityMetric {
  metric: string
  target: number
}
...
export const COMMUNICATION_TEMPLATES: EnhancedDescriptionTemplate = {
  ...
  briefTemplate: {
    summaryPattern:
      'Connect and communicate with {recipients} through {platform} using {primaryMethod}',
  ...
  detailedTemplate: {
    overviewPattern:
      'Streamline your communication workflow with {platform}, enabling seamless {communicationType} across {channels}. Perfect for {primaryAudience} who need to {mainGoal}.',
    functionalityPattern:
      'Core capabilities include {features}, supporting {workflows} with {integrations}',
    useCaseTemplates: [
      {
        scenario: 'team-coordination',
        pattern: 'Coordinate {teamType} activities through {communicationMethod}',
        difficulty: 'beginner',
      },
      {
        scenario: 'customer-outreach',
        pattern: 'Reach customers with {messageType} for {campaignType}',
        difficulty: 'intermediate',
      },
      {
        scenario: 'automated-notifications',
        pattern: 'Set up automated {notificationType} based on {triggers}',
        difficulty: 'advanced',
      },
    ],
    benefitsPattern: [
    ...
    integrationPattern:
      'Integrates with {ecosystem} through {integrationMethods}, supporting {dataFormats} and {authMethods}',
  },

  expertTemplate: {
    architecturePattern:
      'Built on {architecturePattern} architecture with {scalabilityFeatures}, supporting {concurrencyModel} and {reliabilityMechanisms}',
    configurationTemplates: [
      {
        aspect: 'authentication',
        pattern: 'Configure {authMethod} with {credentials} and {scopes}',
        examples: ['OAuth 2.0', 'API Keys', 'Service Accounts'],
      },
      {
        aspect: 'rate-limiting',
        pattern: 'Implement {rateLimitStrategy} with {limits} and {backoffStrategy}',
        examples: ['Token bucket', 'Fixed window', 'Sliding window'],
      },
    ],
    performanceTemplates: [
      {
        metric: 'throughput',
        pattern: 'Supports up to {messagesPerSecond} messages per second',
        benchmarks: { email: 100, slack: 1000, sms: 50 },
      },
      {
        metric: 'latency',
        pattern: 'Average delivery time of {averageLatency}ms',
        benchmarks: { email: 2000, slack: 100, sms: 500 },
      },
    ],
    securityTemplates: [
      {
        aspect: 'encryption',
        pattern: 'Messages encrypted using {encryptionStandard} with {keyManagement}',
        compliance: ['GDPR', 'SOC 2', 'HIPAA'],
      },
      {
        aspect: 'access-control',
        pattern: 'Role-based access control with {permissionModel}',
    ...
    troubleshootingTemplates: [
      {
        ...
        pattern: 'Authentication failures typically caused by {commonCauses}',
        escalation: 'Check {authEndpoint} status and {credentialValidity}',
      },
      {
        category: 'delivery',
        pattern: 'Message delivery issues often related to {deliveryFactors}',
        escalation: 'Verify {recipientDetails} and {platformStatus}',
      },
    ],
    extensibilityPattern:
      'Extend functionality through {extensionPoints} supporting {customizationOptions} and {integrationHooks}',
  },
  ...
  roleTemplates: {
    business_user: {
      ...
      perspectiveAdjustments: [
        {
          aspect: 'complexity',
          adjustment: 'Focus on business outcomes rather than technical implementation',
        },
        {
          aspect: 'workflow',
          adjustment: 'Emphasize integration with existing business processes',
        },
      ],
      relevanceFilters: [
      ...
      recommendedApproachPattern:
        'Start with {basicUseCase}, expand to {advancedUseCases} as comfort grows',
    },
    developer: {
      role: 'developer',
      perspectiveAdjustments: [
        {
          ...
          adjustment: 'Provide detailed API documentation and code examples',
        },
        {
          aspect: 'integration',
          adjustment: 'Focus on technical integration patterns and best practices',
        },
      ],
      relevanceFilters: [
      ...
      recommendedApproachPattern:
        'Review {apiDocumentation}, implement {basicIntegration}, optimize with {advancedFeatures}',
    },
    admin: {
      role: 'admin',
      perspectiveAdjustments: [
        {
          aspect: 'security',
          adjustment: 'Highlight security features and compliance capabilities',
        },
        {
          aspect: 'management',
          adjustment: 'Emphasize user management and organizational control',
        },
      ],
      relevanceFilters: [
      ...
      recommendedApproachPattern:
        'Establish {securityPolicies}, configure {userPermissions}, monitor {usageMetrics}',
    },
    analyst: {
      role: 'analyst',
      perspectiveAdjustments: [
      ...
      recommendedApproachPattern:
        'Define {keyMetrics}, establish {reportingCadence}, analyze {performancePatterns}',
    },
    manager: {
      role: 'manager',
      perspectiveAdjustments: [
        {
          aspect: 'team_impact',
          adjustment: 'Emphasize team productivity and collaboration benefits',
        },
      ...
      recommendedApproachPattern:
        'Pilot with {smallTeam}, measure {impactMetrics}, scale to {fullOrganization}',
    },
    researcher: {
      role: 'researcher',
      perspectiveAdjustments: [
        { aspect: 'methodology', adjustment: 'Focus on research applications and data collection' },
        {
          ...
          adjustment: 'Emphasize academic and research collaboration features',
        },
      ],
      relevanceFilters: [
    ...
    designer: {
      ...
      recommendedApproachPattern:
        'Integrate with {designTools}, establish {feedbackLoops}, optimize {creativeWorkflow}',
    },
    qa_tester: {
      role: 'qa_tester',
      perspectiveAdjustments: [
      ...
      recommendedApproachPattern:
        'Design {testStrategy}, implement {automatedTests}, track {qualityMetrics}',
    },
  },
  ...
  skillTemplates: {
    beginner: {
      ...
      complexityAdjustment: {
        factor: 0.3,
        simplifications: ['step-by-step guidance', 'visual aids', 'common examples'],
      },
      ...
      confidenceBuilders: [
        {
          technique: 'quick_wins',
          application: 'Start with simple tasks that provide immediate value',
        },
        {
          technique: 'progressive_disclosure',
          application: 'Introduce complexity gradually as skills develop',
        },
      ],
    },
    intermediate: {
      ...
      complexityAdjustment: {
        factor: 0.7,
        simplifications: ['focused examples', 'best practices', 'common patterns'],
      },
      prerequisiteTemplates: [
    ...
    advanced: {
      ...
      complexityAdjustment: {
        factor: 0.9,
        simplifications: ['architectural patterns', 'optimization techniques', 'edge cases'],
      },
      prerequisiteTemplates: [
    ...
    expert: {
      ...
      complexityAdjustment: {
        factor: 1.0,
        simplifications: ['full complexity', 'edge cases', 'extensibility'],
      },
      prerequisiteTemplates: [
      ...
      confidenceBuilders: [
        {
          technique: 'thought_leadership',
          application: 'Opportunities to contribute to tool evolution',
        },
  ...
  usagePatterns: [
    {
      ...
      variationPoints: [
        ...
        {
          parameter: 'recipient',
          variations: ['individual', 'team', 'channel', 'distribution list'],
        },
      ],
    },
    {
      ...
      instructionPattern:
        'Schedule {messageType} to {recipient} for {scheduledTime} with content: {messageContent}',
      expectedOutcomePattern: 'Message scheduled and will be sent at {scheduledTime}',
      variationPoints: [
        {
          parameter: 'scheduledTime',
          variations: ['specific datetime', 'relative time', 'recurring schedule'],
        },
      ],
    },
    {
      ...
      instructionPattern:
        'Send {messageType} to {recipientList} with personalized content: {templateContent}',
      expectedOutcomePattern:
        'Messages sent to {recipientCount} recipients with {successRate} success rate',
      variationPoints: [
        {
          parameter: 'recipientList',
          variations: ['CSV file', 'database query', 'predefined list'],
        },
        {
          parameter: 'templateContent',
          variations: ['static message', 'personalized template', 'dynamic content'],
        },
      ],
    },
  ],
  ...
  conversationalTemplates: [
    {
      ...
      responsePattern:
        'I can help you send a {messageType} to {recipient}. What would you like to communicate?',
      followUpSuggestions: [
        'Would you like to schedule this for later?',
        'Should I add anyone else to this message?',
      ],
    ...
    },
    {
      ...
      responsePattern:
        'I can schedule your {messageType} for {scheduledTime}. What content would you like to include?',
      followUpSuggestions: [
        'Would you like to set up a recurring message?',
        'Should I remind you before it sends?',
      ],
      clarificationQuestions: [
        'When should this be sent?',
        'Is this a one-time or recurring message?',
      ],
    },
    {
      ...
      responsePattern:
        'I can help you send a {messageType} to multiple recipients. How would you like to define your audience?',
      followUpSuggestions: [
        'Would you like to personalize messages for each recipient?',
        'Should I track delivery and engagement?',
      ],
      clarificationQuestions: [
        'Who should receive these messages?',
        'Do you have a template or should I help create one?',
      ],
    },
  ],
  ...
  validationRules: [
    ...
    {
      rule: 'All role templates should have balanced perspective adjustments',
      severity: 'warning',
    },
  ...
  qualityMetrics: [
    ...
    { metric: 'contextual_relevance', target: 0.9 },
  ],

  customizationPoints: [
    {
      point: 'message_templates',
      options: ['dynamic substitution', 'conditional content', 'multi-language'],
    },
  ...
  extensionMechanisms: [
    {
      extensionType: 'custom_channels',
      mechanism: 'Plugin architecture for new communication platforms',
    },
    {
      extensionType: 'message_processing',
      mechanism: 'Hook system for message transformation and validation',
    },
    {
      extensionType: 'analytics_integration',
      mechanism: 'Event system for tracking and analytics',
    },
  ],
}
...
export const DATA_STORAGE_TEMPLATES: EnhancedDescriptionTemplate = {
  ...
  detailedTemplate: {
    overviewPattern:
      'Comprehensive data management solution for {storageType} workloads, supporting {dataModels} with {consistencyGuarantees}',
    functionalityPattern:
      'Core features include {storageOperations}, {queryCapabilities}, and {managementFeatures}',
    useCaseTemplates: [
      {
        scenario: 'transactional-data',
        pattern: 'Handle {transactionVolume} transactions with {consistencyLevel}',
        difficulty: 'intermediate',
      },
      {
        scenario: 'analytics-data',
        pattern: 'Store and analyze {analyticsDataType} for {businessInsights}',
        difficulty: 'advanced',
      },
      {
        scenario: 'content-storage',
        pattern: 'Manage {contentType} with {accessPatterns}',
        difficulty: 'beginner',
      },
    ],
    benefitsPattern: [
  ...
  expertTemplate: {
    architecturePattern:
      'Built on {databaseArchitecture} with {distributionStrategy} and {replicationModel}',
    configurationTemplates: [
      {
        aspect: 'performance',
        pattern: 'Optimize for {workloadType} with {performanceTuning}',
        examples: ['indexing strategies', 'connection pooling', 'caching layers'],
      },
      {
        aspect: 'reliability',
        pattern: 'Configure {backupStrategy} with {recoveryObjectives}',
        examples: ['point-in-time recovery', 'cross-region replication', 'automated backups'],
      },
    ],
    performanceTemplates: [
      {
        metric: 'throughput',
        pattern: 'Supports {operationsPerSecond} operations per second',
        benchmarks: { reads: 10000, writes: 5000, queries: 1000 },
      },
      {
        metric: 'latency',
        pattern: 'Average response time of {latencyTarget}ms',
        benchmarks: { simple_query: 10, complex_query: 100, write_operation: 20 },
      },
    ],
    securityTemplates: [
      {
        aspect: 'access_control',
        pattern: 'Row-level security with {authorizationModel}',
        compliance: ['SOX', 'GDPR', 'HIPAA'],
      },
      {
        aspect: 'encryption',
        pattern: 'Data encrypted at rest and in transit using {encryptionStandards}',
    ...
    troubleshootingTemplates: [
      {
        ...
        pattern: 'Performance issues typically caused by {performanceBottlenecks}',
        escalation: 'Check {performanceMetrics} and optimize {performanceAreas}',
      },
      {
        category: 'connectivity',
        pattern: 'Connection problems often related to {connectivityIssues}',
        escalation: 'Verify {networkConfiguration} and {authenticationSettings}',
      },
    ],
  ...
  usagePatterns: [
      ...
      variationPoints: [
        {
          ...
          variations: ['simple equality', 'range queries', 'complex joins'],
        },
    ...
    },
    {
      ...
      instructionPattern:
        'Perform bulk {operation} on {dataSet} with {batchSize} and {errorHandling}',
      expectedOutcomePattern: 'Processed {totalRecords} records with {successRate} success rate',
      variationPoints: [
        {
          parameter: 'batchSize',
          variations: ['small batches (100)', 'medium batches (1000)', 'large batches (10000)'],
        },
        {
          parameter: 'errorHandling',
          variations: ['fail-fast', 'continue-on-error', 'rollback-on-error'],
        },
      ],
    },
  ],
  ...
  conversationalTemplates: [
    {
      ...
      responsePattern:
        'I can help you store {dataType} in {storageLocation}. What information would you like to save?',
      followUpSuggestions: [
        'Would you like to validate the data before storing?',
        'Should I set up automatic backups?',
      ],
      clarificationQuestions: [
    ...
    },
    {
      ...
      responsePattern:
        'I can help you find {dataType} using {queryMethod}. What criteria should I use?',
      followUpSuggestions: [
        'Would you like to export the results?',
        'Should I set up alerts for similar data?',
      ],
      clarificationQuestions: [
        'What are you looking for?',
        'What conditions should I apply?',
    ...
    },
    {
      ...
      responsePattern:
        'I can help you update {dataType} in {storageLocation}. What changes would you like to make?',
      followUpSuggestions: [
        'Would you like to backup before making changes?',
        'Should I validate the updates?',
      ],
      clarificationQuestions: [
  ...
  qualityMetrics: [
    ...
    { metric: 'performance_clarity', target: 0.9 },
  ...
  extensionMechanisms: [
    {
      extensionType: 'custom_types',
      mechanism: 'Plugin system for custom data types and validators',
    },
    { extensionType: 'query_extensions', mechanism: 'Custom query operators and functions' },
    {
      extensionType: 'storage_engines',
      mechanism: 'Pluggable storage engines for different use cases',
    },
  ],
}
...
export const AI_ML_TEMPLATES: EnhancedDescriptionTemplate = {
  ...
  briefTemplate: {
    summaryPattern:
      'Apply {aiCapability} to {inputType} producing {outputType} with {qualityLevel}',
  ...
  detailedTemplate: {
    overviewPattern:
      'Advanced AI capabilities for {applicationDomain}, leveraging {modelArchitecture} to deliver {businessValue}',
    functionalityPattern:
      'Process {inputTypes} through {processingPipeline} to generate {outputTypes}',
    useCaseTemplates: [
      {
        scenario: 'content-generation',
        pattern: 'Generate {contentType} based on {inputCriteria}',
        difficulty: 'intermediate',
      },
      {
        scenario: 'data-analysis',
        pattern: 'Analyze {dataType} to extract {insights}',
        difficulty: 'advanced',
      },
      {
        scenario: 'automation',
        pattern: 'Automate {taskType} using {aiTechnique}',
        difficulty: 'beginner',
      },
    ],
    benefitsPattern: [
    ...
    integrationPattern:
      'Integrate through {apiInterface} with {supportedFormats} and {authenticationMethods}',
  },

  expertTemplate: {
    architecturePattern:
      'Built on {modelArchitecture} with {trainingApproach} and {optimizationTechniques}',
    configurationTemplates: [
      {
        aspect: 'model_parameters',
        pattern: 'Configure {hyperparameters} for optimal {performanceMetric}',
        examples: ['temperature', 'top-p', 'max-tokens'],
      },
      {
        aspect: 'inference_optimization',
        pattern: 'Optimize inference with {optimizationTechniques}',
        examples: ['quantization', 'caching', 'batching'],
      },
    ],
    performanceTemplates: [
      {
        ...
        benchmarks: { classification: 0.95, generation: 0.87, extraction: 0.92 },
      },
      {
        metric: 'speed',
        pattern: 'Processing speed of {tokensPerSecond} tokens per second',
        benchmarks: { text: 100, code: 75, chat: 150 },
      },
    ],
    securityTemplates: [
      {
        aspect: 'data_privacy',
        pattern: 'Protects user data with {privacyMeasures}',
        compliance: ['GDPR', 'CCPA', 'SOC 2'],
      },
      {
        aspect: 'model_security',
        pattern: 'Implements {securityMeasures} against {threatTypes}',
        compliance: ['AI Ethics', 'Responsible AI'],
      },
    ],
    troubleshootingTemplates: [
      {
        category: 'quality',
        pattern: 'Output quality issues often caused by {qualityFactors}',
        escalation: 'Adjust {parameters} and review {inputQuality}',
      },
      {
        category: 'performance',
        pattern: 'Performance degradation typically related to {performanceFactors}',
        escalation: 'Check {resourceUtilization} and {modelConfiguration}',
      },
    ],
  ...
  usagePatterns: [
    {
      ...
      expectedOutcomePattern:
        'Generated {contentLength} of {contentType} with {qualityScore} quality',
      variationPoints: [
    ...
    },
    {
      ...
      expectedOutcomePattern:
        'Identified {insightCount} insights with {confidenceLevel} confidence',
      variationPoints: [
        {
          parameter: 'analysisGoal',
          variations: ['trends', 'anomalies', 'classifications', 'predictions'],
        },
        {
          parameter: 'technique',
          variations: ['statistical', 'machine learning', 'deep learning'],
        },
      ],
    },
  ],
  ...
  conversationalTemplates: [
    {
      ...
      responsePattern:
        'I can help you generate {contentType} about {topic}. What style and length would you prefer?',
    ...
    },
    {
      ...
      responsePattern:
        'I can analyze your {dataType} to find {analysisGoal}. What specific insights are you looking for?',
      followUpSuggestions: [
  ...
  qualityMetrics: [
    { metric: 'output_quality', target: 0.9 },
  ...
  customizationPoints: [
    ...
    { point: 'output_format', options: ['text', 'structured', 'multimodal'] },
    {
      ...
      options: ['confidence-scoring', 'human-review', 'automated-validation'],
    },
  ],

  extensionMechanisms: [
    {
      extensionType: 'custom_models',
      mechanism: 'Model registry for custom and fine-tuned models',
    },
    { extensionType: 'preprocessing', mechanism: 'Pipeline system for custom input processing' },
    {
      extensionType: 'postprocessing',
      mechanism: 'Hook system for output filtering and enhancement',
    },
  ],
}
 ...
 */
export function validateDescriptionTemplate(
