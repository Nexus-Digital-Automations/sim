...
export class JourneyGenerator {
  ...
  private readonly blockConverters = new Map<
    WorkflowBlockType,
    BlockConverter
  >();
  ...
  async generateJourney(
    ...
    try {
      console.log(
        `Starting journey generation for template ${request.templateId}`,
      );
      ...
      const optimizedJourney = await this.applyOptimizations(
        journeyStructure,
        request.options.optimizationTargets || [
          "performance",
          "user_experience",
        ],
      ...
      const analytics = await this.generateJourneyAnalytics(
        optimizedJourney,
        request,
      );
      ...
      console.log(
        `Journey generation completed successfully in ${result.duration}ms`,
      );
      return result;
    } catch (error) {
  ...
  private async generateJourneyStructure(
    ...
    console.log(
      `Generating journey structure for ${workflowData.blocks?.length || 0} blocks`,
    );
    ...
    const journey: GeneratedJourney = {
      ...
      description: this.generateJourneyDescription(
        processedTemplate.template,
        request.parameters,
      ),
      agentId: request.agentId,
  ...
  private async convertEdgeToTransition(
    ...
    try {
      const transition: JourneyTransition = {
        ...
        conditions: edge.conditionalConnection
          ? [edge.conditionalConnection]
          : [],
        priority: 100,
        actions: [],
        validations: [],
      ...
      variables.push({
        ...
        validation: {
          rules: param.validation
            ? this.convertValidationRules(param.validation)
            : [],
        },
  ...
  private determineComplexity(
    ...
    return analysis.complexity.overall === "very_complex"
      ? "complex"
      : analysis.complexity.overall;
  }

  private setInitialAndFinalStates(
    journey: GeneratedJourney,
    analysis: WorkflowAnalysis,
  ): void {
    // Set initial states
    for (const entryPoint of analysis.analysis.entryPoints) {
      const state = journey.states.find(
        (s) => s.metadata.sourceBlockId === entryPoint,
      );
      if (state) {
    ...
    for (const exitPoint of analysis.analysis.exitPoints) {
      const state = journey.states.find(
        (s) => s.metadata.sourceBlockId === exitPoint,
      );
      if (state) {
        state.isFinal = true;
      }
