...
async function basicJourneyGeneration() {
  ...
  try {
    ...
    if (result.success && result.journey) {
      ...
      console.log(
        `   Estimated Duration: ${result.journey.estimatedDuration} minutes`,
      );
      ...
      workflowData: {
        blocks: [
          ...
          },
          {
            ...
            data: {
              prompt:
                "Analyze the support ticket and categorize the issue type.",
            ...
            dynamicProperties: [
              {
                ...
                valueSource: {
                  ...
                  computationRule: {
                    type: "template",
                    expression:
                      "Categorize this {{priority}} priority ticket: {{ticketId}}",
...
async function templateAnalytics() {
  ...
  try {
    ...
    const analytics = await templateLibrary.getTemplateAnalytics(
      "customer-onboarding",
      "acme-corp",
    );
    ...
    console.log(
      `   Average Generation Time: ${analytics.averageGenerationTime}ms`,
    );
    ...
    if (analytics.conversionFailures.length > 0) {
      ...
      analytics.conversionFailures.forEach((failure) => {
        ...
        console.log(
          `         Last seen: ${failure.lastOccurrence.toISOString()}`,
        );
      });
    }
  } catch (error) {
...
async function advancedTemplateProcessing() {
  ...
  try {
    ...
    console.log(
      `   Applied Parameters: ${Object.keys(result.appliedParameters).length}`,
    );
