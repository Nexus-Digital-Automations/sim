...
async function demonstrateTemplateInheritance() {
  ...
  console.log(
    `   Total parameters: ${technicalTemplateResult.parameters.length}`,
  );
}
...
async function demonstrateTemplateMixins() {
  ...
  const satisfactionMixin: TemplateMixin = {
    ...
    description:
      "Adds customer satisfaction survey capabilities to any template",
    parametersToAdd: [
      {
        id: "satisfaction-001",
    ...
    blocksToAdd: [
      {
        ...
        data: {
          prompt:
            "On a scale of 1-5, how satisfied are you with the support you received?",
          responseType: "rating",
          scale: { min: 1, max: 5 },
        },
        isParameterized: true,
            ...
            operands: [
              ...
              },
              {
                ...
                computationRule: {
                  type: "javascript",
                  expression:
                    "currentTime - startTime > escalationThreshold * 60000",
  ...
  console.log(
    `   - Satisfaction mixin parameters: ${satisfactionMixin.parametersToAdd.length}`,
  );
  console.log(
    `   - Escalation mixin parameters: ${escalationMixin.parametersToAdd.length}`,
  );
  console.log(
    ...
    operands: [
      {
        ...
        computationRule: {
          type: "javascript",
          expression:
            "new Date().getHours() >= 17 || new Date().getHours() < 9",
          dependencies: [],
          cacheability: "dynamic",
          timeout: 1000,
        },
  ...
  const businessHoursValidator = (
    ...
    if (!timePattern.test(value)) {
      errors.push({
        code: "INVALID_FORMAT",
        message:
          "Business hours must be in format HH:MM-HH:MM (e.g., 09:00-17:00)",
      });
      return { isValid: false, errors, warnings };
    }
    ...
    if (endMinutes - startMinutes > 12 * 60) {
      warnings.push({
        code: "LONG_HOURS",
        message:
          "Business hours longer than 12 hours may affect response times",
