...
export interface AgentCapability {
  type: 'tool_execution' | 'knowledge_retrieval' | 'workflow_trigger' | 'data_processing'
  name: string
  confidence: number
  restrictions?: string[]
}
...
export interface TransitionAction {
  type: 'set_variable' | 'clear_variable' | 'call_tool' | 'send_notification' | 'log_event'
  configuration: Record<string, any>
  async: boolean
  errorHandling: ErrorHandlingRule
}
...
export interface AccessibilityFeature {
  type: 'high_contrast' | 'large_text' | 'screen_reader' | 'keyboard_navigation'
  enabled: boolean
  configuration?: Record<string, any>
}
...
export interface OptimizationImplementation {
  type: 'state_merge' | 'transition_simplification' | 'content_optimization' | 'caching'
