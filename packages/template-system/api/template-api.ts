...
export class TemplateAPIController {
  ...
  async searchTemplates(
    ...
    try {
      ...
      // Perform search
      const result = await this.templateLibrary.searchTemplates(
        filters,
        workspaceId,
        userId,
      );
      ...
      res.json({
        templates: paginatedTemplates.map((t) =>
          this.sanitizeTemplateForResponse(t),
        ),
        totalCount: result.totalCount,
        facets: result.facets,
        suggestions: result.suggestions,
        relatedTemplates: result.relatedTemplates.map((t) =>
          this.sanitizeTemplateForResponse(t),
        ),
        pagination: {
          limit,
  ...
  async createTemplate(
    ...
    try {
      ...
      const processedTemplateData = {
        ...templateData,
        parameters:
          templateData.parameters?.map(
            (param: any, index: number): TemplateParameter => {
        ...
        workflowData: templateData.workflowData
          ? {
              ...
              parameterMappings:
                (templateData.workflowData as any).parameterMappings || [],
              conditionalBlocks:
                (templateData.workflowData as any).conditionalBlocks || [],
              dynamicContent:
                (templateData.workflowData as any).dynamicContent || [],
              optimizationHints:
                (templateData.workflowData as any).optimizationHints || [],
              performanceSettings: (templateData.workflowData as any)
                .performanceSettings || {
                enableCaching: true,
                cacheStrategy: {
      ...
      if (!(await this.hasCreatePermission(userId, workspaceId))) {
        res
          .status(403)
          .json({ error: "Insufficient permissions to create templates" });
        return;
      }
  ...
  async updateTemplate(
    ...
    try {
      ...
      const processedUpdates: Partial<WorkflowTemplate> = {
        ...
        ...(updatesParameters
          ? {
              parameters: updatesParameters.map(
                (param: any, index: number): TemplateParameter => {
                  const paramId = param.id || `param_${Date.now()}_${index}`;
                  return {
                    id: paramId,
        ...
        ...(updatesWorkflowData
          ? {
              workflowData: {
                ...
                parameterMappings:
                  (updatesWorkflowData as any).parameterMappings || [],
                conditionalBlocks:
                  (updatesWorkflowData as any).conditionalBlocks || [],
                dynamicContent:
                  (updatesWorkflowData as any).dynamicContent || [],
                optimizationHints:
                  (updatesWorkflowData as any).optimizationHints || [],
                performanceSettings: (updatesWorkflowData as any)
                  .performanceSettings || {
                  enableCaching: true,
                  cacheStrategy: {
                    scope: "session" as const,
      ...
      const existingTemplate = await this.templateLibrary.getTemplate(
        templateId,
        workspaceId,
      );
      if (!existingTemplate) {
      ...
      // Check permissions
      if (
        !(await this.hasUpdatePermission(userId, workspaceId, existingTemplate))
      ) {
        res
          .status(403)
          .json({ error: "Insufficient permissions to update this template" });
        return;
      }
  ...
  async deleteTemplate(
    ...
    try {
      ...
      // Check permissions
      if (
        !(await this.hasDeletePermission(userId, workspaceId, existingTemplate))
      ) {
        res
          .status(403)
          .json({ error: "Insufficient permissions to delete this template" });
        return;
      }

      // Delete template
      await this.templateLibrary.deleteTemplate(
        templateId,
        workspaceId,
        userId,
      );
  ...
  async getTemplateAnalytics(
    ...
    try {
      ...
      const analytics = await this.templateLibrary.getTemplateAnalytics(
        templateId,
        workspaceId,
      );
  ...
  async findSimilarTemplates(
    ...
    try {
      ...
      res.json({
        similarTemplates: similarTemplates.map((t) =>
          this.sanitizeTemplateForResponse(t),
        ),
      });
    } catch (error) {
    ...
    try {
      ...
      const options: TemplateImportOptions = {
        ...
        mappingRules: req.body.mappingRules
          ? JSON.parse(req.body.mappingRules)
          : [],
        targetWorkspaceId: workspaceId,
      };

      // Parse template file
      const templateData: TemplateExportData = JSON.parse(
      ...
      if (!(await this.hasCreatePermission(userId, workspaceId))) {
        res
          .status(403)
          .json({ error: "Insufficient permissions to import templates" });
        return;
      }

      // Import template
      const importedTemplate = await this.importTemplateData(
        templateData,
        options,
        userId,
      );
  ...
  async exportTemplate(
    ...
    try {
      ...
      if (!(await this.hasReadPermission(userId, workspaceId, template))) {
        res
          .status(403)
          .json({ error: "Insufficient permissions to export this template" });
        return;
      }
  ...
  private sanitizeTemplateForResponse(
  ...
  private async hasCreatePermission(
    userId: string,
    workspaceId: string,
  ): Promise<boolean> {
