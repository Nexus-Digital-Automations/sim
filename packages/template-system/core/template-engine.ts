...
export class TemplateEngine {
  ...
  async processTemplate(
    ...
    try {
      ...
      const finalData = await this.evaluateConditionals(
        processedData,
        parameters,
        context,
      );
  ...
  async validateParameters(
    ...
    const cacheKey = this.generateValidationCacheKey(
      template.id,
      parameters,
      context,
    );
    ...
    for (const param of template.parameters) {
      ...
      const paramResult = await this.validateParameter(
        param,
        value,
        validationContext,
      );
    ...
    for (const paramName of Object.keys(parameters)) {
      if (!expectedParams.has(paramName)) {
        warnings.push({
          ...
          path: paramName,
          suggestion:
            "Remove this parameter or add it to the template definition",
        });
      }
    }
  ...
  private async applyInheritance(
    ...
    if (template.parentTemplateId) {
      ...
      processedTemplate = this.mergeTemplates(
        parentTemplate,
        processedTemplate,
      );
    }
    ...
    if (template.overrides) {
      processedTemplate = this.applyOverrides(
        processedTemplate,
        template.overrides,
      );
    }
      ...
      for (const block of result.blocks) {
        if (block.parameterBindings) {
          for (const binding of block.parameterBindings) {
            const value = await this.resolveParameterBinding(
              binding,
              parameters,
              context,
            );
  ...
  private async applyOptimizations(
    ...
    if (workflowData.performanceSettings) {
      result = this.applyPerformanceSettings(
        result,
        workflowData.performanceSettings,
      );
    }
