...
class ParlantMigrationMonitor {
  ...
  constructor() {
    ...
    this.logFile = path.join(
      __dirname,
      `../logs/parlant-migration-${timestamp}.log`,
    );
    this.metricsFile = path.join(
      __dirname,
      `../logs/parlant-metrics-${timestamp}.json`,
    );
    ...
    for (const threshold of this.alertThresholds) {
      ...
      switch (threshold.metric) {
        ...
        case "queryExecutionTime":
          value = Math.max(
            ...metrics.queryPerformance.map((q) => q.executionTime),
          );
          break;
        default:
          continue;
  ...
  private generateReport(metrics: MigrationMetrics[]): void {
    ...
    this.log("==========================================");
    this.log(
      `Migration Duration: ${(lastMetric.duration / 1000 / 60).toFixed(2)} minutes`,
    );
    ...
    this.log(
      `Peak Connections: ${Math.max(...metrics.map((m) => m.activeConnections))}`,
    );
    ...
    if (slowQueries.length > 0) {
      ...
      slowQueries.slice(0, 5).forEach((q) => {
        this.log(
          `  - ${(q.executionTime / 1000).toFixed(2)}s: ${q.query.substring(0, 80)}...`,
        );
      });
    }
  ...
  async monitorPreMigration(): Promise<void> {
    ...
    if (lockHeld) {
      this.log(
        "Migration advisory lock detected - migration in progress",
        "WARN",
      );
    }
  ...
  async monitorPostMigration(): Promise<void> {
    ...
    if (metrics.tablesCreated < 11) {
      this.log(
        `WARNING: Expected 11+ tables, found ${metrics.tablesCreated}`,
        "WARN",
      );
    }
    ...
    if (metrics.indexesCreated < 50) {
      this.log(
        `WARNING: Expected 50+ indexes, found ${metrics.indexesCreated}`,
        "WARN",
      );
    }
    ...
    if (metrics.constraintsAdded < 15) {
      this.log(
        `WARNING: Expected 15+ constraints, found ${metrics.constraintsAdded}`,
        "WARN",
      );
    }
    ...
    if (metrics.queryPerformance.length > 0) {
      const longQueries = metrics.queryPerformance.filter(
        (q) => q.executionTime > 30000,
      );
      if (longQueries.length > 0) {
        this.log(
          `WARNING: ${longQueries.length} long-running queries still active`,
          "WARN",
        );
      }
    }
