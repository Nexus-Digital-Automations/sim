...
export const toolRegistry = pgTable(
  "tool_registry",
  {
    ...
    categoryId: text("category_id").references(() => toolCategories.id, {
      onDelete: "set null",
    }),
  ...
  (table) => ({
    ...
    displayNameIdx: index("tool_registry_display_name_idx").on(
      table.displayName,
    ),
    ...
    healthStatusIdx: index("tool_registry_health_status_idx").on(
      table.healthStatus,
    ),
    lastUsedIdx: index("tool_registry_last_used_idx").on(table.lastUsed),
    // Composite indexes for common queries
    activePublicIdx: index("tool_registry_active_public_idx").on(
      table.status,
      table.isPublic,
    ),
    categoryTypeIdx: index("tool_registry_category_type_idx").on(
      table.categoryId,
      table.toolType,
    ),
...
export const toolConfigurations = pgTable(
  ...
  (table) => ({
    ...
    workspaceIdIdx: index("tool_configurations_workspace_id_idx").on(
      table.workspaceId,
    ),
    ...
    toolUserIdx: index("tool_configurations_tool_user_idx").on(
      table.toolId,
      table.userId,
    ),
...
export const toolUsageAnalytics = pgTable(
  "tool_usage_analytics",
  {
    ...
    toolId: text("tool_id")
      .notNull()
      .references(() => toolRegistry.id, { onDelete: "cascade" }),
    configurationId: text("configuration_id").references(
  ...
  (table) => ({
    // Analytics queries
    toolIdTimeIdx: index("tool_usage_analytics_tool_time_idx").on(
      table.toolId,
      table.startTime,
    ),
    ...
    userTimeIdx: index("tool_usage_analytics_user_time_idx").on(
      table.userId,
      table.startTime,
    ),
    successIdx: index("tool_usage_analytics_success_idx").on(table.success),
    // Performance analysis
    durationIdx: index("tool_usage_analytics_duration_idx").on(
      table.durationMs,
    ),
    errorTypeIdx: index("tool_usage_analytics_error_type_idx").on(
      table.errorType,
    ),
    // Session and execution tracking
    sessionIdx: index("tool_usage_analytics_session_idx").on(table.sessionId),
    executionIdx: index("tool_usage_analytics_execution_idx").on(
      table.executionId,
    ),
  }),
);
export const toolRecommendations = pgTable(
  ...
  (table) => ({
    ...
    workspaceIdx: index("tool_recommendations_workspace_idx").on(
      table.workspaceId,
    ),
    ...
    presentedIdx: index("tool_recommendations_presented_idx").on(
      table.presented,
    ),
    usedIdx: index("tool_recommendations_used_idx").on(table.used),
    // Learning and feedback
    feedbackIdx: index("tool_recommendations_feedback_idx").on(
      table.userFeedback,
    ),
    effectivenessIdx: index("tool_recommendations_effectiveness_idx").on(
      table.effectiveness,
    ),
  }),
);
...
export const embedding = pgTable(
  "embedding",
  {
    ...
    contentTsv: tsvector("content_tsv").generatedAlwaysAs(
      () => sql`to_tsvector('english', ${embedding.content})`,
    ),
    // Timestamps
    createdAt: timestamp("created_at").notNull().defaultNow(),
    updatedAt: timestamp("updated_at").notNull().defaultNow(),
...
export const docsEmbeddings = pgTable(
  "docs_embeddings",
  {
    ...
    chunkTextTsv: tsvector("chunk_text_tsv").generatedAlwaysAs(
      () => sql`to_tsvector('english', ${docsEmbeddings.chunkText})`,
    ),
    // Timestamps
    createdAt: timestamp("created_at").notNull().defaultNow(),
    updatedAt: timestamp("updated_at").notNull().defaultNow(),
...
export const { messageStatusEnum, conversationTypeEnum, messageTypeEnum } =
  chatPersistenceEnums;
export const {
  parlantAgent,
  parlantSession,
