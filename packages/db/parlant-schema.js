...
export const parlantAgent = pgTable(
  "parlant_agent",
  {
    ...
    allowProactiveMessages: boolean("allow_proactive_messages")
      .notNull()
      .default(false), // Can send unsolicited messages
  ...
  (table) => ({
    workspaceIdIdx: index("parlant_agent_workspace_id_idx").on(
      table.workspaceId,
    ),
    ...
    lastActiveIdx: index("parlant_agent_last_active_idx").on(
      table.lastActiveAt,
    ),
    compositionModeIdx: index("parlant_agent_composition_mode_idx").on(
      table.compositionMode,
    ),
    modelProviderIdx: index("parlant_agent_model_provider_idx").on(
      table.modelProvider,
    ),
    conversationStyleIdx: index("parlant_agent_conversation_style_idx").on(
      table.conversationStyle,
    ),
...
export const parlantSession = pgTable(
  ...
  (table) => ({
    ...
    workspaceIdIdx: index("parlant_session_workspace_id_idx").on(
      table.workspaceId,
    ),
    userIdIdx: index("parlant_session_user_id_idx").on(table.userId),
    customerIdIdx: index("parlant_session_customer_id_idx").on(
      table.customerId,
    ),
    statusIdx: index("parlant_session_status_idx").on(table.status),
    agentStatusIdx: index("parlant_session_agent_status_idx").on(
      table.agentId,
      table.status,
    ),
    lastActivityIdx: index("parlant_session_last_activity_idx").on(
      table.lastActivityAt,
    ),
    currentJourneyIdx: index("parlant_session_current_journey_idx").on(
      table.currentJourneyId,
    ),
    ...
    satisfactionIdx: index("parlant_session_satisfaction_idx").on(
...
export const parlantEvent = pgTable(
  ...
  (table) => ({
    ...
    sessionTypeIdx: index("parlant_event_session_type_idx").on(
      table.sessionId,
      table.eventType,
    ),
...
export const parlantGuideline = pgTable(
  ...
  (table) => ({
    ...
    agentEnabledIdx: index("parlant_guideline_agent_enabled_idx").on(
      table.agentId,
      table.enabled,
    ),
    priorityIdx: index("parlant_guideline_priority_idx").on(table.priority),
    lastMatchedIdx: index("parlant_guideline_last_matched_idx").on(
      table.lastMatchedAt,
    ),
  }),
);
...
export const parlantJourney = pgTable(
  ...
  (table) => ({
    ...
    agentEnabledIdx: index("parlant_journey_agent_enabled_idx").on(
      table.agentId,
      table.enabled,
    ),
...
export const parlantJourneyState = pgTable(
  ...
  (table) => ({
    journeyIdIdx: index("parlant_journey_state_journey_id_idx").on(
      table.journeyId,
    ),
    ...
    isInitialIdx: index("parlant_journey_state_initial_idx").on(
      table.isInitial,
    ),
...
export const parlantJourneyTransition = pgTable(
  ...
  (table) => ({
    journeyIdIdx: index("parlant_journey_transition_journey_id_idx").on(
      table.journeyId,
    ),
    fromStateIdx: index("parlant_journey_transition_from_state_idx").on(
      table.fromStateId,
    ),
    toStateIdx: index("parlant_journey_transition_to_state_idx").on(
      table.toStateId,
    ),
    ...
    priorityIdx: index("parlant_journey_transition_priority_idx").on(
      table.priority,
    ),
  }),
);
...
export const parlantVariable = pgTable(
  "parlant_variable",
  {
    ...
    agentId: uuid("agent_id")
      .notNull()
      .references(() => parlantAgent.id, { onDelete: "cascade" }),
    sessionId: uuid("session_id").references(() => parlantSession.id, {
      onDelete: "cascade",
    }),
  ...
  (table) => ({
    ...
    agentKeyIdx: index("parlant_variable_agent_key_idx").on(
      table.agentId,
      table.key,
    ),
    sessionKeyIdx: uniqueIndex("parlant_variable_session_key_idx").on(
      table.sessionId,
      table.key,
    ),
...
export const parlantTool = pgTable(
  ...
  (table) => ({
    workspaceIdIdx: index("parlant_tool_workspace_id_idx").on(
      table.workspaceId,
    ),
...
export const parlantTerm = pgTable(
  ...
  (table) => ({
    ...
    agentNameIdx: uniqueIndex("parlant_term_agent_name_idx").on(
      table.agentId,
      table.name,
    ),
...
export const parlantCannedResponse = pgTable(
  ...
  (table) => ({
    ...
    categoryIdx: index("parlant_canned_response_category_idx").on(
      table.category,
    ),
    ...
    priorityIdx: index("parlant_canned_response_priority_idx").on(
      table.priority,
    ),
    lastUsedIdx: index("parlant_canned_response_last_used_idx").on(
      table.lastUsedAt,
    ),
  }),
);
...
export const parlantAgentWorkflow = pgTable(
  ...
  (table) => ({
    ...
    workflowIdIdx: index("parlant_agent_workflow_workflow_id_idx").on(
      table.workflowId,
    ),
    workspaceIdIdx: index("parlant_agent_workflow_workspace_id_idx").on(
      table.workspaceId,
    ),
    ...
    integrationTypeIdx: index("parlant_agent_workflow_type_idx").on(
...
export const parlantAgentApiKey = pgTable(
  ...
  (table) => ({
    ...
    apiKeyIdIdx: index("parlant_agent_api_key_api_key_id_idx").on(
      table.apiKeyId,
    ),
    workspaceIdIdx: index("parlant_agent_api_key_workspace_id_idx").on(
      table.workspaceId,
    ),
...
export const parlantSessionWorkflow = pgTable(
  ...
  (table) => ({
    sessionIdIdx: index("parlant_session_workflow_session_id_idx").on(
      table.sessionId,
    ),
    workflowIdIdx: index("parlant_session_workflow_workflow_id_idx").on(
      table.workflowId,
    ),
    executionIdIdx: index("parlant_session_workflow_execution_id_idx").on(
      table.executionId,
    ),
    ...
    startedAtIdx: index("parlant_session_workflow_started_at_idx").on(
      table.startedAt,
    ),
  }),
);
...
export const parlantAgentTool = pgTable(
  ...
  (table) => ({
    ...
    agentToolUnique: uniqueIndex("parlant_agent_tool_unique").on(
      table.agentId,
      table.toolId,
    ),
    agentEnabledIdx: index("parlant_agent_tool_agent_enabled_idx").on(
      table.agentId,
      table.enabled,
    ),
...
export const parlantJourneyGuideline = pgTable(
  ...
  (table) => ({
    journeyIdIdx: index("parlant_journey_guideline_journey_id_idx").on(
      table.journeyId,
    ),
    guidelineIdIdx: index("parlant_journey_guideline_guideline_id_idx").on(
      table.guidelineId,
    ),
    ...
    journeyEnabledIdx: index(
      "parlant_journey_guideline_journey_enabled_idx",
    ).on(table.journeyId, table.enabled),
    priorityOverrideIdx: index(
      "parlant_journey_guideline_priority_override_idx",
    ).on(table.priorityOverride),
  }),
);
/**
...
export const parlantAgentKnowledgeBase = pgTable(
  ...
  (table) => ({
    ...
    knowledgeBaseIdIdx: index("parlant_agent_kb_kb_id_idx").on(
      table.knowledgeBaseId,
    ),
    agentKbUnique: uniqueIndex("parlant_agent_kb_unique").on(
      table.agentId,
      table.knowledgeBaseId,
    ),
    agentEnabledIdx: index("parlant_agent_kb_agent_enabled_idx").on(
      table.agentId,
      table.enabled,
    ),
...
export const parlantToolIntegration = pgTable(
  ...
  (table) => ({
    parlantToolIdIdx: index("parlant_tool_integration_parlant_tool_idx").on(
      table.parlantToolId,
    ),
    integrationTypeIdx: index("parlant_tool_integration_type_idx").on(
      table.integrationType,
    ),
    targetIdIdx: index("parlant_tool_integration_target_idx").on(
      table.targetId,
    ),
    ...
    healthStatusIdx: index("parlant_tool_integration_health_idx").on(
      table.healthStatus,
    ),
...
export const parlantWorkflowTemplate = pgTable(
  ...
  (table) => ({
    workspaceIdIdx: index("parlant_workflow_template_workspace_idx").on(
      table.workspaceId,
    ),
    workflowIdIdx: index("parlant_workflow_template_workflow_idx").on(
      table.workflowId,
    ),
    ...
    usageCountIdx: index("parlant_workflow_template_usage_count_idx").on(
      table.usageCount,
    ),
    uniqueWorkspaceWorkflowName: uniqueIndex(
      "parlant_workflow_template_unique",
...
export const parlantTemplateParameter = pgTable(
  ...
  (table) => ({
    templateIdIdx: index("parlant_template_parameter_template_idx").on(
      table.templateId,
    ),
    orderIdx: index("parlant_template_parameter_order_idx").on(
      table.displayOrder,
    ),
...
export const parlantConversionCache = pgTable(
  ...
  (table) => ({
    cacheKeyIdx: uniqueIndex("parlant_conversion_cache_key_unique").on(
      table.cacheKey,
    ),
    workflowIdx: index("parlant_conversion_cache_workflow_idx").on(
      table.workflowId,
    ),
    templateIdx: index("parlant_conversion_cache_template_idx").on(
      table.templateId,
    ),
    workspaceIdx: index("parlant_conversion_cache_workspace_idx").on(
      table.workspaceId,
    ),
    expiresAtIdx: index("parlant_conversion_cache_expires_at_idx").on(
      table.expiresAt,
    ),
    lastAccessedIdx: index("parlant_conversion_cache_last_accessed_idx").on(
      table.lastAccessed,
    ),
  }),
);
...
export const parlantConversionHistory = pgTable(
  ...
  (table) => ({
    conversionIdIdx: uniqueIndex(
      "parlant_conversion_history_conversion_id_unique",
    ).on(table.conversionId),
    workflowIdx: index("parlant_conversion_history_workflow_idx").on(
      table.workflowId,
    ),
    templateIdx: index("parlant_conversion_history_template_idx").on(
      table.templateId,
    ),
    workspaceIdx: index("parlant_conversion_history_workspace_idx").on(
      table.workspaceId,
    ),
    statusIdx: index("parlant_conversion_history_status_idx").on(table.status),
    createdAtIdx: index("parlant_conversion_history_created_at_idx").on(
      table.createdAt,
    ),
  }),
);
...
export const parlantJourneyGenerationHistory = pgTable(
  ...
  (table) => ({
    journeyIdx: uniqueIndex("parlant_journey_generation_journey_unique").on(
      table.journeyId,
    ),
    conversionIdx: index("parlant_journey_generation_conversion_idx").on(
      table.conversionId,
    ),
    templateIdx: index("parlant_journey_generation_template_idx").on(
      table.templateId,
    ),
    agentIdx: index("parlant_journey_generation_agent_idx").on(table.agentId),
    workspaceIdx: index("parlant_journey_generation_workspace_idx").on(
      table.workspaceId,
    ),
  }),
);
