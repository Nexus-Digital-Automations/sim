...
export const chatMessage = pgTable(
  ...
  (table) => ({
    ...
    sessionStatusIdx: index("chat_message_session_status_idx").on(
      table.sessionId,
      table.status,
    ),
    deliveredAtIdx: index("chat_message_delivered_at_idx").on(
      table.deliveredAt,
    ),

    // Threading and conversation context
    threadIdx: index("chat_message_thread_idx").on(table.threadId),
    parentMessageIdx: index("chat_message_parent_idx").on(
      table.parentMessageId,
    ),
    threadCreatedIdx: index("chat_message_thread_created_idx").on(
      table.threadId,
      table.createdAt,
    ),
    ...
    sessionTypeIdx: index("chat_message_session_type_idx").on(
      table.sessionId,
      table.messageType,
    ),
    ...
    activeSessionIdx: index("chat_message_active_session_idx").on(
      table.sessionId,
      table.deletedAt,
    ),
  }),
...
export const chatConversation = pgTable(
  ...
  (table) => ({
    ...
    conversationTypeIdx: index("chat_conversation_type_idx").on(
    ...
    isArchivedIdx: index("chat_conversation_is_archived_idx").on(
      table.isArchived,
    ),
    ...
    participantIdsIdx: index("chat_conversation_participant_ids_idx").on(
      table.participantIds,
    ),
    agentIdsIdx: index("chat_conversation_agent_ids_idx").on(table.agentIds),

    // Activity and engagement tracking
    lastActivityIdx: index("chat_conversation_last_activity_idx").on(
      table.lastActivityAt,
    ),
    lastMessageIdx: index("chat_conversation_last_message_idx").on(
      table.lastMessageAt,
    ),
    messageCountIdx: index("chat_conversation_message_count_idx").on(
      table.messageCount,
    ),

    // Session management
    currentSessionIdx: index("chat_conversation_current_session_idx").on(
      table.currentSessionId,
    ),
    sessionIdsIdx: index("chat_conversation_session_ids_idx").on(
      table.sessionIds,
    ),
    ...
    // Analytics indexes
    totalTokensIdx: index("chat_conversation_total_tokens_idx").on(
      table.totalTokens,
    ),
    totalCostIdx: index("chat_conversation_total_cost_idx").on(table.totalCost),
    satisfactionIdx: index("chat_conversation_satisfaction_idx").on(
...
export const chatBrowserSession = pgTable(
  "chat_browser_session",
  {
    ...
    parlantSessionId: uuid("parlant_session_id").references(
      () => parlantSession.id,
      {
        onDelete: "cascade",
      },
  ...
  (table) => ({
    ...
    sessionTokenIdx: uniqueIndex("chat_browser_session_token_unique").on(
      table.sessionToken,
    ),
    ...
    userActiveIdx: index("chat_browser_session_user_active_idx").on(
      table.userId,
      table.isActive,
    ),
    ...
    parlantSessionIdx: index("chat_browser_session_parlant_idx").on(
      table.parlantSessionId,
    ),

    // Session management and cleanup
    isActiveIdx: index("chat_browser_session_is_active_idx").on(table.isActive),
    lastHeartbeatIdx: index("chat_browser_session_last_heartbeat_idx").on(
      table.lastHeartbeat,
    ),
    expiresAtIdx: index("chat_browser_session_expires_at_idx").on(
      table.expiresAt,
    ),

    // Analytics indexes
    sessionDurationIdx: index("chat_browser_session_duration_idx").on(
      table.sessionDuration,
    ),
    messagesCountIdx: index("chat_browser_session_messages_count_idx").on(
      table.messagesInSession,
    ),
    lastMessageIdx: index("chat_browser_session_last_message_idx").on(
      table.lastMessageAt,
    ),
...
export const chatSearchIndex = pgTable(
  ...
  (table) => ({
    ...
    searchContentIdx: index("chat_search_content_idx").on(
    ...
    sessionSearchIdx: index("chat_search_session_idx").on(
      table.sessionId,
      table.searchableContent,
    ),
    ...
    searchableTermsIdx: index("chat_search_terms_idx").on(
      table.searchableTerms,
    ),
    ...
    importanceIdx: index("chat_search_importance_idx").on(
      table.messageImportance,
    ),
    engagementIdx: index("chat_search_engagement_idx").on(
      table.engagementScore,
    ),

    // Composite indexes for common search patterns
    workspaceTypeImportanceIdx: index(
      "chat_search_workspace_type_importance_idx",
    ).on(table.workspaceId, table.contentType, table.messageImportance),

    // Index maintenance
    lastIndexedIdx: index("chat_search_last_indexed_idx").on(table.lastIndexed),
    indexVersionIdx: index("chat_search_index_version_idx").on(
      table.indexVersion,
    ),
  }),
);
...
export const chatExportRequest = pgTable(
  ...
  (table) => ({
    // Primary access patterns
    requestTokenIdx: uniqueIndex("chat_export_request_token_unique").on(
      table.requestToken,
    ),
    ...
    processingIdx: index("chat_export_processing_idx").on(
      table.status,
    ...
    // Analytics
    downloadCountIdx: index("chat_export_download_count_idx").on(
      table.downloadCount,
    ),
