...
export class ContextManager {
  ...
  async preserveContext(
    ...
    try {
      ...
      if (!validation.isValid) {
        throw new ConversionError('Context validation failed', 'CONTEXT_VALIDATION_ERROR', {
   ...
   */
  private mapConditionVariables(variables: string[], variableMapping: VariableMapping): string[] {
...
export class VariableMapper {
   ...
   */
  private async convertVariableValue(value: any, fromType: string, toType: string): Promise<any> {
...
export class SessionStateManager {
  ...
  private nodeRequiresSessionState(node: any): boolean {
    ...
    return (
      sessionRequiredTypes.includes(node.type) || node.data?.requiresSession || node.data?.storeData
    )
  }
  ...
  private nodeRequiresEncryptedState(node: any): boolean {
    const encryptedTypes = ['authentication', 'payment', 'personal-data', 'sensitive-data']

    return encryptedTypes.includes(node.type) || node.data?.sensitive || node.data?.encrypted
  }
   ...
   */
  private async generateSessionContext(node: any, variableMapping: VariableMapping): Promise<any> {
    const context: any = {
      nodeId: node.id,
      nodeType: node.type,
      variables: [],
    ...
    for (const nodeVar of nodeVariables) {
      const mapping = variableMapping.mappings.find((m) => m.workflowVariable.name === nodeVar)
...
export class ExecutionContextManager {
  ...
  private isAsyncExecution(node: any): boolean {
    const asyncTypes = ['api-call', 'database-query', 'file-upload', 'email-send', 'webhook']
  ...
  private shouldCacheResults(node: any): boolean {
    const cacheableTypes = ['api-call', 'database-query', 'calculation', 'data-transform']
...
export class DynamicVariableResolver {
  ...
  private shouldCacheVariable(variable: any): boolean {
    const cacheableTypes = ['calculated', 'api-response', 'database', 'external']

    return cacheableTypes.includes(variable.type) || variable.expensive === true
  }
  ...
  private createErrorHandlingRules(variables: any[]): any {
    return {
      ...
      retries:
        variables.filter((v) => v.type === 'api-response' || v.type === 'database').length > 0,
    ...
    const contextInheritance: ContextInheritance = {
      ...
      statistics: {
        ...
        isolatedNodes: hierarchy.filter(
          (h) => !h.parentContext && (!h.childContexts || h.childContexts.length === 0)
        ).length,
      },
    }
