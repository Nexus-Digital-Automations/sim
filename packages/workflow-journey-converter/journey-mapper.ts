...
export class JourneyMappingService {
   ...
   */
  async mapToJourney(analysis: WorkflowAnalysisResult): Promise<JourneyDefinition> {
    ...
    try {
      ...
      const transitions = await this.convertEdgesToTransitions(analysis, states)
      ...
      const metadata = await this.buildJourneyMetadata(analysis, states, transitions)
  ...
  private async initializeToolMappings(analysis: WorkflowAnalysisResult): Promise<void> {
    ...
    for (const compatibility of analysis.toolAnalysis.toolCompatibility) {
      const toolMapping: ToolDependency = {
        ...
        configuration: {},
        inputMapping: await this.generateInputMapping(compatibility.toolId, analysis),
        outputMapping: await this.generateOutputMapping(compatibility.toolId, analysis),
  ...
  private async convertNodeToState(
    ...
    switch (blockType) {
      ...
      case 'condition':
        stateType = 'chat'
        configuration = this.createConditionalStateConfiguration(node, analysis)
        break
      ...
      default:
        ...
        stateType = 'tool'
        configuration = await this.createDefaultToolStateConfiguration(node, analysis)
        break
    }
    ...
    const state: JourneyStateDefinition = {
      ...
      description: node.data?.description || `State converted from ${blockType} node: ${node.id}`,
      stateType,
  ...
  private async convertEdgesToTransitions(
    ...
    for (const edge of workflow.edges) {
      const transition = await this.convertEdgeToTransition(edge, states, analysis)
      if (transition) {
    ...
    const implicitTransitions = await this.createImplicitTransitions(analysis, states)
  ...
  private async processComplexStructures(
    ...
    for (const parallelSection of analysis.structure.parallelSections) {
      await this.processParallelSection(parallelSection, states, transitions, analysis)
    }

    // Process loop structures
    for (const loopStructure of analysis.structure.loopStructures) {
      await this.processLoopStructure(loopStructure, states, transitions, analysis)
    }

    // Process conditional nodes
    for (const conditionalNode of analysis.structure.conditionalNodes) {
      await this.processConditionalNode(conditionalNode, states, transitions, analysis)
    }
  }
   ...
   */
  private async validateJourneyStructure(journey: JourneyDefinition): Promise<void> {
  ...
  private inferToolType(toolId: string): 'builtin' | 'custom' | 'external' | 'mcp' {
    // Heuristics to determine tool type
    if (toolId.startsWith('builtin_') || toolId.startsWith('sim_')) return 'builtin'
  ...
  private createErrorHandling(node: ReactFlowNode, analysis: WorkflowAnalysisResult): any {
