  ...
  static findLongestPath(
    ...
    for (const nodeId of topologicalOrder) {
      ...
      for (const predecessor of incoming) {
        const newDistance =
          (distances.get(predecessor) || 0) + (nodeWeights.get(nodeId) || 1);
...
export class ExecutionPathAnalyzer {
    ...
    for (const entryPoint of entryPoints) {
      ...
      for (const nodePath of nodePaths) {
        const executionPath: ExecutionPath = {
          ...
          errorProbability: this.calculatePathErrorProbability(
            nodePath,
            workflow,
          ),
   ...
   */
  private calculatePathResources(
    path: string[],
    workflow: SimWorkflowDefinition,
  ): any[] {
    const resources: any[] = [];
  ...
  private calculatePathErrorProbability(
    ...
    for (const nodeId of path) {
      ...
      if (node) {
        ...
        errorProbability =
          1 - (1 - errorProbability) * (1 - nodeErrorProbability);
      }
    }
  ...
  private isCriticalPath(
    ...
    // Calculate overlap
    const overlap = [...criticalPathNodes].filter((node) =>
      pathNodes.has(node),
    );
    ...
    return (
      overlap.length >=
      Math.min(criticalPathNodes.size * 0.7, pathNodes.size * 0.7)
    );
  }
  ...
  private optimizeExecutionPaths(paths: ExecutionPath[]): ExecutionPath[] {
    ...
    for (const path of paths) {
      ...
      if (
        !uniquePaths.has(pathKey) ||
        path.probability > uniquePaths.get(pathKey)!.probability
      ) {
...
export class ParallelExecutionAnalyzer {
  ...
  private analyzeParallelSection(
    ...
    // Find the join node
    const joinNode = this.findJoinNode(
      splitNode.id,
      outgoingEdges,
      nodes,
      edges,
    ...
    if (!joinNode) {
      ...
      return this.createImplicitParallelSection(
        splitNode,
        outgoingEdges,
        nodes,
        edges,
    ...
    for (let i = 0; i < outgoingEdges.length; i++) {
      ...
      const branch = this.analyzeBranch(
        edge.target,
        joinNode.id,
        nodes,
        edges,
   ...
   */
  private hasMultipleOutgoingEdges(
    nodeId: string,
    edges: ReactFlowEdge[],
  ): boolean {
    const outgoingEdges = edges.filter((edge) => edge.source === nodeId);
...
export class LoopStructureAnalyzer {
  ...
  private determineLoopType(
    ...
    if (
      condition.includes("forEach") ||
      entryNode?.data?.forEach ||
      entryNode?.data?.items
    ) {
      return "foreach";
    }

    if (
      condition.includes("for(") ||
      condition.includes("for ") ||
      entryNode?.data?.for
    ) {
      return "for";
  ...
  private analyzeExplicitLoop(
    ...
    const bodyNodes = this.getLoopBodyForExplicitLoop(
      loopNode.id,
      nodes,
      edges,
    );

    const condition = this.extractLoopCondition(
      { data: loopNode.data } as any,
      loopNode,
    );
...
export class WorkflowStructureProcessor {
  ...
  async processWorkflowStructure(workflow: SimWorkflowDefinition): Promise<{
    ...
    const executionPaths = this.pathAnalyzer.analyzeExecutionPaths(
      workflow,
      structure,
    );
  ...
  private async analyzeBasicStructure(
    ...
    const parallelSections = this.parallelAnalyzer.analyzeParallelSections(
      nodes,
      edges,
    );
    const loopStructures = this.loopAnalyzer.analyzeLoopStructures(
      nodes,
      edges,
    );
    ...
    const relevantEdges = edges.filter(
      (edge) =>
        ...
        (edge.data?.condition === branch ||
          (!edge.data?.condition && branch === "true")),
    );
  ...
  private calculateCriticalPath(
    ...
    try {
      ...
      this.log(
        "warn",
        "Could not calculate critical path, using simple traversal",
        {
  ...
  private buildDependencyGraph(
    ...
    const stronglyConnectedComponents =
      GraphAnalyzer.findStronglyConnectedComponents(nodes, edges);
