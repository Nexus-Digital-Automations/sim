...
describe('Real-Time Communication Integration Tests', () => {
  ...
  describe('🔌 Socket.io Connection and Authentication', () => {
    test('Establishes authenticated Socket.io connection', async () => {
    ...
    test('Rejects connection with invalid authentication', async () => {
  ...
  describe('🏢 Workspace-Scoped Real-Time Events', () => {
    test('Users can join workspace-specific rooms', async () => {
      const user = testUsers[0];
      const authToken = generateValidJwtToken(user);
    ...
    test('Workspace isolation prevents cross-workspace event access', async () => {
            ...
            if (user.id === user1.id) {
              ...
              setTimeout(() => {
                ...
                if (messagesReceived === 1) {
                  ...
                  console.log(`  ❌ Workspace isolation failed: ${messagesReceived} messages received`);
                  socket1.disconnect();
                  socket2.disconnect();
              ...
              socket1.disconnect();
              socket2.disconnect();
              reject(new Error('Workspace isolation failed: User 2 received cross-workspace message'));
            }
          });
        };
  ...
  describe('🤖 Real-Time Agent Conversations', () => {
      ...
      return new Promise((resolve, reject) => {
        ...
        socket.on('agent_response_complete', (data) => {
          ...
          resolve({
            ...
            total_content_length: responseChunks.reduce((sum, chunk) => sum + chunk.content.length, 0)
          });
        });
        ...
        setTimeout(() => {
          if (!conversationComplete) {
            console.log(`  ⚠️  Agent conversation timeout after receiving ${responseChunks.length} chunks`);
            socket.disconnect();
    ...
    test('Handles multiple concurrent agent conversations', async () => {
      ...
      return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
          if (conversationsCompleted < concurrentConnections) {
            console.log(`  ⚠️  Concurrent conversation timeout: ${conversationsCompleted}/${concurrentConnections} completed`);
      ...
      return new Promise((resolve, reject) => {
        ...
        for (let i = 0; i < concurrentConnections; i++) {
          ...
          socket.on('connect', () => {
            ...
            if (connectionsEstablished === concurrentConnections && !allConnectionsReady) {
              ...
              console.log(`  ✅ All ${concurrentConnections} connections established in ${connectionTime.toFixed(2)}ms`);
        ...
        setTimeout(() => {
          if (connectionsEstablished < concurrentConnections) {
            console.log(`  ⚠️  Performance test partial success: ${connectionsEstablished}/${concurrentConnections} connections`);
            sockets.forEach(s => s.disconnect());
