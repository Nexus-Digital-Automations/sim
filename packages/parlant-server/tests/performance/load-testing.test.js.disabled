...
describe('Performance and Load Testing Suite', () => {
  ...
  describe('🔐 Authentication Performance Testing', () => {
    test('Concurrent authentication load testing', async () => {
      ...
      for (const user of testUsers) {
        ...
        authPromises.push(
          axios
            .get(`${testConfig.parlant_server_url}/api/v1/profile`, {
              headers: {
      ...
      const avgResponseTime =
        results.filter((r) => r.response_time).reduce((sum, r) => sum + r.response_time, 0) /
        successful
      ...
      console.log(
        `✅ Authentication Load Test: ${successful}/${concurrentUsers} successful (${avgResponseTime.toFixed(2)}ms avg)`
      )
    }, 30000)
    ...
    test('JWT token validation performance under load', async () => {
      ...
      for (let i = 0; i < tokenValidationCount; i++) {
        validationPromises.push(
          axios
            .get(`${testConfig.parlant_server_url}/api/v1/user-context`, {
              headers: {
                Authorization: `Bearer ${authToken}`,
                'Content-Type': 'application/json',
              },
      ...
      console.log(
        `✅ JWT Validation Performance: ${validationsPerSecond.toFixed(2)} validations/sec`
      )
    }, 45000)
  })
  ...
  describe('🤖 Agent Management Performance Testing', () => {
    test('Concurrent agent creation load testing', async () => {
      ...
      const avgCreationTime =
        results.filter((r) => r.creation_time).reduce((sum, r) => sum + r.creation_time, 0) /
        successful
      ...
      console.log(
        `✅ Agent Creation Load Test: ${successful}/${concurrentAgents} successful (${avgCreationTime.toFixed(2)}ms avg)`
      )
    }, 60000)
    ...
    test('Agent CRUD operations performance benchmark', async () => {
      ...
      for (let i = 0; i < Math.min(operationsCount, createdAgents.length * 10); i++) {
        ...
        await axios.get(`${testConfig.parlant_server_url}/api/v1/agents/${agent.id}`, {
          headers: {
            Authorization: `Bearer ${authToken}`,
            'Content-Type': 'application/json',
          },
      ...
      performanceMetrics.agent_management.crud_benchmark = {
        ...
        operations_total:
          crudMetrics.create.length + crudMetrics.read.length + crudMetrics.update.length,
      }
      ...
      console.log(
        `✅ CRUD Benchmark: CREATE avg ${performanceMetrics.agent_management.crud_benchmark.create.avg.toFixed(2)}ms, READ avg ${performanceMetrics.agent_management.crud_benchmark.read.avg.toFixed(2)}ms`
      )
    }, 120000)
  })
  ...
  describe('🔌 Real-Time Communication Performance', () => {
    test('Multiple concurrent Socket.io connections stress test', async () => {
      ...
      const avgConnectionTime =
        results.filter((r) => r.connection_time).reduce((sum, r) => sum + r.connection_time, 0) /
        successful
      ...
      console.log(
        `✅ Socket.io Stress Test: ${successful}/${concurrentConnections} connections (${avgConnectionTime.toFixed(2)}ms avg)`
      )
    }, 60000)
      ...
      return new Promise((resolve, reject) => {
        ...
        socket.on('test_message_ack', (data) => {
          ...
          if (messagesReceived === messageCount) {
            ...
            console.log(
              `✅ Message Throughput: ${messagesPerSecond.toFixed(2)} msg/sec, ${avgLatency.toFixed(2)}ms avg latency`
            )
        ...
        setTimeout(() => {
          if (messagesReceived < messageCount) {
            console.log(
              `  ⚠️  Message throughput test incomplete: ${messagesReceived}/${messageCount} received`
            )
    ...
    test('Database connection pool efficiency under load', async () => {
      ...
      for (let i = 0; i < concurrentDbOperations; i++) {
        ...
        if (operationType === 0) {
          ...
          dbOperationPromises.push(
            axios
              .get(`${testConfig.parlant_server_url}/api/v1/agents`, {
                headers: {
                  Authorization: `Bearer ${authToken}`,
                  'Content-Type': 'application/json',
                },
          ...
          dbOperationPromises.push(
            axios
              .get(`${testConfig.parlant_server_url}/health`)
              .then(() => ({
                operation: 'HEALTH',
                success: true,
                time: performance.now() - startTime,
      ...
      console.log(
        `✅ Database Pool Test: ${successful}/${concurrentDbOperations} operations (${(successful / (totalTime / 1000)).toFixed(2)} ops/sec)`
      )
    }, 60000)
  })
  ...
  async function cleanupCreatedAgents() {
    ...
    const cleanupPromises = createdAgents.map((agent) =>
      axios
        .delete(`${testConfig.parlant_server_url}/api/v1/agents/${agent.id}`, {
          headers: {
  ...
  async function generatePerformanceReport() {
    const report = {
      ...
      summary: {
        ...
        agent_crud_performance:
          performanceMetrics.agent_management?.crud_benchmark?.read?.avg || 'N/A',
        realtime_performance: performanceMetrics.real_time_communication?.success_rate || 'N/A',
        database_performance:
          performanceMetrics.database_performance?.operations_per_second || 'N/A',
      },
    }
