...
class ComprehensiveTestOrchestrator {
  ...
  async executeTestSuites(runId) {
    ...
    for (let i = 0; i < this.executionOrder.length; i++) {
      ...
      try {
        ...
        console.log(`${suiteResult.success ? '✅' : '❌'} ${suiteName} completed: ${suiteResult.passed_tests}/${suiteResult.total_tests} tests passed`);

        // Stop execution if critical suite fails and not configured to continue
        if (!suiteResult.success && this.isCriticalSuite(suiteName) && !this.config.continue_on_failure) {
  ...
  async executeSingleSuite(runId, suiteName, executionOrder) {
    ...
    try {
      ...
      await this.recordSuiteExecution(runId, suiteName, executionOrder, 'completed', normalizedResult);
  ...
  normalizeSuiteResult(result, executionTime) {
    return {
      ...
      failed_tests: result.failedTests || ((result.totalTests || result.totalSuites || 0) - (result.passedTests || result.passedSuites || 0)),
      results: result.results || [],
  ...
  async aggregateResults(suiteResults, overallExecutionTime) {
    const aggregated = {
      ...
      summary: {
        ...
        total_passed: Object.values(suiteResults).reduce((sum, r) => sum + (r.passed_tests || 0), 0),
        total_failures: Object.values(suiteResults).reduce((sum, r) => sum + (r.failed_tests || 0), 0)
      },
    ...
    aggregated.summary.success_rate = aggregated.summary.total_tests > 0 ?
  ...
  calculatePerformanceMetrics(suiteResults) {
    ...
    if (suiteExecutionTimes.length > 0) {
      metrics.total_execution_time = suiteExecutionTimes.reduce((sum, suite) => sum + suite.executionTime, 0);

      const totalTests = suiteExecutionTimes.reduce((sum, suite) => sum + suite.testCount, 0);
      if (totalTests > 0) {
        metrics.average_execution_time_per_test = Math.round(metrics.total_execution_time / totalTests);
      }

      // Find fastest and slowest suites
      const sortedByTime = [...suiteExecutionTimes].sort((a, b) => a.executionTime - b.executionTime);
  ...
  generateRecommendations(suiteResults) {
    ...
    Object.entries(suiteResults).forEach(([suiteName, result]) => {
      ...
      if (result.executionTime > 60000) { // > 1 minute
        recommendations.push({
    ...
    const totalFailures = Object.values(suiteResults).reduce((sum, r) => sum + (r.failed_tests || 0), 0);
    if (totalFailures === 0) {
      recommendations.push({
        category: 'success',
        priority: 'info',
  ...
  async generateComprehensiveReports(runId, aggregatedResults) {
    ...
    try {
      ...
      await fs.writeFile(performanceReportPath, JSON.stringify({
        report_type: 'performance_analysis',
        generated_at: new Date().toISOString(),
        run_id: runId,
        performance_metrics: aggregatedResults.performance_metrics,
        suite_performance: Object.entries(aggregatedResults.suite_results).map(([suite, result]) => ({
          suite,
          execution_time_ms: result.executionTime,
          tests_per_second: result.total_tests > 0 ? (result.total_tests / (result.executionTime / 1000)).toFixed(2) : 0,
          success_rate: result.total_tests > 0 ? ((result.passed_tests / result.total_tests) * 100).toFixed(2) + '%' : '0%'
        }))
      }, null, 2));
      ...
      await fs.writeFile(recommendationsReportPath, JSON.stringify({
        report_type: 'test_recommendations',
        generated_at: new Date().toISOString(),
        run_id: runId,
        recommendations: aggregatedResults.recommendations,
        action_items: aggregatedResults.recommendations.filter(r => r.priority === 'high' || r.priority === 'critical')
      }, null, 2));
  ...
  async recordTestRunCompletion(runId, status, results) {
    ...
    try {
      await client.query(`
        ...
        WHERE run_id = $1
      `, [runId, status, JSON.stringify(results.suite_results || {}), JSON.stringify(results.summary || {})]);
    } finally {
      client.release();
    }
  }
  ...
  async recordSuiteExecution(runId, suiteName, executionOrder, status, result) {
    ...
    try {
      if (status === 'started') {
        ...
        `, [
          runId, suiteName, status, result.executionTime || 0,
          result.total_tests || 0, result.passed_tests || 0, result.failed_tests || 0,
