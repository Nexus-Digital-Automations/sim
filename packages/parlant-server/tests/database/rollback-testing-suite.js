...
class RollbackTestingSuite {
  ...
  async testRollbackWithDataProtection(rollbackScript, migrationName, options = {}) {
    ...
    try {
      ...
      await this.recordTestPhase(testId, testName, migrationName, 'pre_rollback_backup', 'completed', {
      ...
      await this.recordTestPhase(testId, testName, migrationName, 'pre_rollback_validation', 'started');
      ...
      await this.recordTestPhase(testId, testName, migrationName, 'pre_rollback_validation', 'completed');
      ...
      const rollbackResults = await this.executeMonitoredRollback(testId, rollbackScript, migrationName);
      ...
      await this.recordTestPhase(testId, testName, migrationName, 'rollback_execution', 'completed', {
        execution_time_ms: rollbackResults.executionTime
      });

      // Phase 4: Post-rollback data verification
      await this.recordTestPhase(testId, testName, migrationName, 'post_rollback_verification', 'started');

      const postValidation = await this.validatePostRollbackState(testId, migrationName, backupResults);
      if (!postValidation.success) {
        throw new Error(`Post-rollback validation failed: ${postValidation.errors.join(', ')}`);
      }

      await this.recordTestPhase(testId, testName, migrationName, 'post_rollback_verification', 'completed');

      // Phase 5: Data consistency verification
      await this.recordTestPhase(testId, testName, migrationName, 'data_consistency_check', 'started');
      ...
      await this.recordTestPhase(testId, testName, migrationName, 'data_consistency_check', 'completed');
      ...
      return {
        testId,
        success: true,
        phases: ['backup', 'pre_validation', 'rollback', 'post_validation', 'consistency', 'cleanup'],
    ...
    try {
      ...
      for (const tableName of tables) {
        try {
          ...
          `, [crypto.randomUUID(), backupId, tableName, rowCount, checksum, JSON.stringify(backupData)]);
  ...
  async validatePreRollbackState(testId, migrationName) {
    ...
    try {
      ...
      for (const tableName of expectedTables) {
        ...
        await this.recordValidationResult(testId, 'table_existence', tableName, { expected: true }, { exists: exists.rows[0].exists }, isValid);
      ...
      for (const fk of expectedForeignKeys) {
        ...
        const isValid = exists.rows[0].exists;
        if (!isValid) {
          errors.push(`Expected foreign key ${fk.table}.${fk.column} -> ${fk.referenced_table}.${fk.referenced_column} does not exist`);
        }

        await this.recordValidationResult(testId, 'foreign_key_existence', `${fk.table}.${fk.column}`, fk, { exists: exists.rows[0].exists }, isValid);
        validationResults.push({ type: 'foreign_key', constraint: `${fk.table}.${fk.column}`, valid: isValid });
      }
      ...
      for (const tableName of criticalSimTables) {
        try {
          ...
          await this.recordValidationResult(testId, 'sim_table_integrity', tableName, { table: tableName }, { count }, true);
          validationResults.push({ type: 'sim_table_integrity', table: tableName, count, valid: true });

        } catch (error) {
          errors.push(`Critical Sim table '${tableName}' validation failed: ${error.message}`);
          await this.recordValidationResult(testId, 'sim_table_integrity', tableName, { table: tableName }, { error: error.message }, false);
          validationResults.push({ type: 'sim_table_integrity', table: tableName, valid: false, error: error.message });
        }
      }

      console.log(`✅ Pre-rollback validation completed: ${validationResults.length} checks, ${errors.length} errors`);
  ...
  async validatePostRollbackState(testId, migrationName, backupResults) {
    ...
    try {
      ...
      for (const tableName of expectedRemovedTables) {
        ...
        await this.recordValidationResult(testId, 'table_removal', tableName, { expected_removed: true }, { exists: exists.rows[0].exists }, isValid);
      ...
      for (const fk of expectedRemovedForeignKeys) {
        ...
        const isValid = !exists.rows[0].exists; // Should NOT exist after rollback
        if (!isValid) {
          errors.push(`Foreign key ${fk.table}.${fk.column} -> ${fk.referenced_table}.${fk.referenced_column} was not removed`);
        }

        await this.recordValidationResult(testId, 'foreign_key_removal', `${fk.table}.${fk.column}`, fk, { exists: exists.rows[0].exists }, isValid);
        validationResults.push({ type: 'foreign_key_removal', constraint: `${fk.table}.${fk.column}`, valid: isValid });
      }
      ...
      for (const tableName of simTables) {
        try {
          ...
          await this.recordValidationResult(testId, 'sim_table_preservation', tableName, { expected_exists: true }, { exists: exists.rows[0].exists, count }, isValid);
          validationResults.push({ type: 'sim_table_preservation', table: tableName, count, valid: isValid });

        } catch (error) {
          errors.push(`Sim table '${tableName}' validation failed after rollback: ${error.message}`);
          await this.recordValidationResult(testId, 'sim_table_preservation', tableName, { expected_exists: true }, { error: error.message }, false);
          validationResults.push({ type: 'sim_table_preservation', table: tableName, valid: false, error: error.message });
        }
      }

      console.log(`✅ Post-rollback validation completed: ${validationResults.length} checks, ${errors.length} errors`);
    ...
    try {
      ...
      for (const tableBackup of backupData.tables) {
        ...
        try {
          ...
          const countResult = await client.query(`SELECT COUNT(*) as count FROM "${tableBackup.tableName}"`);
          const currentCount = parseInt(countResult.rows[0].count);

          // Check if data counts match (for non-Parlant tables)
          if (!this.isParlantTable(tableBackup.tableName)) {
            if (currentCount !== tableBackup.rowCount) {
              errors.push(`Data count mismatch in '${tableBackup.tableName}': expected ${tableBackup.rowCount}, found ${currentCount}`);
              consistencyResults.push({
                table: tableBackup.tableName,
                type: 'count_mismatch',
          ...
          errors.push(`Consistency check failed for table '${tableBackup.tableName}': ${error.message}`);
          consistencyResults.push({
            table: tableBackup.tableName,
            type: 'check_error',
            error: error.message,
      ...
      console.log(`✅ Data consistency verification completed: ${consistencyResults.length} checks, ${errors.length} errors`);
   ...
   */
  async recordValidationResult(testId, validationType, tableName, expectedState, actualState, isValid, errorMessage = null) {
    try {
      const client = await this.dbPool.connect();
      try {
        await client.query(`
        ...
        `, [testId, validationType, tableName, JSON.stringify(expectedState), JSON.stringify(actualState), isValid, errorMessage]);
      } finally {
        client.release();
      }
    } catch (error) {
  ...
  isCriticalTable(tableName) {
    const criticalTables = ['user', 'workspace', 'workflow', 'session', 'organization', 'permissions'];
  ...
  getExpectedForeignKeysForMigration(migrationName) {
    const migrationForeignKeys = {
      'parlant_schema_extension': [
        { table: 'parlant_agent', column: 'workspace_id', referenced_table: 'workspace', referenced_column: 'id' },
        { table: 'parlant_agent', column: 'created_by', referenced_table: 'user', referenced_column: 'id' },
        { table: 'parlant_session', column: 'agent_id', referenced_table: 'parlant_agent', referenced_column: 'id' },
        { table: 'parlant_session', column: 'workspace_id', referenced_table: 'workspace', referenced_column: 'id' },
        { table: 'parlant_event', column: 'session_id', referenced_table: 'parlant_session', referenced_column: 'id' }
      ]
    };
