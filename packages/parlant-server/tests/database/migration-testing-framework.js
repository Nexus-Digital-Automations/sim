...
class MigrationTestingFramework {
  ...
  async testForwardMigration(migrationScript, migrationName) {
    ...
    try {
      ...
      if (validationResults.success) {
        ...
        console.log(`❌ Forward migration ${migrationName} failed: ${validationResults.errors.join(', ')}`);
      }
  ...
  async testRollbackMigration(rollbackScript, migrationName) {
    ...
    try {
      ...
      if (validationResults.success) {
        ...
        console.log(`❌ Rollback migration ${migrationName} failed: ${validationResults.errors.join(', ')}`);
      }
  ...
  async testMigrationIdempotency(migrationScript, migrationName, iterations = 3) {
    ...
    this.testResults.migrations.push({
      ...
      avgExecutionTime: successfulRuns.reduce((sum, r) => sum + r.executionTime, 0) / successfulRuns.length
    });

    if (isIdempotent) {
      console.log(`✅ Migration ${migrationName} is idempotent (${successfulRuns.length}/${iterations} successful runs)`);
    } else {
      console.log(`❌ Migration ${migrationName} is not idempotent (${changesDetected.length} runs changed state)`);
    }
  ...
  async testMigrationPerformance(migrationScript, migrationName, options = {}) {
    ...
    try {
      ...
      const performanceResults = {
        ...
        avg_execution_time: successfulRuns.length > 0 ?
          successfulRuns.reduce((sum, r) => sum + r.value.executionTime, 0) / successfulRuns.length : 0,
      ...
      await this.recordTestResult({
        ...
        error_details: failedRuns.length > 0 ?
          failedRuns.map(r => r.reason.message).join('; ') : null
      });

      this.testResults.performance.push(performanceResults);

      if (performanceResults.within_threshold) {
        console.log(`✅ Performance test ${migrationName} passed in ${executionTime}ms (${successfulRuns.length}/${testConfig.concurrent_connections} connections)`);
      } else {
        console.log(`❌ Performance test ${migrationName} exceeded threshold: ${executionTime}ms > ${this.config.performance_threshold_ms}ms`);
      }
    ...
    try {
      ...
      for (const fk of expectedForeignKeys) {
        ...
        if (!fkExists.rows[0].exists) {
          errors.push(`Expected foreign key ${fk.table}.${fk.column} -> ${fk.referenced_table}.${fk.referenced_column} was not created`);
        }
      }
  ...
  async validateRollbackResults(client, migrationName, testData) {
    ...
    try {
      ...
      if (testData && testData.existing_data) {
        for (const tableData of testData.existing_data) {
          ...
          if (currentData.rows[0].count !== tableData.row_count) {
            errors.push(`Data loss detected in table '${tableData.table_name}': expected ${tableData.row_count}, found ${currentData.rows[0].count}`);
          }
        }
      }
