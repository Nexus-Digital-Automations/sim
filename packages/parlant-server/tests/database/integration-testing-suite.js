...
class IntegrationTestingSuite {
  ...
  async testAgentUserRelationship(testData) {
    ...
    try {
      ...
      const success = joinResult.rows.length === 1 &&
  ...
  async testAgentWorkspaceRelationship(testData) {
    ...
    try {
      ...
      return {
        ...
        success: parseInt(workspaceCheck.rows[0].count) > 0,
        agentCount: parseInt(workspaceCheck.rows[0].count)
      };
  ...
  async getRelatedRecordCounts(client, testData) {
    ...
    try {
      ...
      counts.agents = parseInt(agentCount.rows[0].count);
    } catch (error) {
    ...
    try {
      ...
      counts.sessions = parseInt(sessionCount.rows[0].count);
    } catch (error) {
      // Table might not exist
    }
  ...
  async cleanupCrossSchemaTestData(testData) {
    ...
    try {
      ...
      await client.query('DELETE FROM parlant_session WHERE id = $1', [testData.sessionId]).catch(() => {});
      await client.query('DELETE FROM parlant_agent WHERE id = $1', [testData.agentId]).catch(() => {});
      await client.query('DELETE FROM workflow WHERE id = $1', [testData.workflowId]).catch(() => {});
      await client.query('DELETE FROM workspace WHERE id = $1', [testData.workspaceId]).catch(() => {});
   ...
   */
  async recordIntegrationResult(category, scenario, integrationType, status, executionTime, performanceMetrics, validationResults, errorDetails) {
    try {
      const client = await this.dbPool.connect();
      try {
        await client.query(`
        ...
        `, [
          category, scenario, integrationType, status, executionTime,
          JSON.stringify(performanceMetrics), JSON.stringify(validationResults), errorDetails
        ]);
      } finally {
        client.release();
      }
  ...
  async generateIntegrationReport() {
    const reportData = {
      ...
      summary: {
        ...
        total_tests: Object.values(this.integrationResults).reduce((sum, tests) => sum + tests.length, 0),
        passed_tests: Object.values(this.integrationResults).reduce((sum, tests) => sum + tests.filter(t => t.success).length, 0),
