...
class RegressionTestingSuite {
  ...
  async establishPerformanceBaselines() {
    ...
    for (const op of coreOperations) {
      try {
        ...
        console.log(
          `✅ Baseline established for ${op.name}: ${baseline.avgTime}ms (${baseline.sampleSize} samples)`
        )
      } catch (error) {
  ...
  async testUserManagement() {
    ...
    try {
      ...
      // Test user update
      const userUpdateResult = await this.testOperation('user_management', 'user_update', () =>
  ...
  async testWorkflowOperations() {
    ...
    try {
      ...
      const blocksResult = await this.testOperation('workflow_operations', 'workflow_blocks', () =>
        this.testWorkflowBlocks(testWorkflowId)
      )
      testResults.push(blocksResult)

      // Test workflow edges operations
      const edgesResult = await this.testOperation('workflow_operations', 'workflow_edges', () =>
      ...
      const folderResult = await this.testOperation('workflow_operations', 'workflow_folders', () =>
  ...
  async testKnowledgeBaseOperations() {
    ...
    try {
      ...
      const kbCreationResult = await this.testOperation('knowledge_base', 'kb_creation', () =>
        this.testKnowledgeBaseCreation(testUserId, testWorkspaceId)
      )
      testResults.push(kbCreationResult)

      // Test document operations
      const documentResult = await this.testOperation('knowledge_base', 'document_operations', () =>
    ...
    try {
      ...
      if (baselineKey && this.baselineMetrics.has(baselineKey)) {
        ...
        if (performanceRatio > this.config.performance_baseline_threshold) {
          console.warn(
            `⚠️  Performance degradation detected in ${operationName}: ${executionTime}ms vs baseline ${baselineTime}ms (${(performanceRatio * 100 - 100).toFixed(1)}% increase)`
          )
        }
      }
  ...
  async generateRegressionReport() {
    const reportData = {
      ...
      summary: {
        ...
        total_tests: Object.values(this.regressionResults).reduce(
          (sum, tests) => sum + tests.length,
          0
        ),
        passed_tests: Object.values(this.regressionResults).reduce(
        ...
        performance_degradations: Object.values(this.regressionResults)
          ...
          .filter(
            (t) =>
              t.performanceRatio && t.performanceRatio > this.config.performance_baseline_threshold
          ).length,
