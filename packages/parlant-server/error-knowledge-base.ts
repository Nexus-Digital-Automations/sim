...
export class ErrorKnowledgeBase extends EventEmitter {
  ...
  async search(query: SearchQuery): Promise<{
    ...
    const scoredResults = await this.scoreResults(filteredIds, searchTerms, query.userContext)
      ...
      versions: [
        ...existingArticle.versions,
        {
          version: this.incrementVersion(
            existingArticle.versions[existingArticle.versions.length - 1].version
          ),
          timestamp: new Date().toISOString(),
          author,
  ...
  private calculateRelevanceScore(
    ...
    const contentMatches = searchTerms.filter((term) => contentText.includes(term)).length
 ...
 */
export const searchKnowledgeBase = (query: SearchQuery) => errorKnowledgeBase.search(query)
