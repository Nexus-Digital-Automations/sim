...
export class ParlantMonitoringService {
    queryTimes = [];
    maxQueryTimeHistory = 1000;
    alertThresholds;
    constructor(alertThresholds = DEFAULT_ALERT_THRESHOLDS) {
            ...
            return {
                ...
                database: {
                    connectionCount: connectionResult.status === 'fulfilled'
                        ? Number.parseInt(connectionResult.value[0]?.total_connections || '0')
                        : 0,
                    queryTime: {
                    ...
                    activeQueries: connectionResult.status === 'fulfilled'
                        ? Number.parseInt(connectionResult.value[0]?.active_connections || '0')
                        : 0,
                    slowQueries: slowQueryResult.status === 'fulfilled'
                        ? Number.parseInt(slowQueryResult.value[0]?.slow_queries || '0')
                        : 0,
                },
                memory: {
     ...
     */
    async getAgentPerformanceMetrics(timeWindowMinutes = 60, agentId) {
        try {
            logger.debug('Collecting agent performance metrics', { timeWindowMinutes, agentId });
     ...
     */
    async getUsageMetrics(periodHours = 24) {
        try {
    ...
    async checkAlertConditions() {
        try {
            ...
            if (heapUsagePercent > this.alertThresholds.memory.heapUsagePercent) {
                alerts.push({
                    severity: heapUsagePercent > 90 ? 'critical' : 'warning',
                    category: 'memory',
            ...
            if (alerts.length > 0) {
                logger.warn('Alert conditions detected', { alertCount: alerts.length, systemHealth });
            }
...
export const monitoring = {
    ...
    agents: (timeWindow, agentId) => parlantMonitoring.getAgentPerformanceMetrics(timeWindow, agentId),
    usage: (periodHours) => parlantMonitoring.getUsageMetrics(periodHours),
    alerts: () => parlantMonitoring.checkAlertConditions(),
    dashboard: () => parlantMonitoring.generateDashboardData(),
    recordQuery: (duration) => parlantMonitoring.recordQueryTime(duration),
};
