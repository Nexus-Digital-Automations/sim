...
export class ParlantMonitoringService {
        ...
        database: {
          connectionCount:
            connectionResult.status === 'fulfilled'
              ? Number.parseInt(connectionResult.value[0]?.total_connections || '0', 10)
              : 0,
          queryTime: {
            average:
          ...
          activeQueries:
            connectionResult.status === 'fulfilled'
              ? Number.parseInt(connectionResult.value[0]?.active_connections || '0', 10)
              : 0,
          slowQueries:
            slowQueryResult.status === 'fulfilled'
              ? Number.parseInt(slowQueryResult.value[0]?.slow_queries || '0', 10)
              : 0,
        },
        memory: {
   ...
   */
  async getAgentPerformanceMetrics(timeWindowMinutes, agentId) {
    try {
