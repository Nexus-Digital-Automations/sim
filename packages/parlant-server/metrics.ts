...
export class AgentMetricsTracker {
  ...
  getMetrics(timeWindowHours = 24): Partial<AgentMetrics> {
    ...
    const totalErrors = relevantSessions.reduce(
      (sum, [_, session]) => sum + session.errors,
      0,
    );
    const totalTokens = relevantSessions.reduce(
      (sum, [_, session]) => sum + session.tokenCount,
      0,
    );
    ...
    const allResponseTimes = relevantSessions.flatMap(
      ([_, session]) => session.responseTimes,
    );
    ...
    const successRate =
      totalSessions > 0 ? (completedSessions / totalSessions) * 100 : 100;
...
export class SystemMetricsCollector {
  ...
  getAgentTracker(agentId: string, workspaceId: string): AgentMetricsTracker {
    ...
    if (!this.agentTrackers.has(key)) {
      this.agentTrackers.set(
        key,
        new AgentMetricsTracker(agentId, workspaceId),
      );
    }
  ...
  async collectSystemMetrics(): Promise<SystemMetrics> {
    ...
    try {
      ...
      const agentMetrics = Array.from(this.agentTrackers.values()).map(
        (tracker) => tracker.getMetrics(1),
      ); // Last hour
      ...
      const totalErrors = agentMetrics.reduce(
            ...
            database: {
              connections:
                dbStats.status === "fulfilled"
                  ? Number.parseInt(
                      (dbStats.value[0] as any)?.total_connections || "0",
                      10,
                    )
                  : 0,
