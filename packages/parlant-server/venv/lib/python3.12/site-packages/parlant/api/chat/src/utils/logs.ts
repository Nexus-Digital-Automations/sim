...
export const handleChatLogs = async (log: Log) => {
  ...
  logEntry.onsuccess = () => {
    ...
    if (!data?.values) {
      if (!log.message?.trim().startsWith('HTTP') || log.message?.includes('/events'))
...
export const getMessageLogsWithFilters = async (
  correlation_id: string,
  filters: { level: string; types?: string[]; content?: string[] }
): Promise<Log[]> => {
  const logs = await getMessageLogs(correlation_id)
  const escapedWords = filters?.content?.map((word) =>
    word.replace(/([.*+?^=!:${}()|[\]/\\])/g, '\\$1')
  )
  const pattern = escapedWords?.map((word) => `\\[?${word}\\]?`).join('.*?')
  const levelIndex = filters.level ? logLevels.indexOf(filters.level) : null
  const validLevels = filters.level
    ? new Set(logLevels.filter((_, i) => i <= (levelIndex as number)))
    : null
    ...
    return new Promise((resolve, reject) => {
      ...
      const deleteOldest = () => {
        ...
        deleteTransaction.oncomplete = () => {
          db.close()
          console.log(
            `Successfully deleted ${completed} records older than ${new Date(deleteTimestamp).toISOString()}`
          )
          resolve()
        }
...
export async function checkAndCleanupLogs(): Promise<void> {
  try {
    ...
    if (agentMessages[MAX_RECORDS]) {
      ...
      console.log(
        `Log count exceeds maximum (${MAX_RECORDS}), deleting logs before ${new Date(recordsToDeleteDate)?.toLocaleString()}`
      )
