...
const MessageRelativeTime = ({ event }: { event: EventInterface }) => {
  const [time, setTime] = useState(
    event.serverStatus === 'pending' ? 'Just Now' : timeAgo(event.creation_utc)
  )
  ...
  useEffect(() => {
    if (event.serverStatus !== 'pending' && time === 'Just Now')
      setTime(timeAgo(event.creation_utc))
  }, [event?.serverStatus])

  useEffect(setMessageRelativeTime, [event.creation_utc])

  return <div className='font-light text-[#A9A9A9] text-[14px]'>{time}</div>
}
