...
export class ErrorRecoveryService {
  ...
  async executeWithRecovery(operation, context, customConfig) {
    ...
    for (let attempt = 1; attempt <= config.maxAttempts; attempt++) {
      ...
      const delayMs =
        attempt === 1 ? 0 : this.calculateDelay(attempt - 1, config);
      if (delayMs > 0) {
      ...
      try {
        ...
        const shouldCircuitBreak = this.shouldTriggerCircuitBreaker(
          circuitKey,
          error,
        );
    ...
    if (error instanceof BaseToolError) {
      if (
        ...
        return (
          ...
          Math.max(1, config.circuitBreakerThreshold / 2)
        );
      }
    }
  ...
  async attemptFallback(context, error, attempts, config) {
    ...
    const fallbackStrategy =
      this.fallbackStrategies.get(fallbackKey) ||
  ...
  getRecoveryStatistics(timeWindowMs = 3600000) {
    ...
    const totalOps = new Set(operations.map((op) => op.attemptNumber === 1))
      .size;
 ...
 */
export const executeWithRecovery = (operation, context, customConfig) =>
