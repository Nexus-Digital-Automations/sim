...
import { ErrorCategory, ErrorImpact, ErrorSeverity, errorClassifier, RecoveryStrategy, } from './error-taxonomy';
...
export class BaseToolError extends Error {
    ...
    constructor(message, category, subcategory, component, context = {}, originalError) {
        ...
        this.classification = errorClassifier.classify(category, subcategory, message, originalError, context, component);
    ...
    async trackError(originalError) {
        try {
            await parlantErrorTracker.trackError(this.severity === ErrorSeverity.CRITICAL
                ? 'critical'
                : this.severity === ErrorSeverity.ERROR
                    ? 'error'
                    : 'warning', this.category, // Type conversion for compatibility
            this.component, this.message, originalError || this, this.context, {
                errorId: this.id,
...
export class ExternalServiceError extends BaseToolError {
    ...
    httpStatus;
    retryAfter;
    constructor(message, subcategory, serviceName, context = {}, originalError, serviceUrl, httpStatus, retryAfter) {
 ...
 */
export const createToolAdapterError = (message, subcategory, toolName, context, originalError) => new ToolAdapterError(message, subcategory, toolName, context, originalError);
export const createToolExecutionError = (message, subcategory, toolName, context, originalError, operationId, executionTime) => new ToolExecutionError(message, subcategory, toolName, context, originalError, operationId, executionTime);
export const createToolAuthenticationError = (message, subcategory, toolName, context, originalError, authMethod, tokenExpired) => new ToolAuthenticationError(message, subcategory, toolName, context, originalError, authMethod, tokenExpired);
export const createUserInputError = (message, subcategory, context, fieldName, validationRule, providedValue) => new UserInputError(message, subcategory, context, fieldName, validationRule, providedValue);
export const createSystemResourceError = (message, subcategory, resourceType, context, originalError, currentUsage, limit) => new SystemResourceError(message, subcategory, resourceType, context, originalError, currentUsage, limit);
export const createExternalServiceError = (message, subcategory, serviceName, context, originalError, serviceUrl, httpStatus, retryAfter) => new ExternalServiceError(message, subcategory, serviceName, context, originalError, serviceUrl, httpStatus, retryAfter);
