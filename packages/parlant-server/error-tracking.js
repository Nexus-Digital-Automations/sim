...
export class ParlantErrorTracker {
    ...
    async doesErrorMatchRule(error, rule) {
        ...
        if (conditions.frequency) {
            ...
            const recentErrors = this.errors.filter((err) => new Date(err.timestamp).getTime() >= windowStart &&
    ...
    determineSeverity(errors) {
        ...
        if (levels.includes('critical'))
            return 'critical';
        if (levels.filter((l) => l === 'error').length >= 5)
            return 'high';
     ...
     */
    async executeWebhook(webhook, alert) {
        const payload = {
            alert: {
                id: alert.id,
                title: alert.title,
...
export const errorTracker = {
    trackWarning: (category, service, message, error, context) => parlantErrorTracker.trackError('warning', category, service, message, error, context),
    trackError: (category, service, message, error, context) => parlantErrorTracker.trackError('error', category, service, message, error, context),
    trackCritical: (category, service, message, error, context) => parlantErrorTracker.trackError('critical', category, service, message, error, context),
    ...
    acknowledgeAlert: (alertId, acknowledgedBy) => parlantErrorTracker.acknowledgeAlert(alertId, acknowledgedBy),
