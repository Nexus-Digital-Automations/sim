...
export interface ParlantLogContext {
    ...
    messageId?: string;
    operation?: 'agent_create' | 'agent_update' | 'session_start' | 'session_end' | 'message_process' | 'tool_execute' | 'guideline_apply' | 'journey_step';
    duration?: number;
    responseTime?: number;
    tokenCount?: number;
    toolCalls?: number;
...
export declare class ParlantLogger {
     ...
     */
    logAgentOperation(operation: ParlantLogContext['operation'], message: string, context: ParlantLogContext, level?: 'DEBUG' | 'INFO' | 'WARN' | 'ERROR'): void;
    /**
     * Log session lifecycle events
     */
    logSession(event: 'start' | 'end' | 'message' | 'tool_call', sessionId: string, context?: Omit<ParlantLogContext, 'sessionId'>): void;
    /**
     ...
     */
    logIntegration(event: 'workflow_trigger' | 'tool_execute' | 'auth_check' | 'data_sync', message: string, context: ParlantLogContext): void;
    /**
 ...
 */
export declare function createParlantLogger(module: string, correlationId?: string): ParlantLogger;
/**
