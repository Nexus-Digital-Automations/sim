...
export class ParlantLogger {
     ...
     */
    logAgentOperation(operation, message, context, level = 'INFO') {
     ...
     */
    logSession(event, sessionId, context = {}) {
        const operation = event === 'start' ? 'session_start' : event === 'end' ? 'session_end' : 'message_process';
     ...
     */
    logIntegration(event, message, context) {
    ...
    filterLogs(criteria) {
        return this.logs.filter((log) => {
            if (criteria.level && log.level !== criteria.level)
                return false;
            if (criteria.operation && log.context.operation !== criteria.operation)
                return false;
            if (criteria.agentId && log.context.agentId !== criteria.agentId)
                return false;
            if (criteria.workspaceId && log.context.workspaceId !== criteria.workspaceId)
                return false;
            if (criteria.since && new Date(log.timestamp) < criteria.since)
                return false;
            return true;
        });
    }
     ...
     */
    generateLogAggregation(windowMinutes = 60) {
     ...
     */
    recordErrorMetric(operation, errorType) {
 ...
 */
export function createParlantLogger(module, correlationId) {
