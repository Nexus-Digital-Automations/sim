...
export class MonitoringTestSuite {
  ...
  private async testHealthChecks(): Promise<void> {
    ...
    await this.runTest('Comprehensive Health Check', async () => {
      ...
      if (!healthStatus.status || !healthStatus.timestamp || !healthStatus.services) {
  ...
  private async testMonitoringSystem(): Promise<void> {
    await this.runTest('System Metrics Collection', async () => {
      ...
      if (!systemMetrics.timestamp || !systemMetrics.database || !systemMetrics.memory) {
    ...
    await this.runTest('Usage Metrics Collection', async () => {
      ...
      if (!usageMetrics.period || !usageMetrics.agents || !usageMetrics.sessions) {
  ...
  private async testLoggingSystem(): Promise<void> {
    await this.runTest('Structured Logging', async () => {
      const testLogger = createParlantLogger('TestLogger', 'test-correlation-123')
    ...
    await this.runTest('Log Filtering and Aggregation', async () => {
      ...
      const workspaceLogs = testLogger.filterLogs({ workspaceId: 'workspace-1' })
      const operationLogs = testLogger.filterLogs({ operation: 'agent_create' })
  ...
  private async testMetricsCollection(): Promise<void> {
    ...
    await this.runTest('System Metrics Dashboard', async () => {
      ...
      if (!dashboard.systemMetrics || !dashboard.topPerformingAgents || !dashboard.recentAlerts) {
  ...
  private async testAlertSystem(): Promise<void> {
    await this.runTest('Alert Creation and Management', async () => {
      ...
      // Test acknowledgment
      const acknowledged = alerts.acknowledgeAlert(warningAlert.id, 'test-user')
      if (!acknowledged) {
        throw new Error('Failed to acknowledge alert')
      }

      // Test resolution
      const resolved = alerts.resolveAlert(criticalAlert.id, 'Test resolved by test suite')
      if (!resolved) {
  ...
  private async testIntegrationScenarios(): Promise<void> {
    await this.runTest('Full System Status Check', async () => {
      ...
      if (!systemStatus.status || !systemStatus.health || !systemStatus.metrics) {
    ...
    await this.runTest('Monitoring System Initialization', async () => {
      ...
      if (!['healthy', 'degraded', 'unhealthy'].includes(monitoringSystem.status)) {
        throw new Error(`Invalid monitoring status: ${monitoringSystem.status}`)
      }
  ...
  private async runTest(name: string, testFn: () => Promise<any>): Promise<void> {
    ...
    try {
      logger.debug(`Starting test: ${name}`, { operation: 'test_start', testName: name })
