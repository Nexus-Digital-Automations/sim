...
export class ErrorClassifier {
     ...
     */
    classify(category, subcategory, message, error, context = {}, component = 'unknown') {
        ...
        const { recoverable, recoveryStrategy, suggestedActions } = this.determineRecoveryApproach(category, subcategory, error);
    ...
    assessImpact(category, subcategory, context, severity) {
        ...
        if (subcategory.includes('service') ||
    ...
    generateTags(category, subcategory, component, context) {
        ...
        if (context.toolName)
            tags.push(`tool:${context.toolName}`);
        if (context.agentId)
    ...
    checkPatternFrequency(pattern, newError) {
        ...
        const matchingErrors = this.recentErrors.filter((error) => new Date(error.timestamp).getTime() >= cutoff && this.doesErrorMatchPattern(error, pattern));
    ...
    findRelatedErrors(newError) {
        ...
        const cutoff = Date.now() - timeWindow;
        const recentErrors = this.recentErrors.filter((error) => error.id !== newError.id && new Date(error.timestamp).getTime() >= cutoff);
        // Find errors with same correlation ID
        const correlatedErrors = recentErrors.filter((error) => error.correlationId && error.correlationId === newError.correlationId);
        // Find errors affecting same entities
        const relatedErrors = recentErrors.filter((error) => error.affectedUsers.some((user) => newError.affectedUsers.includes(user)) ||
    ...
    getErrorStatistics(timeWindowMs = 3600000) {
        ...
        const relevantErrors = this.recentErrors.filter((error) => new Date(error.timestamp).getTime() >= cutoff);
        const byCategory = {};
        const bySeverity = {};
        const byComponent = {};
 ...
 */
export const classifyError = (category, subcategory, message, error, context, component) => errorClassifier.classify(category, subcategory, message, error, context, component);
export const getErrorStats = (timeWindowMs) => errorClassifier.getErrorStatistics(timeWindowMs);
