...
const getServerModules = () => {
  if (!isServer) {
    throw new Error("HTML parsing is only available on the server side");
  }
...
export class HtmlParser implements FileParser {
  ...
  async parseBuffer(buffer: Buffer): Promise<FileParseResult> {
    try {
      ...
      $(
        "script, style, noscript, meta, link, iframe, object, embed, svg",
      ).remove();
      ...
      throw new Error(
        `Failed to parse HTML buffer: ${(error as Error).message}`,
      );
    }
  }
