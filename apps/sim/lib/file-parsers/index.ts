...
// Server-side only functions with proper error handling for client-side
const getServerModules = () => {
  if (!isServer) {
    throw new Error("File parsing is only available on the server side");
  }
...
export async function parseBuffer(
  ...
  try {
    ...
    logger.info(
      "Attempting to parse buffer with extension:",
      normalizedExtension,
    );
