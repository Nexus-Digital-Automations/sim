...
function RunSkipButtons({
  ...
  const handleOpenDriveAccess = async () => {
    try {
      ...
      const credsRes = await fetch(
        `/api/auth/oauth/credentials?provider=${providerId}`,
      );
      if (!credsRes.ok) return;
      const credsData = await credsRes.json();
      const creds = Array.isArray(credsData.credentials)
  ...
  if (
    toolCall.Name === "gdrive_request_access" &&
            ...
            await instance.handleAccept?.({
              openDrivePicker: async (accessToken: string) => {
                try {
                  ...
                  const apiKey = getEnv("NEXT_PUBLIC_GOOGLE_API_KEY") || "";
                  const projectNumber =
                    getEnv("NEXT_PUBLIC_GOOGLE_PROJECT_NUMBER") || "";
...
export function InlineToolCall({
  ...
  const renderDisplayIcon = () => {
    try {
      ...
      if (
        state === (ClientToolCallState as any).aborted ||
        state === "aborted"
      ) {
        colorClass = "text-amber-500";
      } else if (
        state === (ClientToolCallState as any).error ||
        state === "error"
      ) {
        colorClass = "text-red-500";
      } else if (
        state === (ClientToolCallState as any).success ||
        state === "success"
      ) {
        const isBuildOrEdit =
          toolCall.Name === "build_workflow" ||
          toolCall.Name === "edit_workflow";
        colorClass = isBuildOrEdit
  ...
  return (
    <div className="flex w-full flex-col gap-1 py-1">
      ...
      {isExpandableTool && expanded && (
        <div className="pr-1 pl-5">{renderPendingDetails()}</div>
      )}
    </div>
  );
}
