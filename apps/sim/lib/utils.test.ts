...
describe("encryption and decryption", () => {
  ...
  it.concurrent("should throw error for invalid decrypt format", async () => {
    await expect(decryptSecret("invalid-format")).rejects.toThrow(
...
describe("convertScheduleOptionsToCron", () => {
  it.concurrent("should convert minutes schedule to cron", () => {
    const result = convertScheduleOptionsToCron("minutes", {
      minutesInterval: "5",
    });
  ...
  it.concurrent("should convert hourly schedule to cron", () => {
    const result = convertScheduleOptionsToCron("hourly", {
      hourlyMinute: "30",
    });
  ...
  it.concurrent("should use custom cron expression directly", () => {
    ...
    const result = convertScheduleOptionsToCron("custom", {
  ...
  it.concurrent("should throw error for unsupported schedule type", () => {
    expect(() => convertScheduleOptionsToCron("invalid", {})).toThrow(
      "Unsupported schedule type",
    );
  });

  it.concurrent(
    "should use default values when options are not provided",
    () => {
      const result = convertScheduleOptionsToCron("daily", {});
...
describe("getTimezoneAbbreviation", () => {
  ...
  it.concurrent("should return PST/PDT for Los Angeles timezone", () => {
    ...
    const winterResult = getTimezoneAbbreviation(
      "America/Los_Angeles",
      winterDate,
    );
    const summerResult = getTimezoneAbbreviation(
      "America/Los_Angeles",
      summerDate,
    );
  ...
  it.concurrent(
    "should return full timezone name for unknown timezones",
    () => {
