 ...
 */
export function createMockNexusContext(
  overrides: Partial<NexusOperationContext> = {}
...
export class NexusTestAssertions {
  ...
  static assertSuccess<T>(
    response: any
  ): asserts response is { status: 'success'; data: T; metadata: any } {
  ...
  static assertError(
    response: any
  ): asserts response is { status: 'error'; message: string; metadata: any } {
...
export class IntegrationTestHelper {
  ...
  async setupTestEnvironment() {
    // Create test users
    const user1 = this.mockDatabase.addUser(
      createMockUser({
    ...
    const user2 = this.mockDatabase.addUser(
      createMockUser({
    ...
    const workspace = this.mockDatabase.addWorkspace(
      createMockWorkspace({
        name: 'Test Workspace',
        ownerId: user1.id,
      })
    ...
    const workflow1 = this.mockDatabase.addWorkflow(
      createMockWorkflow({
    ...
    const workflow2 = this.mockDatabase.addWorkflow(
      createMockWorkflow({
