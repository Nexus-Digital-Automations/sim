...
describe("StreamingResponseFormatProcessor", () => {
  ...
  describe("processStream", () => {
    it.concurrent(
      "should return original stream when no response format selection",
      async () => {
    ...
    it.concurrent(
      "should return original stream when no response format provided",
      async () => {
        const mockStream = new ReadableStream({
          start(controller) {
            controller.enqueue(new TextEncoder().encode('{"content": "test"}'));
    ...
    it.concurrent(
      "should process stream and extract single selected field",
      async () => {
        const mockStream = new ReadableStream({
          start(controller) {
            controller.enqueue(
              new TextEncoder().encode('{"username": "alice", "age": 25}'),
            );
            controller.close();
          },
        });
    ...
    it.concurrent(
      "should process stream and extract multiple selected fields",
      async () => {
        const mockStream = new ReadableStream({
          start(controller) {
            controller.enqueue(
              new TextEncoder().encode(
                '{"username": "bob", "age": 30, "email": "bob@test.com"}',
              ),
            );
            controller.close();
          },
    ...
    it.concurrent(
      "should handle non-string field values by JSON stringifying them",
      async () => {
        const mockStream = new ReadableStream({
          start(controller) {
            controller.enqueue(
    ...
    it.concurrent(
      "should handle streaming JSON that comes in chunks",
      async () => {
        const mockStream = new ReadableStream({
          start(controller) {
            // Simulate streaming JSON in chunks
            controller.enqueue(
              new TextEncoder().encode('{"username": "charlie"'),
            );
    ...
    it.concurrent(
      "should filter selected fields for correct block ID",
      async () => {
        const mockStream = new ReadableStream({
          start(controller) {
            controller.enqueue(
              new TextEncoder().encode('{"username": "eve", "age": 28}'),
            );
            controller.close();
          },
        });
    ...
    it.concurrent(
      "should handle empty result when no matching fields",
      async () => {
        const mockStream = new ReadableStream({
          start(controller) {
            controller.enqueue(
              new TextEncoder().encode('{"other_field": "value"}'),
            );
            controller.close();
          },
        });
  ...
  describe("singleton instance", () => {
    it.concurrent("should export a singleton instance", () => {
      expect(streamingResponseFormatProcessor).toBeInstanceOf(
        StreamingResponseFormatProcessor,
      );
    });
  ...
  describe("edge cases", () => {
    ...
    it.concurrent("should handle very large JSON objects", async () => {
      ...
      const mockStream = new ReadableStream({
        start(controller) {
          controller.enqueue(
            new TextEncoder().encode(JSON.stringify(largeObject)),
          );
          controller.close();
        },
      });
