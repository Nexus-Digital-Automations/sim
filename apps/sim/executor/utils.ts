...
export class StreamingResponseFormatProcessor
  implements ResponseFormatStreamProcessor
{
  processStream(
        ...
        try {
          while (true) {
            ...
            if (!hasProcessedComplete) {
              const processedChunk = self.processStreamingChunk(
                buffer,
                selectedFields,
              );
          ...
          logger.error("Error processing streaming response format:", {
            error,
            blockId,
          });
  ...
  private processCompleteJson(
    ...
    try {
      ...
      logger.warn("Failed to parse complete JSON in streaming processor:", {
        error,
      });
    }
  }
...
export const streamingResponseFormatProcessor =
  new StreamingResponseFormatProcessor();
