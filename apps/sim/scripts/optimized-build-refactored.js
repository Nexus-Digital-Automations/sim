...
function checkModularArchitecture() {
  ...
  if (missingFiles.length > 0) {
    console.log(`⚠️  Warning: ${missingFiles.length} modular architecture files are missing.`)
...
function analyzeBundleSizes() {
  ...
  chunks.slice(0, 10).forEach((chunk) => {
    const sizeStr = chunk.sizeMB > 1 ? `${chunk.sizeMB}MB` : `${chunk.sizeKB}KB`
  ...
  const largeChunks = chunks.filter((chunk) => chunk.size > BUILD_CONFIG.largeFileThresholds.error)
...
async function runOptimizedBuild() {
  ...
  const configFile = hasModularArch ? 'next.config.optimized.ts' : 'next.config.ts'
