    ...
    return new Promise((resolve) => {
      ...
      const child = spawn('npx', ['next', 'build'], {
        ...
        env: {
          ...
          // Use Turbopack in production for faster builds
          TURBOPACK: '1'
        },
        stdio: 'pipe',
      })
  ...
  async verifyOptimizedBuild() {
    ...
    try {
      ...
      if (hasOptimizedChunks) {
        ...
        console.log('⚠️ Build completed but optimization artifacts are incomplete')
        return false
      }
    } catch (error) {
  ...
  async run() {
    ...
    while (this.attempt <= MAX_RETRIES && !this.buildSuccess) {
      ...
      if (result.success) {
        ...
        if (result.reason === 'activity_timeout' || result.reason === 'overall_timeout') {
          console.log('💡 Build hung during optimization - this indicates a codebase issue')
          console.log('🔧 Possible causes: circular dependencies, heavy computations, or large assets')
        }
      }
    ...
    if (!this.buildSuccess) {
      ...
      console.log('\n🚨 RECOMMENDATION:')
      console.log('The codebase has a fundamental issue preventing optimized builds.')
      console.log('Consider using the emergency bypass temporarily while investigating:')
      console.log('- Check for circular import dependencies')
      console.log('- Review large components that may cause optimization hangs')
    ...
    console.log('🚀 Your Next.js application is fully optimized for production')
  }
}
