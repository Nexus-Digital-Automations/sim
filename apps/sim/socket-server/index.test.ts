...
describe("Socket Server Index Integration", () => {
  ...
  beforeEach(async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(
          new Error(`Server failed to start on port ${PORT} within 15 seconds`),
        );
      }, 15000);
  ...
  describe("Module Integration", () => {
    it.concurrent("should properly import all extracted modules", async () => {
      ...
      const { createSocketIOServer } = await import(
    ...
    it.concurrent(
      "should maintain all original functionality after refactoring",
      () => {
  ...
  describe("Validation and Utils", () => {
    ...
    it.concurrent(
      "should validate block operations with autoConnectEdge",
      async () => {
        const { WorkflowOperationSchema } = await import(
          "@/socket-server/validation/schemas"
        );
        ...
        expect(() =>
          WorkflowOperationSchema.parse(validOperationWithAutoEdge),
        ).not.toThrow();
      },
    );
    ...
    it.concurrent("should validate edge operations", async () => {
      ...
      expect(() =>
        WorkflowOperationSchema.parse(validEdgeOperation),
      ).not.toThrow();
    });
    ...
    it("should validate subflow operations", async () => {
      ...
      expect(() =>
        WorkflowOperationSchema.parse(validSubflowOperation),
      ).not.toThrow();
    });
  });
});
