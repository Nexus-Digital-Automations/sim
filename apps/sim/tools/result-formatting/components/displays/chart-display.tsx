...
import { BarChart3, Download, LineChart, PieChart, Settings, TrendingUp } from 'lucide-react'
...
export function ChartDisplay({ content, onAction, compact = false, className }: ChartDisplayProps) {
  ...
  const stats = useMemo(() => {
    ...
    const numericalFields = Object.keys(content.data[0]).filter(
      (key) =>
        key !== 'x' && key !== 'label' && content.data.some((item) => typeof item[key] === 'number')
    )
  ...
  return (
    <div className={cn('w-full space-y-4', className)}>
      ...
      <div className='flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center'>
        <div>
          {content.title && (
            <h3 className='flex items-center gap-2 font-semibold text-gray-900 text-lg dark:text-gray-100'>
      ...
      {stats && !compact && (
        <div className='grid grid-cols-2 gap-4 sm:grid-cols-4'>
          <Card className='p-4'>
            <div className='font-bold text-2xl text-blue-600 dark:text-blue-400'>{stats.count}</div>
            <div className='text-gray-500 text-xs dark:text-gray-400'>Data Points</div>
          </Card>
          ...
          <Card className='p-4'>
            ...
            <div className='text-gray-500 text-xs dark:text-gray-400'>Maximum</div>
          </Card>
          ...
          <Card className='p-4'>
            ...
            <div className='text-gray-500 text-xs dark:text-gray-400'>Average</div>
          </Card>
          ...
          <Card className='p-4'>
            ...
            <div className='text-gray-500 text-xs dark:text-gray-400'>Total</div>
          </Card>
        </div>
      )}
      ...
      <Card>
        <CardContent className='p-6'>
          <div
            className='flex w-full items-center justify-center rounded-lg bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900'
            style={{
          ...
          >
            ...
            <div className='space-y-4 text-center'>
              <ChartIcon className='mx-auto h-16 w-16 text-gray-400' />
              <div>
                <div className='font-medium text-gray-700 text-lg dark:text-gray-300'>
                  {selectedChartType.charAt(0).toUpperCase() + selectedChartType.slice(1)} Chart
                </div>
                <div className='text-gray-500 text-sm dark:text-gray-400'>
                  {content.data.length} data points • {content.config.yAxis} vs{' '}
              ...
              {(content.config.yAxis as string[]).map((series, index) => (
                <div key={series} className='flex items-center gap-2'>
                  <div
                    ...
                    style={{
                      backgroundColor:
                        content.config.colors?.[index] || `hsl(${index * 60}, 70%, 50%)`,
                    }}
                  />
          ...
          <CardContent>
            <div className='overflow-x-auto'>
              ...
              {content.data.length > 10 && (
                <div className='py-3 text-center text-gray-500 text-sm dark:text-gray-400'>
      ...
      {stats && !compact && (
        <Card className='border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950'>
          <CardHeader className='pb-3'>
            <CardTitle className='text-blue-900 text-sm dark:text-blue-100'>
              Data Insights
            </CardTitle>
          </CardHeader>
