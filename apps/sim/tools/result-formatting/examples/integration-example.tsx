...
import { FormattedResultDisplay, formatToolResult } from '../index'
...
export function ResultFormattingIntegrationExample() {
  ...
  const executeExampleTool = useCallback(async (toolType: string) => {
      ...
      analytics: {
        ...
        result: {
          ...
          output: {
            ...
            summary:
              'Sales performance showing strong growth trajectory with increasing profit margins.',
          },
          ...
          output: {
            results: [
              ...
              },
              {
                ...
                description:
                  'Advanced fitness tracking with heart rate monitoring, GPS, and waterproof design.',
                price: 299.99,
                category: 'Wearables',
                rating: 4.7,
    ...
    try {
      ...
      const formatted = await formatToolResult(example.result, example.config, {
      ...
      setResults((prev) =>
        prev.map((r) =>
          r.id === toolId ? { ...r, formattedResult: formatted, isFormatting: false } : r
        )
      )
    } catch (error) {
      ...
      setResults((prev) =>
        prev.map((r) =>
          r.id === toolId ? { ...r, error: (error as Error).message, isFormatting: false } : r
        )
      )
    }
  }, [])
      ...
      <Card>
        <CardHeader>
          ...
          <p className='text-gray-600 text-sm dark:text-gray-400'>
            This example demonstrates how to integrate the Universal Tool Adapter Result Formatting
            System with the Sim platform. Click the buttons below to simulate different tool
            executions and see how their results are automatically formatted for optimal
        ...
        {results.map((result) => (
          <div key={result.id}>
            ...
            <div className='mb-3 flex items-center gap-2'>
              ...
              {result.isFormatting && <Badge variant='outline'>Formatting...</Badge>}
              {result.error && <Badge variant='destructive'>Format Error</Badge>}
      ...
      <Card className='border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950'>
        ...
        <CardContent>
          <div className='grid grid-cols-1 gap-4 text-sm sm:grid-cols-2'>
            ...
            <div>
              <div className='mb-2 font-medium text-blue-800 dark:text-blue-200'>Key Features</div>
              <ul className='space-y-1 text-blue-700 dark:text-blue-300'>
