...
export function getToolParametersConfig(
  ...
  const allParameters: ToolParameterConfig[] = Object.entries(
    toolConfig.params,
  ).map(([paramId, param]) => {
    ...
    if (blockConfig) {
      ...
      if (!subBlock) {
        subBlock = blockConfig.subBlocks?.find(
          (sb: SubBlockConfig) => sb.id === paramId,
        );
      }
  ...
  const userInputParameters = allParameters.filter(
    (param) =>
      param.visibility === "user-or-llm" || param.visibility === "user-only",
  );
...
export function createLLMToolSchema(
  ...
  Object.entries(toolConfig.params).forEach(([paramId, param]) => {
    ...
    if (
      (param.visibility === "user-or-llm" || param.visibility === "llm-only") &&
      param.required
    ) {
...
export function isPasswordParameter(paramId: string): boolean {
  ...
  return passwordFields.some((field) =>
    paramId.toLowerCase().includes(field.toLowerCase()),
  );
}
