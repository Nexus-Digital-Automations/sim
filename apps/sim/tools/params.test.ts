...
describe("Tool Parameters Utils", () => {
  ...
  describe("createLLMToolSchema", () => {
    it.concurrent(
      "should create schema excluding user-provided parameters",
      () => {
    ...
    it.concurrent(
      "should include all parameters when none are user-provided",
      () => {
  ...
  describe("mergeToolParameters", () => {
    it.concurrent(
      "should merge parameters with user-provided taking precedence",
      () => {
        const userProvided = {
          apiKey: "user-key",
          channel: "#general",
  ...
  describe("validateToolParameters", () => {
    it.concurrent(
      "should validate successfully with all required parameters",
      () => {
        const finalParams = {
          apiKey: "test-key",
          message: "Hello world",
    ...
    it.concurrent(
      "should fail validation with missing required parameters",
      () => {
        const finalParams = {
          channel: "#general",
        };
  ...
  describe("filterSchemaForLLM", () => {
    it.concurrent(
      "should filter out user-provided parameters from schema",
      () => {
  ...
  describe("Type Interface Validation", () => {
    ...
    it.concurrent("should have properly typed ValidationResult", () => {
      const result: ValidationResult = validateToolParameters(
        mockToolConfig,
        {},
      );
      ...
      expect(
        result.missingParams.every((param) => typeof param === "string"),
      ).toBe(true);
    });
    ...
    it.concurrent("should have properly typed ToolParameterConfig", () => {
      ...
      if (config) {
        config.allParameters.forEach((param: ToolParameterConfig) => {
          ...
          expect(
            ["user-or-llm", "user-only", "llm-only", "hidden"].includes(
              param.visibility!,
            ),
          ).toBe(true);
