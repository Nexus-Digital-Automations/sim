...
export function useAgentSelection({
  ...
  const loadPersistedState = useCallback(() => {
    ...
    try {
      ...
      if (lastSelectionData) {
        try {
          newState.lastSelectionTime = Number.parseInt(lastSelectionData, 10)
        } catch (error) {
  ...
  const saveState = useCallback(
    (stateToSave: AgentSelectionState) => {
      if (!persistToStorage || typeof window === 'undefined') return
  ...
  const getRecentAgents = useCallback(() => {
    return state.recentAgents.filter(
      (agent) => !state.selectedAgent || agent.id !== state.selectedAgent.id
    )
  ...
  const updateAgent = useCallback((updatedAgent: Agent) => {
    setState((prevState) => {
      ...
      return {
        ...prevState,
        selectedAgent:
          prevState.selectedAgent?.id === updatedAgent.id ? updatedAgent : prevState.selectedAgent,
