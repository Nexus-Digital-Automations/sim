...
export function CategoryFilter({
  ...
  const filteredCategories = useMemo(() => {
    ...
    return categories.filter((category) => {
      // Check main category
      const mainMatch = category.name.toLowerCase().includes(term) ||
   ...
   */
  const getCategoryCount = useCallback((category: TemplateCategory): number => {
   ...
   */
  const isCategorySelected = useCallback((categoryId: string): boolean => {
    if (multiSelect) {
    ...
    return (
      <div key={category.id} className={cn('category-item', indentClass)}>
        ...
        <div className='flex items-center gap-2'>
          ...
          >
            {hasSubcategories ? (
              isExpanded ? <FolderOpen className='h-3 w-3' style={{ color: category.color }} /> 
  ...
  getTotalTemplateCount(category: TemplateCategory): number {
    ...
    if (category.subcategories) {
      count += category.subcategories.reduce(
        (sum, sub) => sum + this.getTotalTemplateCount(sub),
        0
      )
    }
