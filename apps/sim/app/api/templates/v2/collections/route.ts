...
const CreateCollectionSchema = z.object({
  ...
  tags: z.array(z.string().max(50)).max(20).default([]).describe('Collection tags'),
  templateIds: z.array(z.string().uuid()).max(100).default([]).describe('Initial template IDs'),
})
...
const CollectionQuerySchema = z.object({
  ...
  limit: z.coerce.number().min(1).max(100).default(20).describe('Results per page'),
  sort: z
  ...
  visibility: z.enum(['public', 'private', 'all']).default('all').describe('Visibility filter'),
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let query = db
      ...
      .leftJoin(creatorAlias, eq(templateCollections.createdByUserId, creatorAlias.id))
    ...
    if (validatedParams.userId) {
      filters.push(eq(templateCollections.createdByUserId, validatedParams.userId))
    }
    ...
    const [collectionsData, countData] = await Promise.all([
      query.limit(validatedParams.limit).offset((validatedParams.page - 1) * validatedParams.limit),
      countQuery,
    ])
...
export async function POST(request: NextRequest) {
  ...
  logger.info(`[${requestId}] POST /collections - Collection creation request`, {
    userAgent: request.headers.get('user-agent')?.substring(0, 100),
  })
  ...
  try {
    ...
    if (validatedData.templateIds.length > 0) {
      ...
      accessibleTemplateIds = templatesQuery
        .filter((template) => template.isPublic || template.createdByUserId === userId)
        .map((template) => template.id)

      if (accessibleTemplateIds.length < validatedData.templateIds.length) {
        const inaccessibleCount = validatedData.templateIds.length - accessibleTemplateIds.length
    ...
    const newCollection = await db.transaction(async (tx) => {
      ...
      if (accessibleTemplateIds.length > 0) {
        const collectionItems = accessibleTemplateIds.map((templateId, index) => ({
    ...
    const response = {
      ...
      meta: {
        ...
        timestamp: new Date().toISOString(),
        templatesAdded: accessibleTemplateIds.length,
        templatesSkipped: validatedData.templateIds.length - accessibleTemplateIds.length,
      },
    }
