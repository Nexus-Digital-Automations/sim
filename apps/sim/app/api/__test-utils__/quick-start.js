...
function displayTemplateOptions() {
  ...
  Object.entries(templates).forEach(([key, template], index) => {
    console.log(
      `  ${colors.bright}${index + 1}.${colors.reset} ${colors.green}${template.name}${colors.reset}`
    )
...
async function getTemplateChoice(rl, args) {
  ...
  while (true) {
    ...
    if (/^[1-5]$/.test(choice)) {
      ...
      return templateKeys[Number.parseInt(choice) - 1]
    }
...
async function getEndpointInfo(rl, args) {
  const endpoint =
    args.endpoint || (await ask(rl, 'Enter the API endpoint path (e.g., /api/users):'))
