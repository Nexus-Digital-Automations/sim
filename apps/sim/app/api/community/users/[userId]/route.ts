...
export async function GET(
  ...
  try {
    ...
    const responseData = {
      id: userData.id,
      name: userData.show_real_name !== false ? userData.name : (userData.display_name || 'Anonymous'),
    ...
    console.log(`[UserProfileAPI] Profile retrieved successfully for user ${userId} in ${executionTime}ms`)
...
export async function PUT(
  ...
  try {
    ...
    if (session.user.id !== userId) {
      console.warn(`[UserProfileAPI] User ${session.user.id} attempted to update profile of ${userId}`)
      return NextResponse.json(
    ...
    const executionTime = Date.now() - startTime
    console.log(`[UserProfileAPI] Profile updated successfully for user ${userId} in ${executionTime}ms`)
...
export async function DELETE(
  ...
  try {
    ...
    if (session.user.id !== userId) {
      console.warn(`[UserProfileAPI] User ${session.user.id} attempted to delete profile of ${userId}`)
      return NextResponse.json(
    ...
    try {
      ...
      const executionTime = Date.now() - startTime
      console.log(`[UserProfileAPI] Profile deleted successfully for user ${userId} in ${executionTime}ms`)
