...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const aggregation = {
      ...
      successRate:
        metrics.length > 0
          ? (metrics.filter((m) => !m.metrics.errorRate || m.metrics.errorRate === 0).length /
              metrics.length) *
            100
          : 0,
      resourceUtilization: {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    await performanceCollector.collectMetrics(validatedData.executionId, validatedData.blockId, {
      executionId: validatedData.executionId,
