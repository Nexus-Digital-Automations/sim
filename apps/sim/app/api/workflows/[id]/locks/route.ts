        ...
        if (!isExpired) {
          conflicts.push({
            ...
            conflictingLock: {
              ...
              isExpired: false,
              remainingMinutes: Math.ceil((existingLock.expiresAt.getTime() - now.getTime()) / (1000 * 60)),
            },
...
async function getWorkflowLocks(
  ...
  try {
    ...
    if (!includeExpired) {
      ...
      whereClause = and(
        eq(workflowElementLocks.workflowId, workflowId),
        // Note: Should use gt (greater than) instead of eq for proper comparison
        // This is a bug in the above conflict check as well
        // gt(workflowElementLocks.expiresAt, new Date())
      )
    }
    ...
    const elementLocks: ElementLock[] = locks.map(lock => {
      ...
      const remainingMinutes = Math.max(0, Math.ceil((lock.expiresAt.getTime() - now.getTime()) / (1000 * 60)))
...
export async function GET(
  ...
  try {
    ...
    logger.info(`[${requestId}] Successfully listed ${totalLocks} locks (${expiredLocks} expired, ${cleanedUpCount} cleaned) in ${elapsed}ms`)
...
export async function POST(
  ...
  try {
    ...
    const elapsed = Date.now() - startTime
    logger.info(`[${requestId}] Created ${createdLocks.length}/${elements.length} locks with ${conflicts.length} conflicts in ${elapsed}ms`)
...
export async function DELETE(
  ...
  try {
    ...
    let whereConditions = [eq(workflowElementLocks.workflowId, workflowId)]
