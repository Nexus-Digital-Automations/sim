...
import { errorTracker, parlantErrorTracker } from '../../../../../../packages/parlant-server/error-tracking'
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const timeWindow = parseInt(url.searchParams.get('window') || '60', 10) // Default 60 minutes
    ...
    if (category) {
      alerts = alerts.filter(alert =>
        alert.errors.some(error => error.category === category)
      )
    }
    ...
    const response = {
      ...
      errors: {
        ...
        summary: {
          ...
          mostActiveCategory: Object.entries(errorStats.byCategory)
            .sort(([, a], [, b]) => b - a)[0]?.[0] || 'none',
          mostActiveService: Object.entries(errorStats.byService)
            .sort(([, a], [, b]) => b - a)[0]?.[0] || 'none'
        }
      },
      meta: {
    ...
    parlantLoggers.monitoring.info('Alerts data accessed', {
    ...
    parlantLoggers.monitoring.error('Alerts API request failed', {
      operation: 'alerts_access',
      duration,
      errorType: 'system',
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    // Log alert action
    parlantLoggers.monitoring.info(`Alert ${actionType}d`, {
    ...
    const response = {
      rules: rules.map(rule => ({
        ...
        actions: {
          ...
          email: rule.actions.email ? { recipientCount: rule.actions.email.recipients.length } : undefined
        },
...
export async function GET_STATS(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const url = new URL(request.url)
    const timeWindow = parseInt(url.searchParams.get('window') || '60', 10)
    ...
    const alertStats = {
      ...
      averageErrorsPerAlert: activeAlerts.length > 0 ?
        activeAlerts.reduce((sum, alert) => sum + alert.errors.length, 0) / activeAlerts.length : 0
    }
    ...
    const response = {
      ...
      alerts: alertStats,
      health: {
        errorRate: errorStats.total > 0 ? ((errorStats.byLevel.error || 0) + (errorStats.byLevel.critical || 0)) / errorStats.total * 100 : 0,
        criticalIssues: (errorStats.byLevel.critical || 0) + alertStats.bySeverity.critical,
        status: alertStats.bySeverity.critical > 0 ? 'critical' :
                alertStats.bySeverity.high > 0 ? 'degraded' : 'healthy'
      },
      meta: {
        requestId,
        timestamp: new Date().toISOString(),
