...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const response = {
      ...
      error: databaseHealth.status === 'rejected' ?
        (databaseHealth.reason instanceof Error ? databaseHealth.reason.message : 'Database check failed') :
        undefined,
      metrics: systemMetrics.status === 'fulfilled' ? {
    ...
    logger.info('Database health check completed', {
      ...
      httpStatus,
      connectionCount: systemMetrics.status === 'fulfilled' ?
        systemMetrics.value.database.connectionCount : 'unknown'
    })
...
export async function GET_WITH_DETAILS(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const response = {
      ...
      diagnostics: {
        lastCheckDuration: databaseHealth.status === 'fulfilled' ? databaseHealth.value.duration : undefined,
