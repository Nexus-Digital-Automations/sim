...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const response = {
      ...
      details: {
        ...
        agents: {
          ...
          summary:
            agentMetrics.status === 'fulfilled'
              ? {
                  totalAgents: agentMetrics.value.length,
                  activeAgents: agentMetrics.value.filter(
                    (agent) =>
                      new Date(agent.metrics.lastActiveAt) > new Date(Date.now() - 60 * 60 * 1000)
                  ).length,
                  averageResponseTime:
                    agentMetrics.value.length > 0
                      ? agentMetrics.value.reduce(
                          (sum, agent) => sum + agent.metrics.averageResponseTime,
                          0
                        ) / agentMetrics.value.length
                      : 0,
                }
              : undefined,
        },
      ...
      error:
        parlantHealth.status === 'rejected'
          ? parlantHealth.reason instanceof Error
            ? parlantHealth.reason.message
            : 'Parlant service check failed'
          : undefined,
    ...
    parlantLoggers.monitoring.info(
      'Parlant service health check completed',
      {
    ...
    parlantLoggers.monitoring.error(
      'Parlant service health check failed',
      {
        operation: 'service_health_check',
        duration,
        errorType: 'system',
...
export async function GET_AGENTS(): Promise<NextResponse> {
  ...
  try {
    ...
    const response = {
      ...
      summary:
        agentMetrics.status === 'fulfilled'
          ? {
              totalAgents: agentMetrics.value.length,
              healthyAgents: agentMetrics.value.filter((agent) => agent.metrics.errorCount === 0)
                .length,
              activeAgents: agentMetrics.value.filter(
                (agent) =>
                  new Date(agent.metrics.lastActiveAt) > new Date(Date.now() - 60 * 60 * 1000)
              ...
              averageResponseTime:
                agentMetrics.value.length > 0
                  ...
                  : 0,
              totalSessions: agentMetrics.value.reduce(
                (sum, agent) => sum + agent.metrics.totalSessions,
                0
              ),
              totalMessages: agentMetrics.value.reduce(
                (sum, agent) => sum + agent.metrics.totalMessages,
                0
              ),
              overallSuccessRate:
                agentMetrics.value.length > 0
                  ? agentMetrics.value.reduce((sum, agent) => sum + agent.metrics.successRate, 0) /
                    agentMetrics.value.length
                  : 100,
            }
          : undefined,
    ...
    if (response.summary) {
      const errorRate =
        response.summary.totalAgents > 0
          ? ((response.summary.totalAgents - response.summary.healthyAgents) /
              response.summary.totalAgents) *
            100
          : 0
    ...
    parlantLoggers.agent.info(
      'Agent health metrics collected',
      {
