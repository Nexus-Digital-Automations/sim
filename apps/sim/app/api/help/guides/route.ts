...
const guideQuerySchema = z.object({
  ...
  limit: z.string().transform((val) => Math.min(Number.parseInt(val) || 10, 50)),
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (
      validatedQuery.workflowState ||
      validatedQuery.userAction ||
      validatedQuery.errorCondition
    ) {
      guides = guides.filter((guide) => {
        return guide.contextTriggers.some((trigger) => {
          if (validatedQuery.workflowState && trigger.condition === validatedQuery.workflowState)
            return true
          if (validatedQuery.userAction && trigger.condition === validatedQuery.userAction)
            return true
          if (validatedQuery.errorCondition && trigger.condition === validatedQuery.errorCondition)
            return true
          return false
        })
      })
...
async function handleProgressUpdate(requestId: string, body: any, userId: string) {
  try {
    ...
    const savedProgress = {
      ...
      completionPercent:
        (validatedProgress.completedSteps.length / getGuideStepCount(validatedProgress.guideId)) *
        100,
    }
...
export type { InteractiveGuide, GuideStep, GuideProgress, ContextTrigger, WorkflowStateCondition }
