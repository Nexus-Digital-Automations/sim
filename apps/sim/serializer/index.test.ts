  ...
  getBlock: (type: string) => {
    ...
    const mockConfigs: Record<string, any> = {
        ...
        tools: {
          ...
          config: {
            ...
            tool: (params: Record<string, any>) =>
              getProviderFromModel(params.model || "gpt-4o"),
          },
        },
        subBlocks: [
      ...
      condition: {
        ...
        subBlocks: [
          { id: "condition", type: "long-input", label: "Condition" },
        ],
        inputs: {
          input: { type: "any" },
        },
...
describe("Serializer", () => {
  ...
  describe("serializeWorkflow", () => {
    it.concurrent("should serialize a minimal workflow correctly", () => {
      ...
      expect(starterBlock?.config.params.description).toBe(
        "This is the starter block",
      );
      ...
      expect(agentBlock?.config.params.model).toBe(
    ...
    it.concurrent("should serialize a conditional workflow correctly", () => {
      ...
      const conditionBlock = serialized.blocks.find(
        (b) => b.id === "condition1",
      );
      ...
      const falsePathConnection = serialized.connections.find(
        (c) =>
          c.source === "condition1" && c.sourceHandle === "condition-false",
      );
    ...
    it.concurrent(
      "should serialize a complex workflow with multiple block types",
      () => {
        ...
        expect(apiBlock?.config.params.url).toBe(
        ...
        const functionBlock = serialized.blocks.find(
          (b) => b.id === "function1",
        );
    ...
    it.concurrent(
      "should serialize agent block with custom tools correctly",
      () => {
  ...
  describe("deserializeWorkflow", () => {
    it.concurrent("should deserialize a serialized workflow correctly", () => {
      ...
      expect(starterBlock.subBlocks.description.value).toBe(
        "This is the starter block",
      );
      ...
      expect(agentBlock.subBlocks.model.value).toBe(
        "claude-3-7-sonnet-20250219",
      );
    ...
    it.concurrent(
      "should deserialize a complex workflow with all block types",
      () => {
        const { blocks, edges, loops } = createComplexWorkflowState();
        const serializer = new Serializer();
        ...
        expect(apiBlock.subBlocks.url.value).toBe(
          "https://api.example.com/data",
        );
    ...
    it.concurrent(
      "should handle serialized workflow with invalid block metadata",
      () => {
        const invalidWorkflow =
          createInvalidSerializedWorkflow() as SerializedWorkflow;
        const serializer = new Serializer();
    ...
    it.concurrent(
      "should handle serialized workflow with missing metadata",
      () => {
        const invalidWorkflow =
          createMissingMetadataWorkflow() as SerializedWorkflow;
        const serializer = new Serializer();
  ...
  describe("round-trip serialization", () => {
    it.concurrent(
      "should preserve all data through serialization and deserialization",
      () => {
        const { blocks, edges, loops } = createComplexWorkflowState();
        const serializer = new Serializer();
        ...
        expect(reserialized.connections.length).toBe(
          serialized.connections.length,
        );

        // Check blocks by ID
        serialized.blocks.forEach((originalBlock) => {
          const reserializedBlock = reserialized.blocks.find(
            (b) => b.id === originalBlock.id,
          );

          expect(reserializedBlock).toBeDefined();
          expect(reserializedBlock?.config.tool).toBe(
            originalBlock.config.tool,
          );
          expect(reserializedBlock?.metadata?.id).toBe(
            originalBlock.metadata?.id,
          );

          // Check params - we only check a subset because some default values might be added
          Object.entries(originalBlock.config.params).forEach(
            ([key, value]) => {
              if (value !== null) {
  ...
  describe("validation during serialization", () => {
    it.concurrent(
      "should throw error for missing user-only required fields",
      () => {
        const serializer = new Serializer();
    ...
    it.concurrent(
      "should not throw error when all user-only required fields are present",
      () => {
        const serializer = new Serializer();
    ...
    it.concurrent(
      "should not validate user-or-llm fields during serialization",
      () => {
        const serializer = new Serializer();
    ...
    it.concurrent("should not validate when validateRequired is false", () => {
      ...
      expect(() => {
        serializer.serializeWorkflow(
          { "test-block": blockWithMissingField },
          [],
          {},
        );
      }).not.toThrow();
    ...
    it.concurrent(
      "should validate multiple user-only fields and report all missing",
      () => {
        const serializer = new Serializer();
    ...
    it.concurrent(
      "should handle blocks with no tool configuration gracefully",
      () => {
        const serializer = new Serializer();
    ...
    it.concurrent(
      "should only validate user-only fields, not user-or-llm fields",
      () => {
        const serializer = new Serializer();
        ...
        expect(() => {
          ...
          "Test Reddit Block is missing required fields: Reddit Account",
        );
      },
    );
  });
  ...
  describe("advanced mode field filtering", () => {
    it.concurrent(
      "should include all fields when block is in advanced mode",
      () => {
        const serializer = new Serializer();
    ...
    it.concurrent(
      "should exclude advanced-only fields when block is in basic mode",
      () => {
        const serializer = new Serializer();
    ...
    it.concurrent(
      "should filter memories field correctly in agent blocks",
      () => {
        const serializer = new Serializer();
    ...
    it.concurrent(
      "should include memories field when agent is in advanced mode",
      () => {
        const serializer = new Serializer();
    ...
    it.concurrent(
      "should handle blocks with no matching subblock config gracefully",
      () => {
        const serializer = new Serializer();
