-- Comprehensive Workflow Monitoring System Database Schema
-- Migration: 0084_monitoring_system_comprehensive.sql
...
-- Workflow Execution Tracking table - enhanced execution tracking
...
-- Workflow Execution Steps table - detailed step tracking
CREATE TABLE IF NOT EXISTS workflow_execution_steps (
    ...
    FOREIGN KEY (execution_id) REFERENCES workflow_executions_monitoring(execution_id) ON DELETE CASCADE
);
...
CREATE INDEX IF NOT EXISTS idx_workflow_executions_monitoring_workflow_id ON workflow_executions_monitoring(workflow_id);
CREATE INDEX IF NOT EXISTS idx_workflow_executions_monitoring_status ON workflow_executions_monitoring(status);
CREATE INDEX IF NOT EXISTS idx_workflow_executions_monitoring_started_at ON workflow_executions_monitoring(started_at);
CREATE INDEX IF NOT EXISTS idx_workflow_executions_monitoring_user_id ON workflow_executions_monitoring(user_id);
...
CREATE INDEX IF NOT EXISTS idx_workflow_execution_steps_block_id ON workflow_execution_steps(block_id);
...
CREATE TRIGGER update_workflow_executions_monitoring_updated_at 
    BEFORE UPDATE ON workflow_executions_monitoring 
    FOR EACH ROW 
    EXECUTE FUNCTION update_updated_at_column();
...
BEGIN
    ...
    DELETE FROM workflow_executions_monitoring 
...
-- Create views for monitoring insights
...
SELECT 
    ...
    w.workspace_id,
    COUNT(wem.id) as total_executions_24h,
    COUNT(CASE WHEN wem.status = 'failed' THEN 1 END) as failed_executions_24h,
    COUNT(CASE WHEN wem.status = 'completed' THEN 1 END) as successful_executions_24h,
    ROUND(
        (COUNT(CASE WHEN wem.status = 'completed' THEN 1 END)::NUMERIC / NULLIF(COUNT(wem.id), 0)) * 100, 
        2
    ) as success_rate_24h,
    AVG(EXTRACT(EPOCH FROM (wem.completed_at - wem.started_at)) * 1000) as avg_duration_ms_24h,
    COUNT(CASE WHEN ai.status = 'active' THEN 1 END) as active_alerts
FROM workflow w
LEFT JOIN workflow_executions_monitoring wem ON w.id = wem.workflow_id 
    AND wem.started_at >= NOW() - INTERVAL '24 hours'
LEFT JOIN alert_instances ai ON w.id = ai.workflow_id 
    AND ai.status = 'active'
GROUP BY w.id, w.name, w.workspace_id
ORDER BY total_executions_24h DESC;

CREATE OR REPLACE VIEW live_executions_dashboard AS
SELECT 
    wem.execution_id,
    wem.workflow_id,
    wem.workflow_name,
    wem.status,
    wem.progress,
    wem.current_block_name,
    wem.current_block_type,
    wem.started_at,
    wem.estimated_completion,
    wem.user_id,
    wem.workspace_id,
    u.email as user_email,
    EXTRACT(EPOCH FROM (NOW() - wem.started_at)) as running_duration_seconds
FROM workflow_executions_monitoring wem
JOIN users u ON wem.user_id = u.id
WHERE wem.status IN ('queued', 'running')
ORDER BY wem.started_at DESC;
...
COMMENT ON TABLE workflow_executions_monitoring IS 'Enhanced tracking of workflow executions with detailed monitoring';
COMMENT ON TABLE workflow_execution_steps IS 'Detailed step-by-step tracking of workflow block execution';
...
COMMENT ON VIEW live_executions_dashboard IS 'Real-time dashboard view of currently running workflow executions';