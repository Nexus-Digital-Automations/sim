...
export function useCollaborativeWorkflow() {
  ...
  useEffect(() => {
        ...
        if (target === "block") {
          switch (operation) {
            ...
            case "update-position": {
              ...
              if (!data.timestamp) {
                logger.warn(
                  "Position update missing timestamp, applying without ordering check",
                  {
                    blockId,
                  },
                );
              ...
              const lastTimestamp =
                lastPositionTimestamps.current.get(blockId) || 0;
            ...
            case "update-advanced-mode":
              workflowStore.setBlockAdvancedMode(
                payload.id,
                payload.advancedMode,
              );
              break;
            case "update-trigger-mode":
              workflowStore.setBlockTriggerMode(
                payload.id,
                payload.triggerMode,
              );
              break;
            case "toggle-handles": {
              ...
              if (
                currentBlock &&
                currentBlock.horizontalHandles !== payload.horizontalHandles
              ) {
            ...
            case "duplicate":
              ...
              if (payload.subBlocks && typeof payload.subBlocks === "object") {
                Object.entries(payload.subBlocks).forEach(
                  ([subblockId, subblock]) => {
            ...
            case "update":
              ...
              if (payload.type === "loop") {
                ...
                if (config.forEachItems !== undefined) {
                  workflowStore.updateLoopCollection(
                    payload.id,
                    config.forEachItems,
                  );
                }
                ...
                if (config.parallelType !== undefined) {
                  workflowStore.updateParallelType(
                    payload.id,
                    config.parallelType,
                  );
                }
          ...
          switch (operation) {
            ...
            case "remove":
              ...
              break;
            case "duplicate":
              variablesStore.duplicateVariable(
                payload.sourceVariableId,
                payload.id,
              );
              break;
          }
    ...
    const handleSubblockUpdate = (data: any) => {
      ...
      logger.info(
        `Received subblock update from user ${userId}: ${blockId}.${subblockId}`,
      );
    ...
    const handleVariableUpdate = (data: any) => {
      ...
      logger.info(
        `Received variable update from user ${userId}: ${variableId}.${field}`,
      );
    ...
    const handleWorkflowReverted = async (data: any) => {
      ...
      if (activeWorkflowId === workflowId) {
        logger.info(
          `Currently active workflow ${workflowId} was reverted, reloading state`,
        );
            ...
            if (workflowData?.state) {
              ...
              try {
                ...
                useWorkflowStore.setState({
                  ...
                  hasActiveWebhook:
                    workflowData.state.hasActiveWebhook || false,
                  deploymentStatuses:
                    workflowData.state.deploymentStatuses || {},
                });

                // Update subblock store with reverted values
                const subblockValues: Record<string, Record<string, any>> = {};
                Object.entries(workflowData.state.blocks || {}).forEach(
                  ([blockId, block]) => {
                ...
                logger.info(
                  `Successfully loaded reverted workflow state for ${workflowId}`,
                );
              } finally {
                isApplyingRemoteChange.current = false;
              }
            } else {
              logger.error("No state found in workflow data after revert", {
                workflowData,
              });
            }
          } else {
            logger.error(
              `Failed to fetch workflow data after revert: ${response.statusText}`,
            );
          }
        } catch (error) {
  ...
  const executeQueuedDebouncedOperation = useCallback(
    (
      ...
      if (isShowingDiff) {
        logger.debug(
          "Skipping debounced socket operation in diff mode:",
          operation,
        );
        return;
      }
  ...
  const collaborativeAddBlock = useCallback(
    (
      ...
      if (!isInActiveRoom()) {
        logger.debug(
          "Skipping collaborative add block - not in active workflow",
          {
            currentWorkflowId,
            activeWorkflowId,
          },
  ...
  const collaborativeRemoveBlock = useCallback(
    (id: string) => {
      ...
      executeQueuedOperation("remove", "block", { id }, () =>
  ...
  const collaborativeUpdateBlockPosition = useCallback(
    (id: string, position: Position) => {
      executeQueuedDebouncedOperation(
        "update-position",
        "block",
        { id, position },
  ...
  const collaborativeAddEdge = useCallback(
    (edge: Edge) => {
      executeQueuedOperation("add", "edge", edge, () =>
      ...
      try {
        ...
        if (blockConfig?.subBlocks && Array.isArray(blockConfig.subBlocks)) {
          const dependents = blockConfig.subBlocks.filter(
            (sb: any) =>
              Array.isArray(sb.dependsOn) && sb.dependsOn.includes(subblockId),
          );
    ...
    (sourceId: string) => {
      ...
      const duplicatedBlockData = {
        ...
        outputs: sourceBlock.outputs
          ? JSON.parse(JSON.stringify(sourceBlock.outputs))
          : {},
