...
// Hook configuration
export interface UseBidirectionalSyncConfig
  extends Omit<SyncSystemConfig, "workflowId"> {
  workflowId: string;
  autoStart?: boolean;
...
export function useBidirectionalSync(
  ...
  const [visualState, setVisualState] = useState<WorkflowVisualState>(
    createEmptyVisualState,
  );
  const [chatState, setChatState] =
    useState<ChatWorkflowState>(createEmptyChatState);
  const [health, setHealth] = useState<HealthMetrics | null>(null);
  const [alerts, setAlerts] = useState<Alert[]>([]);
  const [conflicts, setConflicts] = useState<Conflict[]>([]);
  const [currentMode, setCurrentMode] = useState<"visual" | "chat" | "hybrid">(
    "visual",
  );
    ...
    (system: BidirectionalSyncSystem) => {
      ...
      const changeUnsubscribe = system.onChange((change) => {
        if (config.enableRealtimeSync !== false) {
          ...
          if (
            change.path.startsWith("blocks") ||
            change.path.startsWith("edges")
          ) {
            setVisualState(system.getVisualState());
          }

          if (
            change.path.startsWith("messages") ||
            change.path.startsWith("executionState")
          ) {
            setChatState(system.getChatState());
          }
        }
  ...
  const resolveConflict = useCallback(
    (conflictId: string, response: UserPromptResponse) => {
      const system = syncSystemRef.current;
      if (!system) return;
  ...
  const onChange = useCallback(
    (handler: (change: ChangeEvent) => void): (() => void) => {
      const system = syncSystemRef.current;
      if (!system) return () => {};
  ...
  const onHealthUpdate = useCallback(
    (handler: (health: HealthMetrics) => void): (() => void) => {
      const system = syncSystemRef.current;
      if (!system) return () => {};
