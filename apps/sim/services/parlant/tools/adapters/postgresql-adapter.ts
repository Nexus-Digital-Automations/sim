...
export class PostgreSQLAdapter extends UniversalToolAdapter {
  protected transformToParlant(blockConfig: BlockConfig): ParlantTool {
    return {
      ...
      longDescription:
        'Connect to PostgreSQL databases to run queries, insert data, update records, and manage database operations with full SQL support.',
      category: 'data',
      parameters: [
        {
          name: 'query',
  ...
  protected async executeSimTool(
    ...
    try {
      ...
      const result = await this.simulatePostgreSQLQuery(
        simParams.query,
