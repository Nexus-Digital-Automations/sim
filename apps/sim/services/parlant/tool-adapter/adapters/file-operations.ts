...
class GoogleDriveAdapter extends BaseToolAdapter {
      ...
      'Use when you need to upload, download, organize, or search files in Google Drive. Supports file sharing and permission management.',
      {
        ...
        properties: {
          operation: {
            ...
            enum: ['list_files', 'upload_file', 'download_file', 'delete_file', 'create_folder', 'move_file', 'share_file', 'search_files'],
            default: 'list_files'
          },
          file_id: {
            type: 'string',
  ...
  validate(args: any): ValidationResult {
    ...
    if (['download_file', 'delete_file', 'move_file', 'share_file'].includes(args.operation) && !args.file_id) {
  ...
  protected async executeInternal(args: any, context: AdapterContext): Promise<AdapterResult> {
    try {
      logger.info('Executing Google Drive operation', { operation: args.operation, file_id: args.file_id, context })
      ...
      logger.error('Google Drive operation failed', { error: error.message, operation: args.operation })
      return this.createErrorResult(
        'GOOGLE_DRIVE_FAILED',
        error.message,
        'Google Drive operation failed. Please check your configuration.',
        ['Verify Google Drive credentials', 'Check file permissions', 'Verify file exists', 'Check storage quota'],
        true
      )
    }
  }
...
class OneDriveAdapter extends BaseToolAdapter {
      ...
      'Use when you need to upload, download, organize, or search files in OneDrive. Supports file sharing and collaboration.',
      {
        ...
        properties: {
          operation: {
            ...
            enum: ['list_files', 'upload_file', 'download_file', 'delete_file', 'create_folder', 'move_file', 'share_file'],
            default: 'list_files'
          },
          item_id: {
            type: 'string',
  ...
  protected async executeInternal(args: any, context: AdapterContext): Promise<AdapterResult> {
    try {
      logger.info('Executing OneDrive operation', { operation: args.operation, item_id: args.item_id, context })
      ...
      return this.createSuccessResult(
        result,
        `OneDrive ${args.operation} completed successfully`,
        { operation: args.operation, item_id: args.item_id }
      )
    } catch (error: any) {
...
class AWSStorageAdapter extends BaseToolAdapter {
      ...
      'Use when you need to upload, download, or manage files in AWS S3 buckets. Supports bucket operations and object management.',
      {
        ...
        properties: {
          operation: {
            ...
            enum: ['list_objects', 'upload_object', 'download_object', 'delete_object', 'create_bucket', 'list_buckets'],
  ...
  protected async executeInternal(args: any, context: AdapterContext): Promise<AdapterResult> {
    try {
      logger.info('Executing AWS storage operation', { operation: args.operation, bucket: args.bucket_name, context })
      ...
      return this.createSuccessResult(
        result,
        `AWS S3 ${args.operation} completed successfully`,
        { operation: args.operation, bucket: args.bucket_name }
      )
    } catch (error: any) {
      logger.error('AWS storage operation failed', { error: error.message, operation: args.operation })
      return this.createErrorResult(
        'AWS_STORAGE_FAILED',
        error.message,
        'AWS storage operation failed. Please check your configuration.',
        ['Verify AWS credentials', 'Check bucket permissions', 'Verify bucket exists', 'Check object key'],
        true
      )
    }
  }
...
class LocalFileAdapter extends BaseToolAdapter {
      ...
      'Use when you need to read, write, or organize files on the local system. Supports file operations and directory management.',
      {
        ...
        properties: {
          operation: {
            ...
            enum: ['read_file', 'write_file', 'delete_file', 'list_directory', 'create_directory', 'move_file', 'copy_file', 'file_stats'],
  ...
  protected async executeInternal(args: any, context: AdapterContext): Promise<AdapterResult> {
    try {
      logger.info('Executing local file operation', { operation: args.operation, path: args.file_path, context })
      ...
      const result = {
        ...
        data: {
          ...
          ...(args.operation === 'list_directory' && {
            files: [
              { name: 'file1.txt', size: 512, isDirectory: false, lastModified: new Date().toISOString() },
              { name: 'folder1', size: 0, isDirectory: true, lastModified: new Date().toISOString() }
            ],
            count: 2
          }),
      ...
      logger.error('Local file operation failed', { error: error.message, operation: args.operation, path: args.file_path })
      return this.createErrorResult(
        'LOCAL_FILE_FAILED',
        error.message,
        'Local file operation failed. Please check the file path and permissions.',
        ['Verify file path exists', 'Check file permissions', 'Verify disk space', 'Check file locks'],
        true
      )
    }
  }
...
class DocumentProcessorAdapter extends BaseToolAdapter {
      ...
      'Use when you need to extract text from PDFs, Word docs, images, or other document formats. Supports OCR and content analysis.',
      {
        ...
        properties: {
          operation: {
            ...
            enum: ['extract_text', 'extract_images', 'convert_format', 'ocr_image', 'analyze_document'],
  ...
  protected async executeInternal(args: any, context: AdapterContext): Promise<AdapterResult> {
    try {
      logger.info('Executing document processing', { operation: args.operation, file_path: args.file_path, context })
      ...
      logger.error('Document processing failed', { error: error.message, operation: args.operation })
      return this.createErrorResult(
        'DOCUMENT_PROCESSING_FAILED',
        error.message,
        'Document processing failed. Please check the file format and try again.',
        ['Verify file format is supported', 'Check file is not corrupted', 'Try different processing options'],
        true
      )
    }
  }
...
class FileConverterAdapter extends BaseToolAdapter {
  ...
  protected async executeInternal(args: any, context: AdapterContext): Promise<AdapterResult> {
    try {
      ...
      logger.error('File conversion failed', { error: error.message, source_format: args.source_format })
      return this.createErrorResult(
        'FILE_CONVERSION_FAILED',
        error.message,
        'File conversion failed. Please check the file format and try again.',
        ['Verify source file format', 'Check conversion compatibility', 'Try different quality settings'],
        true
      )
    }
  }
...
class BatchFileProcessorAdapter extends BaseToolAdapter {
      ...
      'Use when you need to perform the same operation on multiple files simultaneously. Supports batch upload, download, convert, and process.',
      {
        ...
        properties: {
          operation: {
            ...
            enum: ['batch_upload', 'batch_download', 'batch_convert', 'batch_process', 'batch_delete', 'batch_move']
          },
          files: {
            type: 'array',
            items: { type: 'string' },
  ...
  protected async executeInternal(args: any, context: AdapterContext): Promise<AdapterResult> {
    try {
      logger.info('Executing batch file processing', { operation: args.operation, file_count: args.files.length, context })
      ...
      const result = {
        ...
        execution_summary: {
          ...
          average_processing_time: successful.reduce((sum, file) => sum + file.processing_time_ms, 0) / successful.length,
      ...
      logger.error('Batch file processing failed', { error: error.message, operation: args.operation })
      return this.createErrorResult(
        'BATCH_PROCESSING_FAILED',
        error.message,
        'Batch file processing failed. Please check the configuration.',
        ['Verify file paths are valid', 'Check destination exists', 'Reduce parallel limit', 'Check available resources'],
        true
      )
    }
  }
...
class FileSearchAdapter extends BaseToolAdapter {
  ...
  protected async executeInternal(args: any, context: AdapterContext): Promise<AdapterResult> {
    try {
      logger.info('Executing file search', { search_type: args.search_type, query: args.query, context })
      ...
      const results = [
        {
          ...
          location: 'local',
          match_score: 0.95,
          match_context: args.include_content ? `...found "${args.query}" in document...` : undefined
        },
        {
          ...
          match_score: 0.87,
          match_context: args.include_content ? `...contains relevant content about "${args.query}"...` : undefined
        }
      ]
      ...
      return this.createErrorResult(
        ...
        ['Simplify search query', 'Check search locations exist', 'Try different search type', 'Reduce result limit'],
        true
      )
    }
  }
