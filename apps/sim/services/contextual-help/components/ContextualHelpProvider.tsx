...
export function ContextualHelpProvider({
  ...
  const initializeHelp = useCallback(
    async (userContext: HelpContext) => {
  ...
  const getContextualHelp = useCallback(
    async (deliveryConfig?: HelpDeliveryConfig) => {
  ...
  const generateIntelligentHelp = useCallback(
    async (config: NLHelpContentConfig) => {
      dispatch({ type: 'SET_LOADING', payload: true })
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          dispatch({
            type: 'SET_ERROR',
            payload: response.error?.message || 'Failed to generate help',
          })
        }
      } catch (error) {
        dispatch({ type: 'SET_ERROR', payload: 'Network error occurred' })
  ...
  const startGuidance = useCallback(
    async (toolId: string, tutorialType: 'quick_start' | 'comprehensive' | 'troubleshooting') => {
      if (!state.userContext) return
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          dispatch({
            type: 'SET_ERROR',
            payload: response.error?.message || 'Failed to start guidance',
          })
        }
      } catch (error) {
        dispatch({ type: 'SET_ERROR', payload: 'Network error occurred' })
  ...
  const submitFeedback = useCallback(
    async (feedbackData: Omit<FeedbackData, 'id' | 'status' | 'metadata'>) => {
      if (!state.userContext) return
  ...
  // Adapt help content
  const adaptHelpContent = useCallback(
    async (contentId: string, newContext: HelpContext) => {
      dispatch({ type: 'SET_LOADING', payload: true })
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          dispatch({
            type: 'SET_ERROR',
            payload: response.error?.message || 'Failed to adapt content',
          })
        }
      } catch (error) {
        dispatch({ type: 'SET_ERROR', payload: 'Network error occurred' })
  ...
  return (
    <ContextualHelpContext.Provider value={contextValue}>{children}</ContextualHelpContext.Provider>
  )
}
