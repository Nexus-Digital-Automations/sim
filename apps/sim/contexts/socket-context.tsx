      ...
      try {
        ...
        const rehydrateWorkflowStores = async (
          ...
          if (activeWorkflowId !== workflowId) {
            logger.info(
              `Skipping rehydration - workflow ${workflowId} is not active`,
            );
            return false;
          }
          ...
          const hasPending = useOperationQueueStore
            ...
            .operations.some(
              (op: any) =>
                op.workflowId === workflowId && op.status !== "confirmed",
            );
          if (hasPending) {
            logger.info(
              `Skipping ${source} rehydration due to pending operations in queue`,
            );
            return false;
          }
  ...
  const leaveWorkflow = useCallback(() => {
    if (socket && currentWorkflowId) {
      ...
      try {
        const {
          useOperationQueueStore,
        } = require("@/stores/operation-queue/store");
        useOperationQueueStore
          .getState()
          .cancelOperationsForWorkflow(currentWorkflowId);
      } catch {}
      socket.emit("leave-workflow");
      setCurrentWorkflowId(null);
      setPresenceUsers([]);
  ...
  const emitVariableUpdate = useCallback(
    (variableId: string, field: string, value: any, operationId?: string) => {
      ...
      if (socket && currentWorkflowId) {
        ...
        logger.warn(
          "Cannot emit variable update: no socket connection or workflow room",
          {
            hasSocket: !!socket,
            currentWorkflowId,
            variableId,
