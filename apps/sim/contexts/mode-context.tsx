...
type ModeAction =
  | { type: 'START_TRANSITION'; payload: { fromMode: ViewMode; toMode: ViewMode } }
  ...
  | { type: 'RESTORE_CONTEXT'; payload: { mode: ViewMode; context: ModeContext } }
  | { type: 'UPDATE_PREFERENCES'; payload: { preferences: Partial<ModePreferences> } }
  ...
  switch (action.type) {
    ...
    case 'UPDATE_PREFERENCES': {
      ...
      if (typeof window !== 'undefined') {
        try {
          localStorage.setItem('mode-preferences', JSON.stringify(updatedPreferences))
        } catch (error) {
...
export function ModeProvider({ children }: { children: React.ReactNode }) {
  ...
  const switchMode = useCallback(
    async (mode: ViewMode, config: Partial<ModeConfig> = {}) => {
      if (state.isTransitioning) {
        logger.warn('Mode switch already in progress, ignoring request', { mode })
        return
      }
      ...
      logger.info('Starting mode transition', { fromMode, toMode: mode, config })
      ...
      const transitionEvent: ModeTransitionEvent = {
        ...
        toMode: mode,
        timestamp: Date.now(),
        preserveContext: config.preserveState ?? state.preferences.contextPreservation.visual,
        trigger: 'user',
      }

      // Start transition
      dispatch({ type: 'START_TRANSITION', payload: { fromMode, toMode: mode } })
      ...
      function completeTransition(targetMode: ViewMode) {
        ...
        dispatch({ type: 'COMPLETE_TRANSITION', payload: { mode: targetMode } })
        logger.info('Mode transition completed', { mode: targetMode })
      }
    },
    [state.currentMode, state.isTransitioning, state.preferences, state.context]
  )
  ...
  // Event handling
  const onModeChange = useCallback((callback: (event: ModeTransitionEvent) => void) => {
