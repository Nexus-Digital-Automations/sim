...
export function ChatSocketProvider({ children, user }: ChatSocketProviderProps) {
  ...
  const leaveChatSession = useCallback(() => {
    if (socket && currentSession) {
      ...
      socket.emit('chat:leave-session', { sessionId: currentSession.sessionId })
  ...
  const sendTyping = useCallback(
    (isTyping: boolean) => {
      if (socket && currentSession) {
        socket.emit('chat:typing', { sessionId: currentSession.sessionId, isTyping })
  ...
  const updatePresence = useCallback(
    (status: 'active' | 'idle' | 'away') => {
      if (socket && currentSession) {
        socket.emit('chat:update-presence', { sessionId: currentSession.sessionId, status })
