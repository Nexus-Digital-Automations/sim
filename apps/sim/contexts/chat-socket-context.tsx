...
export function ChatSocketProvider({
  ...
  const eventHandlers = useRef<{
    ...
    agentStreamChunk?: (data: {
      messageId: string;
      chunk: string;
      isComplete: boolean;
    }) => void;
    sessionStatusChanged?: (data: {
      sessionId: string;
      status: string;
    }) => void;
  ...
  useEffect(() => {
    ...
    if (initializedRef.current || socket || isConnecting) {
      logger.info(
        "Chat socket already exists or is connecting, skipping initialization",
      );
      return;
    }
      ...
      try {
        ...
        const socketInstance = io(socketUrl, {
          ...
          auth: async (cb) => {
            try {
              ...
              logger.error(
                "Failed to generate fresh token for chat connection:",
                error,
              );
              cb({ token: null });
            }
        ...
        socketInstance.on(
          ...
          (indicator: TypingIndicator) => {
            ...
            if (indicator.userId) {
              setCurrentSession((prev) => {
                if (!prev || prev.sessionId !== indicator.sessionId)
                  return prev;
                return {
                  ...prev,
                  isTyping: {
        ...
        socketInstance.on("chat:user-joined", (presence: ChatPresence) => {
          ...
          setCurrentSession((prev) => {
            ...
            const existingUser = prev.participants.find(
            ...
            setCurrentSession((prev) => {
              ...
              return {
                ...
                messageHistory:
                  data.offset === 0
                    ? data.messages
                    : [...data.messages, ...prev.messageHistory],
              };
            });
  ...
  const requestHistory = useCallback(
    (limit = 50, offset = 0) => {
      if (socket && currentSession) {
        ...
        logger.debug(
          `Requested chat history: limit=${limit}, offset=${offset}`,
        );
      }
    },
    [socket, currentSession],
  ...
  const onTypingIndicator = useCallback(
    (handler: (indicator: TypingIndicator) => void) => {
  ...
  const onPresenceUpdated = useCallback(
    (handler: (presence: ChatPresence) => void) => {
