...
async function executeWebhookJobInternal(
  ...
  try {
    ...
    const workflowData = await loadWorkflowFromNormalizedTables(
      payload.workflowId,
    );
    if (!workflowData) {
    ...
    const { personalEncrypted, workspaceEncrypted } =
      await getPersonalAndWorkspaceEnv(payload.userId, workspaceId);
    ...
    const decryptedEnvVars: Record<string, string> =
      Object.fromEntries(decryptedPairs);
    ...
    const input = formatWebhookInput(
      mockWebhook,
      mockWorkflow,
      payload.body,
      mockRequest,
