...
export const logsWebhookDelivery = task({
  ...
  run: async (params: {
    ...
    try {
      ...
      if (claimed.length === 0) {
        logger.info(
          `Delivery ${deliveryId} not claimable (already in progress or not due)`,
        );
        return;
      }
      ...
      if (
        (subscription.includeRateLimits || subscription.includeUsageData) &&
        log.executionData
      ) {
        const executionData = log.executionData as any;

        const needsRateLimits =
          subscription.includeRateLimits && executionData.includeRateLimits;
        const needsUsage =
          subscription.includeUsageData && executionData.includeUsageData;
        if (needsRateLimits || needsUsage) {
          ...
          if (workflow.length > 0) {
            try {
              ...
              logger.warn("Failed to fetch limits/usage for webhook", {
                error,
              });
            }
          }
      ...
      logger.info(
        `Attempting webhook delivery ${deliveryId} (attempt ${attempts})`,
        {
      ...
      logger.error(
        `Webhook delivery ${deliveryId} encountered unexpected error`,
        {
          error: error.message,
          stack: error.stack,
        },
