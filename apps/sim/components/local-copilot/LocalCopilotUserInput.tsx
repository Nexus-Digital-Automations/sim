...
import { Code, FileText, Image, Loader2, Paperclip, Send, Square, X } from 'lucide-react'
...
const ContextPreview: React.FC<ContextPreviewProps> = ({ context, onRemove }) => {
  return (
    <div className='group flex items-center gap-2 rounded-md border bg-muted/50 p-2'>
      ...
      <span className='min-w-0 flex-1 truncate text-muted-foreground text-xs'>{context.label}</span>
      <Button
        variant='ghost'
        size='sm'
        onClick={onRemove}
...
export const LocalCopilotUserInput = forwardRef<
  LocalCopilotUserInputRef,
  LocalCopilotUserInputProps
>(
  (
    {
      onSubmit,
          ...
          <form onSubmit={handleSubmit}>
            <div
              className={cn(
                ...
                isDragOver && 'border-primary/20 bg-primary/5'
              )}
            ...
            >
              {isDragOver && (
                <div className='absolute inset-0 z-10 flex items-center justify-center rounded-lg border-2 border-primary/30 border-dashed bg-primary/5'>
                  <div className='font-medium text-primary text-sm'>Drop files here to attach</div>
                </div>
              )}
