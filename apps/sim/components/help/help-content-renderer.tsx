...
class MarkdownProcessor {
  static process(content: string): string {
    ...
    return content
      ...
      .replace(
        /^## (.*$)/gm,
        '<h2 class="text-2xl font-semibold mt-6 mb-3 text-foreground">$1</h2>'
      )
...
const ImageViewer: React.FC<{
  ...
  return (
    <div className={cn('group relative', className)}>
      <div className='relative overflow-hidden rounded-lg bg-muted'>
        ...
        {enableZoom && isLoaded && !hasError && (
          <div className='absolute top-2 right-2 opacity-0 transition-opacity group-hover:opacity-100'>
            <Badge variant='secondary' className='bg-black/60 text-white'>
              <ZoomInIcon className='mr-1 h-3 w-3' />
              Click to zoom
            </Badge>
          </div>
        )}
      ...
      {asset.caption && (
        <p className='mt-2 text-center text-muted-foreground text-sm'>{asset.caption}</p>
      )}
    </div>
  )
}
    ...
    <div className={cn('group relative', className)}>
      <div className='rounded-lg bg-muted'>
        <div className='flex items-center justify-between border-border border-b px-4 py-2'>
          <span className='font-mono text-muted-foreground text-sm'>{language || 'code'}</span>
          {showCopy && (
            <Button
              variant='ghost'
              size='sm'
    ...
    return (
      <div className='mb-6 space-y-4'>
        {parsedContent.metadata.mediaAssets.map((asset) => (
          <div key={asset.id}>
            {asset.type === 'video' && (
              <VideoPlayer asset={asset} onPlay={onMediaPlay} className='mb-2' />
            )}
            {asset.type === 'image' && (
              <ImageViewer asset={asset} enableZoom={enableImageZoom} className='mb-2' />
            )}
          </div>
        ))}
      </div>
    ...
    <div className={cn('help-content-renderer', className)} style={{ maxWidth }}>
      <div className='flex gap-6'>
        ...
        <div className='flex-1'>
          ...
          {showMetadata && (
            <div className='mb-6'>
              <h1 className='mb-2 font-bold text-2xl'>{parsedContent.title}</h1>
              ...
              <div className='mb-4 flex items-center gap-4 text-muted-foreground text-sm'>
                ...
                {showLastUpdated && parsedContent.updatedAt && (
                  <span>Updated {new Date(parsedContent.updatedAt).toLocaleDateString()}</span>
                )}
                <Badge variant='secondary'>{parsedContent.metadata?.category}</Badge>
              </div>

              {parsedContent.metadata?.description && (
                <p className='mb-4 text-muted-foreground'>{parsedContent.metadata.description}</p>
              )}
