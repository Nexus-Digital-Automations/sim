...
try {
    ...
    try {
      if (fs.existsSync(filePath)) {
        ...
        if (file.endsWith('.tsx') || file.endsWith('.ts')) {
          ...
          if (file.endsWith('.tsx') && !content.includes('import type React') && !content.includes("import React")) {
  ...
  validationResults.forEach(result => {
    ...
    console.log(`${status} ${result.file.padEnd(30)} ${result.status.padEnd(8)} (${result.lines} lines, ${Math.round(result.size/1024)}KB)`);
