      ...
      <CardContent className="relative z-20 p-8 text-white">
        <div className="flex items-start gap-6">
          ...
          <div className="flex-1">
            ...
            <p className="mb-4 text-white/90 text-lg leading-relaxed">
              {featuredTemplate.description}
            </p>
            <div className="mb-6 flex items-center gap-4 text-white/80 text-sm">
              ...
              <div className="flex items-center gap-1">
                ...
                <span>
                  {featuredTemplate.ratingAverage?.toFixed(1) || "N/A"}
                </span>
              </div>
            </div>
...
const TemplateCarousel: React.FC<{
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentIndex(
        (current) => (current + 1) % Math.max(templates.length - 2, 1),
      );
    }, 5000);
  ...
  const handleNext = useCallback(() => {
    ...
    setCurrentIndex((current) =>
      Math.min(current + 1, Math.max(templates.length - 3, 0)),
    );
  }, [templates.length]);
              ...
              >
                ...
                >
                  <CardContent className="p-4">
                    ...
                    <p className="line-clamp-2 mb-3 text-muted-foreground text-sm">
      ...
      <CardContent className="p-4">
        ...
        <div className="mb-4 space-y-2">
          {templates.slice(0, 3).map((template) => (
            <div
              key={template.id}
              className="flex items-center gap-3 cursor-pointer rounded p-2 transition-colors hover:bg-gray-50"
              onClick={() => onTemplateSelect?.(template)}
            >
              <div
      ...
      <CardContent className="p-4">
        <div className="mb-4 flex items-center gap-3">
          <div className="h-12 w-12 rounded-full bg-gradient-to-r from-blue-400 to-purple-400 p-0.5">
            <div className="flex h-full w-full items-center justify-center rounded-full bg-white text-lg font-bold">
        ...
        <div className="mb-4 space-y-2">
          {creator.templates.slice(0, 2).map((template) => (
            <div
              key={template.id}
              className="flex items-center gap-2 cursor-pointer rounded p-2 transition-colors hover:bg-gray-50"
              onClick={() => onTemplateSelect?.(template)}
            >
              <div
                className="flex h-6 w-6 items-center justify-center rounded text-xs font-bold text-white"
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              {content.creatorHighlights.map((creator) => (
                <CreatorHighlight
                  ...
                  creator={creator}
                  onTemplateSelect={onTemplateSelect}
                  onCreatorProfile={(author) =>
                    console.log("View creator profile:", author)
                  }
                />
              ))}
            </div>
