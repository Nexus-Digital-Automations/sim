...
import type { Template, TemplateCategory, TemplateRecommendation } from '@/lib/templates/types'
...
interface RecommendationEngineState {
  ...
  userFeedback: Record<string, { strategy: RecommendationStrategy; feedback: string }>
}
...
const useRecommendationEngine = (
  ...
  const [state, setState] = useState<RecommendationEngineState>({
    recommendations: {} as Record<RecommendationStrategy, TemplateRecommendation[]>,
    loading: false,
    error: null,
    lastUpdated: null,
    userFeedback: {},
  ...
  const generateRecommendations = useCallback(
    async (strategy: RecommendationStrategy): Promise<TemplateRecommendation[]> => {
      ...
      switch (strategy) {
        case 'personalized':
          ...
          recommendations = availableTemplates
            .filter((template) => {
              ...
              if (context.userPreferences?.categories?.includes(template.category)) return true
              // Prefer templates with user's preferred difficulty
              if (
                context.userPreferences?.difficulty?.includes(template.metadata?.difficulty || '')
              )
                return true
        ...
        case 'similar': {
          ...
          const referenceTemplate = context.currentTemplate || context.recentlyViewed?.[0]
        ...
        case 'completion': {
          ...
          const installedCategories = context.installedTemplates?.map((t) => t.category) || []
          recommendations = availableTemplates
            .filter((template) => {
              ...
              return (
                ...
                template.metadata?.tags?.some((tag) =>
                  ['integration', 'data', 'notification', 'automation'].includes(tag.toLowerCase())
                )
              )
            })
            .slice(0, 6)
  ...
  const generateAllRecommendations = useCallback(async () => {
    ...
    try {
      const allRecommendations: Record<RecommendationStrategy, TemplateRecommendation[]> = {} as any
...
const RecommendationSection: React.FC<{
  ...
  onTemplateSelect?: (template: Template) => void
  onTemplateInstall?: (template: Template) => void
  onFeedback?: (templateId: string, feedback: 'positive' | 'negative' | 'not_interested') => void
}> = ({
  title,
  description,
  icon,
        ...
        <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3'>
          {recommendations.map((recommendation) => {
            ...
            return (
              ...
              >
                <Card className='h-full cursor-pointer transition-all duration-200 hover:shadow-blue-500/10 hover:shadow-lg'>
...
export const TemplateRecommendationEngine: React.FC<TemplateRecommendationEngineProps> = ({
  ...
  const { recommendations, loading, error, lastUpdated, refreshRecommendations, provideFeedback } =
    useRecommendationEngine(context, availableTemplates, categories, enabledStrategies)
  ...
  if (error) {
    return (
      <Card className={className}>
        <CardContent className='flex items-center justify-center p-8'>
          <div className='text-center'>
            <p className='mb-2 font-medium text-red-600'>Failed to load recommendations</p>
    ...
    <div className={cn('space-y-6', className)}>
      ...
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-muted-foreground'>Personalized suggestions based on AI analysis</p>
        </div>
        <div className='flex items-center gap-2'>
          ...
          <Button size='sm' variant='outline' onClick={refreshRecommendations} disabled={loading}>
        ...
        {enabledStrategies.map((strategy) => (
          <TabsContent key={strategy} value={strategy}>
            <RecommendationSection
              ...
              icon={strategyConfig[strategy]?.icon || <Sparkles className='h-5 w-5' />}
              recommendations={recommendations[strategy]?.slice(0, maxRecommendations) || []}
              strategy={strategy}
              onTemplateSelect={onTemplateSelect}
              onTemplateInstall={onTemplateInstall}
              onFeedback={(templateId, feedback) => handleFeedback(templateId, strategy, feedback)}
            />
          </TabsContent>
        ))}
      </Tabs>
