...
const WorkflowEditor = dynamic(
  () => import('@/app/workspace/[workspaceId]/w/[workflowId]/workflow'),
  {
...
const ChatInterface = dynamic(
  () =>
    import('@/app/chat/workspace/[workspaceId]/agent/[agentId]/components/parlant-chat-interface'),
  {
    ...
    <div className='h-full w-full animate-pulse'>
      <div className='flex h-full'>
        ...
        <div className='flex-1 bg-background'>
          ...
          {/* Node skeletons */}
          <div className='absolute top-20 left-20 h-24 w-80 rounded-lg bg-muted/50' />
          <div className='absolute top-60 left-20 h-24 w-80 rounded-lg bg-muted/50' />
          <div className='absolute top-40 right-20 h-32 w-96 rounded-lg bg-muted/50' />
        </div>
...
function ModeNavigationBar({
  ...
  const { mode, switchToVisual, switchToChat, switchToHybrid, canSwitchMode, isTransitioning } =
    useModeSwitch()
  ...
  return (
    <div
      className={cn(
        '-translate-x-1/2 fixed top-4 left-1/2 z-40 flex items-center rounded-lg bg-background/95 shadow-lg ring-1 ring-border backdrop-blur-sm',
        className
      )}
    >
        ...
        {navigationItems.map((item) => (
          <button
            ...
            className={cn(
              ...
              (!item.available || !canSwitchMode(item.mode)) && 'cursor-not-allowed opacity-50',
...
export function ModeSwitcher({
  ...
  const renderModeContent = () => {
    switch (mode) {
      ...
      case 'hybrid':
        return <HybridMode visualComponent={visualComponent} chatComponent={chatComponent} />
  ...
  return (
    <ModeShortcutsProvider>
      <div className={cn('relative h-full w-full', className)}>
        {/* Mode navigation bar */}
        <ModeNavigationBar workspaceId={workspaceId} workflowId={workflowId} agentId={agentId} />

        {/* Main content with transitions */}
        <ModeTransition>{renderModeContent()}</ModeTransition>
        ...
        <div className='sr-only' aria-live='polite' aria-atomic='true'>
          {isTransitioning ? `Switching to ${mode} mode...` : `Currently in ${mode} mode`}
        </div>
      </div>
    </ModeShortcutsProvider>
  )
