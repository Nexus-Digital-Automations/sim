...
export const MockParlantProvider: React.FC<MockParlantProviderProps> = ({
  ...
  const simulateDelay = useCallback(async (customDelay?: number) => {
  ...
  const createAgent = useCallback(async (config: Partial<ParlantAgent>): Promise<ParlantAgent> => {
    try {
      setAgentLoading(true)
      setError(null)
      simulateError()
  ...
  // Session Management
  const createSession = useCallback(async (agentId: string): Promise<ChatSession> => {
    try {
      setIsLoading(true)
      setError(null)
      simulateError()
  ...
  const sendMessage = useCallback(async (content: string, type: 'text' | 'image' | 'file' = 'text') => {
    try {
      ...
      if (simulateRealTime && currentAgent) {
        setTimeout(async () => {
          await simulateAgentResponse(`Thank you for your message: "${content}". How else can I help?`)
  ...
  const simulateAgentResponse = useCallback(async (message: string, delay: number = 1000) => {
  ...
  return (
    <MockParlantContext.Provider value={contextValue}>
      {children}
    </MockParlantContext.Provider>
  )
}
