...
class ConcurrentTestHelper {
  ...
  static async raceOperations<T>(operations: Array<() => Promise<T>>): Promise<T[]> {
    return await Promise.allSettled(operations.map((op) => op())).then((results) =>
...
describe('Concurrent Access and Transaction Tests', () => {
  ...
  const testSessions: ParlantSession[] = []
  ...
  describe('Transaction Isolation and Consistency', () => {
    it('should maintain transaction consistency during concurrent operations', async () => {
      ...
      const allSessions = await db.select().from(parlantSessions).where(sql`agent_id = ${agentId}`)
      const allEvents = await db.select().from(parlantEvents).where(sql`agent_id = ${agentId}`)
    ...
    it('should handle transaction rollbacks properly during failures', async () => {
      ...
      const dbSessions = await db.select().from(parlantSessions).where(sql`agent_id = ${agentId}`)
      const dbEvents = await db.select().from(parlantEvents).where(sql`agent_id = ${agentId}`)
  ...
  describe('Race Condition Prevention', () => {
    it('should prevent race conditions in agent status updates', async () => {
      ...
      const statusUpdates = Array.from({ length: 50 }, (_, index) => {
        return async () => {
          const newStatus = index % 2 === 0

          const updated = await db
            .update(parlantAgents)
            .set({
              enabled: newStatus,
              updatedAt: new Date(),
            })
      ...
      const finalAgent = await db.select().from(parlantAgents).where(sql`id = ${agentId}`).limit(1)
