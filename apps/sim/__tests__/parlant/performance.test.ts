...
describe('Parlant Database Performance Tests', () => {
  ...
  afterEach(async () => {
    ...
    ctx.metrics.forEach((metric) => {
      console.log(
        `${metric.operation}: ${metric.duration}ms (${metric.recordCount} records, ${metric.throughputPerSecond.toFixed(2)} ops/sec)`
      )
    })
  })
  ...
  describe('Bulk Insert Performance', () => {
    it('should handle bulk agent creation efficiently', async () => {
      ...
      const result = await measurePerformance('Bulk Agent Insert', BULK_INSERT_SIZE, () =>
    ...
    it('should handle bulk session and event creation', async () => {
      ...
      const sessionResults = await measurePerformance('Bulk Session Insert', BULK_INSERT_SIZE, () =>
      ...
      const eventResults = await measurePerformance('Bulk Event Insert', events.length, () =>
        db.insert(parlantEvent).values(events).returning()
      )

      expect(eventResults).toHaveLength(1000) // 100 sessions * 10 events each
      expect(
        ctx.metrics.find((m) => m.operation === 'Bulk Event Insert')?.throughputPerSecond
      ).toBeGreaterThan(200)
    })
    ...
    it('should handle bulk tool and integration creation', async () => {
      ...
      const toolResults = await measurePerformance('Bulk Tool Insert', BULK_INSERT_SIZE, () =>
  ...
  describe('Complex Query Performance', () => {
    it('should efficiently execute complex analytical queries', async () => {
      ...
      const dashboardQuery = await measurePerformance('Agent Performance Dashboard Query', 1, () =>
        db
          .select({
      ...
      expect(
        ctx.metrics.find((m) => m.operation === 'Agent Performance Dashboard Query')?.duration
      ).toBeLessThan(1000)

      // Complex analytical query 2: Session trends over time
      const trendsQuery = await measurePerformance('Session Trends Analysis', 1, () =>
        db
          .select({
          ...
          .groupBy(sql`DATE(${parlantSession.lastActivityAt})`, parlantSession.sessionType)
          .orderBy(desc(sql`DATE(${parlantSession.lastActivityAt})`), parlantSession.sessionType)
      )

      expect(trendsQuery.length).toBeGreaterThan(0)

      // Complex analytical query 3: Customer journey analysis
      const journeyQuery = await measurePerformance('Customer Journey Analysis', 1, () =>
        db
          .select({
      ...
      expect(
        ctx.metrics.find((m) => m.operation === 'Customer Journey Analysis')?.duration
      ).toBeLessThan(2000)
    })
    ...
    it('should efficiently handle pagination and filtering', async () => {
      ...
      const filteredQuery = await measurePerformance('Filtered Query Performance', 1, () =>
        db
          .select({
            id: parlantSession.id,
            title: parlantSession.title,
      ...
      expect(
        ctx.metrics.find((m) => m.operation === 'Filtered Query Performance')?.duration
      ).toBeLessThan(500)

      // Test search-like queries
      const searchQuery = await measurePerformance('Text Search Performance', 1, () =>
        db
          .select()
          .from(parlantSession)
          .innerJoin(parlantAgent, eq(parlantSession.agentId, parlantAgent.id))
  ...
  describe('Concurrent Access Performance', () => {
    it('should handle concurrent session creation efficiently', async () => {
      ...
      const concurrentOperations = Array.from({ length: CONCURRENT_OPERATIONS }, (_, i) =>
        measurePerformance(`Concurrent Session Creation ${i + 1}`, 100, () =>
          db
            .insert(parlantSession)
            .values(
      ...
      const concurrentMetrics = ctx.metrics.filter((m) =>
        m.operation.startsWith('Concurrent Session Creation')
      )
    ...
    it('should handle concurrent event insertion with proper ordering', async () => {
      ...
      const concurrentEventBatches = Array.from({ length: 5 }, (_, batchIndex) =>
        measurePerformance(`Concurrent Event Batch ${batchIndex + 1}`, 100, () => {
  ...
  describe('Memory and Resource Usage', () => {
    it('should maintain reasonable memory usage during bulk operations', async () => {
      ...
      await measurePerformance(
        'Memory Test - Large Dataset Creation',
        LARGE_DATASET_SIZE * 2,
        async () => {
          const agent = await db
            .insert(parlantAgent)
            .values({
    ...
    it('should handle streaming/pagination efficiently for large datasets', async () => {
      ...
      const streamingMetric = ctx.metrics.find(
        (m) => m.operation === 'Streaming Large Dataset Processing'
      )
