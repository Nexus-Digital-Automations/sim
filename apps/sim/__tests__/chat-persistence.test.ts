...
describe("Chat Persistence System", () => {
  ...
  afterEach(async () => {
    // Cleanup test data
    await db.execute(
      sql`DELETE FROM chat_message WHERE "workspaceId" = ${testWorkspaceId}`,
    );
    await db.execute(
      sql`DELETE FROM chat_conversation WHERE "workspaceId" = ${testWorkspaceId}`,
    );
    await db.execute(
      sql`DELETE FROM chat_browser_session WHERE "workspaceId" = ${testWorkspaceId}`,
    );
    await db.execute(
      sql`DELETE FROM chat_export_request WHERE "workspaceId" = ${testWorkspaceId}`,
    );
    await db.execute(
      sql`DELETE FROM parlant_session WHERE "workspaceId" = ${testWorkspaceId}`,
    );
    await db.execute(
      sql`DELETE FROM parlant_agent WHERE "workspaceId" = ${testWorkspaceId}`,
    );
  ...
  describe("Conversation Management", () => {
    ...
    it("should link session to conversation", async () => {
      ...
      await conversationManager.linkSessionToConversation(
        conversation.id,
        testSessionId,
      );
  ...
  describe("Browser Session Management", () => {
    ...
    it("should restore browser session", async () => {
      ...
      const restoredSession =
        await browserSessionManager.restoreSession(sessionToken);
  ...
  describe("Session Persistence Service", () => {
    ...
    it("should restore existing session", async () => {
      ...
      const restoreResult =
        await newSessionPersistence.restoreExistingSession(sessionToken);
  ...
  describe("Data Export", () => {
    ...
    it("should get export request by token", async () => {
      ...
      const retrievedRequest = await dataExporter.getExportRequest(
    ...
    it("should mark export as completed", async () => {
      ...
      const completedRequest = await dataExporter.getExportRequest(
        exportRequest.requestToken,
      );
  ...
  describe("Data Retention", () => {
    ...
    it("should generate retention report", async () => {
      const report =
        await dataRetention.generateRetentionReport(testWorkspaceId);
...
describe("Performance Tests", () => {
  ...
  it("should efficiently retrieve paginated history", async () => {
    ...
    console.log(
      `Retrieved 5 pages of ${pageSize} messages each in ${endTime - startTime}ms`,
    );
  });
});
