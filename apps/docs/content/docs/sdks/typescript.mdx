...
description: Official TypeScript SDK for the Sim API with advanced type safety, generics, and comprehensive workflow management
---
...
The official Sim TypeScript SDK provides the most comprehensive, type-safe interface to the Sim API. Built specifically for TypeScript projects with advanced generics, strict type checking, and enhanced developer experience.
...
<Tab value="npm">
```bash
npm install @sim/typescript-sdk
```
</Tab>
<Tab value="yarn">
```bash
yarn add @sim/typescript-sdk
```
</Tab>
<Tab value="pnpm">
```bash
pnpm add @sim/typescript-sdk
```
</Tab>
<Tab value="bun">
```bash
bun add @sim/typescript-sdk
```
</Tab>
</Tabs>
...
```typescript
import { SimClient } from '@sim/typescript-sdk';
import type { ClientConfig, Workflow } from '@sim/typescript-sdk/types';
...
const sim = new SimClient(config);

// Or with environment variable
const sim = new SimClient(); // Uses SIM_API_KEY env var
```
...
import type { 
  ...
  WorkflowBlock
} from '@sim/typescript-sdk/types';

// Create workflow with full type checking
...
const workflow: Workflow = await sim.workflows.create(createRequest);

// Type-safe workflow retrieval
const retrieved: Workflow = await sim.workflows.get(workflow.id);

// Type-safe listing with filters
const response: WorkflowListResponse = await sim.workflows.list({
  workspaceId: 'ws_abc123',
  status: 'active', // TypeScript ensures this is a valid status
  limit: 20
});
```
...
- [JavaScript SDK](/docs/sdks/javascript) - Official JavaScript client library
- [Python SDK](/docs/sdks/python) - Official Python client library  
- [API Reference](/docs/api/overview) - Complete API documentation
- [Type Definitions](https://github.com/sim-dev/typescript-sdk/blob/main/src/types.ts) - Complete type definitions
