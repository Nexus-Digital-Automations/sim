  ...
  try {
    ...
    if (toolConfigMatch) {
      ...
      while ((paramMatch = paramBlocksRegex.exec(paramsContent)) !== null) {
        ...
        if (braceCount === 0) {
          const paramBlock = paramsContent
            .substring(startPos + 1, endPos - 1)
            .trim();
          paramPositions.push({
            name: paramName,
    ...
    if (outputsFieldMatch) {
      ...
      console.log(
        `Found tool outputs field for ${toolName}:`,
        Object.keys(outputs),
      );
    }
...
function formatOutputStructure(
  outputs: Record<string, any>,
  indentLevel = 0,
): string {
  let result = "";
  ...
  for (const [key, output] of Object.entries(outputs)) {
    ...
    if (
      ...
      if (output.items?.properties) {
        ...
        const arrayItemsResult = formatOutputStructure(
    ...
    // For objects, expand properties
      ...
      const nestedResult = formatOutputStructure(
        output.properties,
        indentLevel + 1,
      );
      result += nestedResult;
...
function parseToolOutputsField(outputsContent: string): Record<string, any> {
  ...
  while ((match = fieldStartRegex.exec(outputsContent)) !== null) {
    ...
    const openBrace = braces.find(
      (b) => b.type === "open" && b.pos === bracePos,
    );
    if (openBrace) {
      // Find the matching closing brace
      let braceCount = 1;
  ...
  topLevelFields.forEach((field) => {
    const fieldContent = outputsContent
      .substring(field.start + 1, field.end - 1)
      .trim();
...
function parseFieldContent(fieldContent: string): any {
  ...
  const typeMatch = fieldContent.match(/type\s*:\s*['"]([^'"]+)['"]/);
  const descMatch = fieldContent.match(
    /description\s*:\s*['"`]([^'"`\n]+)['"`]/,
  );
  ...
  if (fieldType === "object" || fieldType === "json") {
    ...
    if (propertiesStart !== -1) {
      ...
      if (braceCount === 0) {
        const propertiesContent = fieldContent
          .substring(braceStart + 1, braceEnd - 1)
          .trim();
  ...
  if (itemsStart !== -1) {
    ...
    if (braceCount === 0) {
      const itemsContent = fieldContent
        .substring(braceStart + 1, braceEnd - 1)
        .trim();
      ...
      const searchContent =
        propertiesStart >= 0
          ? itemsContent.substring(0, propertiesStart)
          : itemsContent;
      const itemsDesc = searchContent.match(
        /description\s*:\s*['"`]([^'"`\n]+)['"`]/,
      );
...
function parsePropertiesContent(
  ...
  while ((match = propStartRegex.exec(propertiesContent)) !== null) {
    ...
    if (braceCount === 0) {
      const propContent = propertiesContent
        .substring(startPos + 1, endPos - 1)
        .trim();
